...
- **Doubt (ダウト)**: CLI only (1 human vs 3 CPUs, 10-second async doubt window)
...
- **Doubt (ダウト)**: Entities in `internal/domain/Doubt.go`, `internal/domain/DoubtPlayer.go`; interactor in `internal/usecase/DoubtInteractor.go`. CLI-only (1 human vs 3 CPUs), 10-second async doubt window, random CPU bluff/doubt AI
