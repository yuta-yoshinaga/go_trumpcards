...
test.describe('Écarté E2E', () => {
  test('loads, resets, and renders the play/exchange UI', async ({ page }) => {
    await navigateTo(page, '/ecarte');
    ...
    // Some interactive control must be present: an exchange decision (propose /
    // stand / accept / refuse), a discard / play control, a next-deal advance,
    // or — once the deal resolves via CPU play — the reset / next-game button.
    const anyControl = page
      .getByRole('button', { name: '交換を提案' })
      .or(page.getByRole('button', { name: '勝負' }))
