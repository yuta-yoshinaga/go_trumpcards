...
test.describe('TappTarock E2E', () => {
  test('deals sixteen cards and opens the auction', async ({ page }) => {
    await navigateTo(page, '/tapptarock');
    await expect(page.getByTestId('zw-info')).toBeVisible({ timeout: TIMEOUT_TRANSITION });

    // **入札できるのはドライヤーとソロだけ。** トリシャーケンは全員パスの結果。
    await expect(page.getByTestId('tapp-bid-dreier')).toBeVisible();
    await expect(page.getByTestId('zw-bid-solo')).toBeVisible();
    await expect(page.getByTestId('zw-pass')).toBeVisible();

    await expect(hand(page)).toHaveCount(16);
    // 3 席ぶんの行が出る。
    await expect(page.getByTestId('zw-seat-2')).toBeVisible();
  });
  ...
  test('buries six cards when the talon exchange is offered', async ({ page }) => {
    await navigateTo(page, '/tapptarock');
    await expect(page.getByTestId('tapp-bid-dreier')).toBeVisible({ timeout: TIMEOUT_TRANSITION });
    // 他の席が先に 20番呼び以上を宣言していれば、このボタンは押せない (#5786)。
    if (await page.getByTestId('tapp-bid-dreier').isDisabled()) return;
    await page.getByTestId('tapp-bid-dreier').click();
    await waitForLoaded(page);
    ...
    await expect(hand(page)).toHaveCount(22);
  ...
  test('plays a card and the hand shrinks', async ({ page }) => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if ((await playableCards(page).count()) === 16) break;
  ...
  test('resets from the footer', async ({ page }) => {
    ...
    await expect(hand(page)).toHaveCount(16, { timeout: TIMEOUT_TRANSITION });
    await expect(page.getByTestId('tapp-bid-dreier')).toBeVisible();
  });
});
