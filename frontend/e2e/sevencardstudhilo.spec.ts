...
test.describe('Seven Card Stud Hi-Lo E2E', () => {
  test('plays a full round: reset → check/call through streets → showdown → reset', async ({ page }) => {
    ...
    // The showdown must explain the split. Either the pot halved (a low
    // qualified) or the high took it all -- one of the two lines is always
    // there, and neither depends on the shuffle.
    const split = page.getByTestId('studhilo-split');
    const hiTakesAll = page.getByTestId('studhilo-hi-takes-all');
