...
test.describe('Cruel E2E', () => {
  ...
  test('reset button is present', async ({ page }) => {
    ...
    await expect(page.getByText(/手数/).first()).toBeVisible({ timeout: TIMEOUT_TRANSITION });
    await expect(page.locator('[data-tutorial="cruel-reset-button"]')).toBeVisible({ timeout: TIMEOUT_TRANSITION });
  });

  test('shift button is present and clickable', async ({ page }) => {
    await navigateTo(page, '/cruel');

    await expect(page.getByText(/手数/).first()).toBeVisible({ timeout: TIMEOUT_TRANSITION });
    const shiftButton = page.locator('[data-testid="shift-button"]');
    await expect(shiftButton).toBeVisible({ timeout: TIMEOUT_TRANSITION });
    await shiftButton.click();
    // After shift, the page should still render the game (no error state).
    await expect(page.getByText(/手数/).first()).toBeVisible({ timeout: TIMEOUT_TRANSITION });
  });
