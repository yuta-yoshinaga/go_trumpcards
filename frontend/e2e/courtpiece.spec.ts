...
test.describe('Court Piece (Rang) E2E', () => {
  test('loads, resets, and renders the trump/play UI', async ({ page }) => {
    await navigateTo(page, '/courtpiece');
    ...
    // The game starts in the TrumpDeclaration phase; some interactive control must
    // be present (a trump-suit button, the human's play control, a trick/round
    // advance, or — once the round resolves via CPU play — the reset / next-game
    // button).
    ...
    // If a trump-suit button is offered, declare spades to progress the round.
    const spadeButton = page.getByRole('button', { name: /♠/ });
    if (await isVisibleWithin(spadeButton, TIMEOUT_TRANSITION)) {
      await spadeButton
        .first()
