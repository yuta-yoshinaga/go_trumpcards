...
test.describe('Knockout Whist E2E', () => {
  test('loads, resets, and renders the play UI', async ({ page }) => {
    await navigateTo(page, '/knockoutwhist');
    ...
    // Round / hand-size / trick info renders.
    await expect(page.getByText(/^ラウンド \d+$/).first()).toBeVisible({ timeout: TIMEOUT_TRANSITION });
    await expect(page.getByText(/^手札 \d+枚$/).first()).toBeVisible({ timeout: TIMEOUT_TRANSITION });
    ...
    // (the human's play control, a trick/round advance, or — once the round
    // resolves via CPU play — the reset / next-game button).
