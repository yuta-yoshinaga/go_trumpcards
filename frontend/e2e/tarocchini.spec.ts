...
test.describe('Tarocchini E2E', () => {
  test('loads, resets, and renders the play UI', async ({ page }) => {
    await navigateTo(page, '/tarocchini');
    ...
    // The papi rule is the one thing a player cannot infer from the cards, so it
    // must be on screen in every phase.
    await expect(page.getByTestId('tarocchini-papi-note')).toContainText('後から出した方が勝ち', {
      timeout: TIMEOUT_TRANSITION,
    });

    // This is a team game — the score readout must be per team, not per player.
    await expect(page.getByTestId('tarocchini-team-scores')).toBeVisible({ timeout: TIMEOUT_TRANSITION });
