...
import { sambaApi, sessionId } from './gameApi';
...
describe('sambaApi', () => {
  ...
  it('reset hits /samba/exec with command=reset', async () => {
    mockFetch.mockReturnValue(ok(payload));
    const result = await sambaApi.exec('reset');
    expect(mockFetch).toHaveBeenCalledWith('/samba/exec', {
      method: 'POST',
  ...
  it('reset with config includes the config object', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await sambaApi.exec('reset', undefined, { cpuDifficulty: 2, pointLimit: 15000 });
  ...
  it('drawdiscard forwards the natural pair indices', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await sambaApi.exec('drawdiscard', undefined, undefined, [0, 1]);
    expect(mockFetch).toHaveBeenCalledWith(
      '/samba/exec',
      expect.objectContaining({
  ...
  it('meld forwards the meld groups', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await sambaApi.exec('meld', undefined, undefined, undefined, [[0, 1, 2]]);
    expect(mockFetch).toHaveBeenCalledWith(
      '/samba/exec',
  ...
  it('throws on HTTP error', async () => {
    ...
    await expect(sambaApi.exec('reset')).rejects.toThrow('HTTP error: 500');
  });
});
