...
import { bouillotteApi, sessionId } from './gameApi';

describe('bouillotteApi', () => {
  ...
  it('reset hits /bouillotte/exec with command=reset', async () => {
    mockFetch.mockReturnValue(ok(payload));
    const result = await bouillotteApi.exec('reset');
    expect(mockFetch).toHaveBeenCalledWith('/bouillotte/exec', {
      method: 'POST',
  ...
  it('reset with config includes the config object', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await bouillotteApi.exec('reset', undefined, { playerCount: 3, ante: 20, startingChips: 500, targetRounds: 20 });
  ...
  it('bet call sends command=bet with action=call', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await bouillotteApi.exec('bet', 'call');
    expect(mockFetch).toHaveBeenCalledWith(
      '/bouillotte/exec',
  ...
  it('bet raise sends command=bet with action=raise', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await bouillotteApi.exec('bet', 'raise');
    expect(mockFetch).toHaveBeenCalledWith(
      '/bouillotte/exec',
  ...
  it('bet fold sends command=bet with action=fold', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await bouillotteApi.exec('bet', 'fold');
    expect(mockFetch).toHaveBeenCalledWith(
      '/bouillotte/exec',
  ...
  it('nextround sends command=nextround', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await bouillotteApi.exec('nextround');
    expect(mockFetch).toHaveBeenCalledWith(
      '/bouillotte/exec',
  ...
  it('throws on HTTP error', async () => {
    ...
    await expect(bouillotteApi.exec('reset')).rejects.toThrow('HTTP error: 500');
  });
});
