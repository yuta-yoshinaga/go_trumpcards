...
import { ombreApi, sessionId } from './gameApi';
...
describe('ombreApi', () => {
  ...
  it('reset hits /ombre/exec with command=reset', async () => {
    mockFetch.mockReturnValue(ok(payload));
    const result = await ombreApi.exec('reset');
    expect(mockFetch).toHaveBeenCalledWith('/ombre/exec', {
      method: 'POST',
  ...
  it('reset with config includes the config object', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await ombreApi.exec('reset', { config: { cpuDifficulty: 2, targetRounds: 7 } });
  ...
  it('entrar bid sends bid=1 with the chosen trump suit', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await ombreApi.exec('bid', { bid: 1, trumpSuit: 3 });
    expect(mockFetch).toHaveBeenCalledWith(
      '/ombre/exec',
  ...
  it('play sends command=play with the card index', async () => {
    mockFetch.mockReturnValue(ok(payload));
    await ombreApi.exec('play', { cardIndex: 2 });
    expect(mockFetch).toHaveBeenCalledWith(
      '/ombre/exec',
  ...
  it('throws on HTTP error', async () => {
    ...
    await expect(ombreApi.exec('reset')).rejects.toThrow('HTTP error: 500');
  });
});
