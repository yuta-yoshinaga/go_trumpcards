...
export function SkipNavLink({ targetId, label }: SkipNavLinkProps) {
  return (
    <a
      href={`#${targetId}`}
      className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-50 focus:bg-ds-accent focus:text-white focus:px-4 focus:py-2 focus:rounded"
    >
      {label}
    </a>
  );
