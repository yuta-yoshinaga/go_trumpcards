...
import { calcTongitsHandTotal, TongitsOnDealCelebration } from './TongitsOnDealCelebration';
...
describe('calcTongitsHandTotal', () => {
  it('uses Gin-Rummy values: A=1, 2-9 face, 10/J/Q/K=10', () => {
    ...
    // 1 + 7 + 10 + 10 + 10 = 38 (not Tongits, but the helper is unit-tested)
    expect(calcTongitsHandTotal(hand)).toBe(38);
  });

  it('returns 50 for the classic Tongits-on-deal high case (5 face cards)', () => {
    const hand = [card('SPADE', 13), card('HEART', 12), card('DIAMOND', 11), card('CLOVER', 13), card('SPADE', 12)];
    expect(calcTongitsHandTotal(hand)).toBe(50);
  });

  it('returns 49 for the Tongits-on-deal low case (4 face cards + 9)', () => {
    const hand = [card('SPADE', 13), card('HEART', 12), card('DIAMOND', 11), card('CLOVER', 13), card('SPADE', 9)];
    expect(calcTongitsHandTotal(hand)).toBe(49);
  });

  it('treats Joker as 0 to be defensive against unexpected input', () => {
    const hand = [card('JOKER', 0)];
    expect(calcTongitsHandTotal(hand)).toBe(0);
  });
});

describe('TongitsOnDealCelebration', () => {
  it('keeps the aria-live container in the DOM but hidden when show=false', () => {
    render(<TongitsOnDealCelebration show={false} />);
    ...
    const container = screen.getByTestId('tongits-on-deal-celebration');
    expect(container).toBeInTheDocument();
    expect(container).toHaveAttribute('data-visible', 'false');
    expect(container).toHaveClass('invisible');
    expect(screen.queryByText('TONGITS!')).not.toBeInTheDocument();
  });

  it('renders the TONGITS! banner when show=true', () => {
    render(
  ...
  it('auto-dismisses after the configured delay', () => {
    render(<TongitsOnDealCelebration show={true} dismissAfterMs={1500} winnerCards={[card('SPADE', 13)]} />);
    expect(screen.getByTestId('tongits-on-deal-celebration')).toHaveAttribute('data-visible', 'true');
    act(() => {
  ...
  it('keeps banner visible when dismissAfterMs=0', () => {
    render(<TongitsOnDealCelebration show={true} dismissAfterMs={0} winnerCards={[]} />);
    vi.advanceTimersByTime(10_000);
    expect(screen.getByTestId('tongits-on-deal-celebration')).toHaveAttribute('data-visible', 'true');
  });

  it('uses assertive aria-live so screen readers announce the win', () => {
    render(<TongitsOnDealCelebration show={true} />);
    expect(screen.getByTestId('tongits-on-deal-celebration')).toHaveAttribute('aria-live', 'assertive');
  });
});
