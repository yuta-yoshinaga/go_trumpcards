...
/** Props for {@link TongitsOnDealCelebration}. */
export interface TongitsOnDealCelebrationProps {
  /** Whether the player or CPU achieved Tongits on the deal. */
  show: boolean;
  /** Cards that produced the Tongits total — used to compute the 49 / 50 sum displayed. */
  winnerCards?: Card[];
...
/** Tongits uses Gin Rummy values: A=1, 2-9 face, 10/J/Q/K=10. Joker is unused for Tongits. */
function tongitsCardValue(card: Card): number {
...
/** Sum of Gin-Rummy card values across a hand — produces 49 or 50 when Tongits-on-deal triggers. */
export function calcTongitsHandTotal(cards: Card[]): number {
  return cards.reduce((sum, c) => sum + tongitsCardValue(c), 0);
}

/**
 * Full-screen celebratory overlay shown once when Tongits-on-deal (49/50 hand sum) triggers.
 * Auto-dismisses after `dismissAfterMs` so the round-end UI behind it stays reachable.
 */
export function TongitsOnDealCelebration({
  show,
  winnerCards = [],
  winnerName,
  dismissAfterMs = 3500,
}: TongitsOnDealCelebrationProps): ReactElement | null {
  const { t } = useTranslation('tongits');
  ...
  const points = calcTongitsHandTotal(winnerCards);
  ...
  return (
    <div
      role="status"
      aria-live="assertive"
      data-testid="tongits-on-deal-celebration"
      ...
      {visible && (
        ...
        >
          <div className="text-ds-accent text-5xl sm:text-6xl font-bold tracking-widest drop-shadow">
            {t('tongitsOnDealBanner.title')}
          </div>
          {points > 0 && (
            <div className="text-ds-text-primary text-xl sm:text-2xl mt-2">
              {t('tongitsOnDealBanner.points', { points })}
            </div>
          )}
          {winnerName && (
            <div className="text-ds-text-muted text-base mt-1">
              {t('tongitsOnDealBanner.winner', { name: winnerName })}
            </div>
          )}
        </motion.div>
      )}
