...
export function useDiplomatGame() {
  const { state, loading, error, exec: rawExec, retry } = useGameApi(diplomatApi.exec);
  const [selectedSource, setSelectedSource] = useState<DiplomatMoveZone | null>(null);
  const [hint, setHint] = useState<DiplomatHint | null>(null);
  ...
  const handleHint = useHintRequest({
    fetchHint: () => diplomatApi.exec('hint'),
