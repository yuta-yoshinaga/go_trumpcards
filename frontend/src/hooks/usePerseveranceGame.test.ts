...
import { usePerseveranceGame } from './usePerseveranceGame';
...
const baseState: PerseveranceResponse = {
...
describe('usePerseveranceGame', () => {
  ...
  it('calls reset on mount', async () => {
    renderHook(() => usePerseveranceGame(), { wrapper: makeWrapper() });
    await waitFor(() => expect(mockExec).toHaveBeenCalledWith('reset'));
  });

  it('handleReset / handleGiveUp / handleUndo forward to exec', async () => {
    const { result } = renderHook(() => usePerseveranceGame(), { wrapper: makeWrapper() });
