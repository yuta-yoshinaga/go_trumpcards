...
export function useCongressGame() {
  const { state, loading, error, exec: rawExec, retry } = useGameApi(congressApi.exec);
  const [selectedSource, setSelectedSource] = useState<CongressMoveZone | null>(null);
  const [hint, setHint] = useState<CongressHint | null>(null);
  ...
  const handleHint = useHintRequest({
    fetchHint: () => congressApi.exec('hint'),
