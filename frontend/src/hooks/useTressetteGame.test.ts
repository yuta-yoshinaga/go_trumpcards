...
const defaultState = makeTressetteState();

beforeEach(() => {
  mockExec.mockResolvedValue(defaultState);
});

describe('useTressetteGame', () => {
  it('calls reset on mount with default config', async () => {
    renderHook(() => useTressetteGame(), { wrapper: createWrapper() });
    await waitFor(() =>
  ...
  it('returns initial state after mount', async () => {
    const { result } = renderHook(() => useTressetteGame(), { wrapper: createWrapper() });
