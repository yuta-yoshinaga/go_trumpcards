...
const defaultState = makeSambaState();

beforeEach(() => {
  mockExec.mockResolvedValue(defaultState);
});

describe('useSambaGame', () => {
  it('calls reset on mount with default config', async () => {
    renderHook(() => useSambaGame(), { wrapper: createWrapper() });
    await waitFor(() =>
  ...
  it('handleConfigChange updates config with a valid number', async () => {
    ...
    expect(result.current.sambaConfig.pointLimit).toBe(7500);
  });
  ...
  it('handleConfigChange ignores NaN values', async () => {
    ...
    expect(result.current.sambaConfig.pointLimit).toBe(10000);
  });
