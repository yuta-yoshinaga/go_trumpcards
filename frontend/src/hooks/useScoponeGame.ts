...
import { type ScoponeConfigInput, scoponeApi } from '../api/gameApi';
...
export function useScoponeGame() {
  const [handIndex, setHandIndex] = useState<number | null>(null);
  const [tableIndices, setTableIndices] = useState<number[]>([]);
  const [configInput, setConfigInput] = useState<ScoponeConfigInput>(defaultConfigInput);
  ...
  const onSuccess = useCallback(
    async (_res: ScoponeResponse) => {
      clearSelection();
    },
    [clearSelection],
  );

  const { loading, error, state, exec: callApi, retry } = useGameApi(scoponeApi.exec, { onSuccess });

  useEffect(() => {
    callApi('r');
  }, [callApi]);

  const handleConfigChange = useCallback((key: keyof ScoponeConfigInput, value: number) => {
