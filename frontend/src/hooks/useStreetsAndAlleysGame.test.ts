...
const baseState: StreetsAndAlleysResponse = {
...
describe('useStreetsAndAlleysGame', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('calls reset on mount', async () => {
    renderHook(() => useStreetsAndAlleysGame(), { wrapper: makeWrapper() });
    await waitFor(() => expect(mockExec).toHaveBeenCalledWith('reset'));
  });

  it('handleReset / handleGiveUp / handleUndo forward to exec', async () => {
    const { result } = renderHook(() => useStreetsAndAlleysGame(), { wrapper: makeWrapper() });
