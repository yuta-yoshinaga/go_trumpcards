...
import { useSomersetGame } from './useSomersetGame';
...
describe('useSomersetGame', () => {
  ...
  it('calls reset on mount', async () => {
    renderHook(() => useSomersetGame(), { wrapper: makeWrapper() });
    await waitFor(() => expect(mockExec).toHaveBeenCalledWith('reset'));
  });

  it('handleReset / handleGiveUp / handleUndo forward to exec', async () => {
    const { result } = renderHook(() => useSomersetGame(), { wrapper: makeWrapper() });
