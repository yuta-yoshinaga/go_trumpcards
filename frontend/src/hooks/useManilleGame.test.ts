...
beforeEach(() => {
  mockExec.mockReset();
  mockExec.mockResolvedValue(makeManilleState());
});

describe('useManilleGame', () => {
  ...
  it('handleConfigChange updates the config used by reset', async () => {
    ...
    act(() => result.current.reset());
    await waitFor(() =>
      expect(mockExec).toHaveBeenCalledWith('reset', { config: { ...DEFAULT_MANILLE_CONFIG, targetPoints: 51 } }),
    );
  });
});
