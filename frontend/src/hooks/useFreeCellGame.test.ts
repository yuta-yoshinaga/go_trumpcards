...
const defaultState: FreeCellResponse = {
  tableau: [[], [], [], [], [], [], [], []],
...
describe('useFreeCellGame', () => {
  it('calls reset on mount', async () => {
    renderHook(() => useFreeCellGame(), { wrapper: createWrapper() });
  ...
  it('handleReset dispatches reset command and clears selection and hint', async () => {
    const hintResponse: FreeCellResponse = {
      ...defaultState,
  ...
  it('handleSelectSource with cell field (freecell zone)', async () => {
    const { result } = renderHook(() => useFreeCellGame(), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.state).not.toBeNull());

    act(() => {
      result.current.handleSelectSource({ zone: 'freecell', cell: 2 });
    });

    expect(result.current.selectedSource).toEqual({ zone: 'freecell', cell: 2 });
  });

  it('handleSelectSource toggles off freecell source when same cell clicked', async () => {
    const { result } = renderHook(() => useFreeCellGame(), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.state).not.toBeNull());
