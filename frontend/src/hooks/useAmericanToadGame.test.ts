...
import { useAmericanToadGame } from './useAmericanToadGame';
...
const baseState: AmericanToadResponse = {
  reserve: [],
...
describe('useAmericanToadGame', () => {
  ...
  it('calls reset on mount', async () => {
    renderHook(() => useAmericanToadGame(), { wrapper: makeWrapper() });
  ...
  it('handleSelectSource toggles the reserve and the waste', async () => {
    const { result } = renderHook(() => useAmericanToadGame(), { wrapper: makeWrapper() });
    await waitFor(() => expect(mockExec).toHaveBeenCalled());
  ...
  it('handleSelectTarget moves a reserve card through the move API', async () => {
    ...
    await waitFor(() =>
      expect(mockExec).toHaveBeenCalledWith('move', { zone: 'reserve' }, { zone: 'tableau', col: 3 }),
    );
    expect(result.current.selectedSource).toBeNull();
  });
});
