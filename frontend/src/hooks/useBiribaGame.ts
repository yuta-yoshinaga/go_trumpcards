...
import type { BiribaConfig } from '../types/card';
import { useCardSelection } from './useCardSelection';
import { useGameApi } from './useGameApi';
import { useGameConfig } from './useGameConfig';

/** Default Biriba game configuration. */
export const DEFAULT_CANASTA_CONFIG: BiribaConfig = {
  cpuDifficulty: 1,
  pointLimit: 5000,
};

/** CPU difficulty level options for Biriba. */
...
/** Available point limit options for Biriba. */
export const POINT_LIMIT_OPTIONS = [3000, 5000, 7500, 10000] as const;

/** Hook that manages Biriba game state and player actions. */
export function useBiribaGame() {
  const { selected: selectedCardIndices, toggle: toggleCard, clear: clearSelection } = useCardSelection();
  const { config: biribaConfig, handleConfigChange } = useGameConfig<BiribaConfig>(DEFAULT_CANASTA_CONFIG);

  const onSuccess = useCallback(() => {
    clearSelection();
  }, [clearSelection]);
  const { state, loading, error, exec: rawExec, retry } = useGameApi(biribaApi.exec, { onSuccess });
