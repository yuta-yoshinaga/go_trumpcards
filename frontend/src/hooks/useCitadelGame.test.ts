...
import { useCitadelGame } from './useCitadelGame';
...
const baseState: CitadelResponse = {
...
describe('useCitadelGame', () => {
  ...
  it('calls reset on mount', async () => {
    renderHook(() => useCitadelGame(), { wrapper: makeWrapper() });
    await waitFor(() => expect(mockExec).toHaveBeenCalledWith('reset'));
  });

  it('handleReset / handleGiveUp / handleUndo forward to exec', async () => {
    const { result } = renderHook(() => useCitadelGame(), { wrapper: makeWrapper() });
