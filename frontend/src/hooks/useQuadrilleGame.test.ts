...
import { DEFAULT_QUADRILLE_CONFIG, useQuadrilleGame } from './useQuadrilleGame';
...
beforeEach(() => {
  mockExec.mockReset();
  mockExec.mockResolvedValue(makeQuadrilleState());
});

describe('useQuadrilleGame', () => {
  it('reset dispatches reset with the default config', async () => {
    const { result } = renderHook(() => useQuadrilleGame(), { wrapper: createWrapper() });
    act(() => result.current.reset());
    await waitFor(() => expect(mockExec).toHaveBeenCalledWith('reset', { config: DEFAULT_QUADRILLE_CONFIG }));
  });
  ...
  it('handleConfigChange updates the config used by reset', async () => {
    ...
    act(() => result.current.reset());
    await waitFor(() =>
      expect(mockExec).toHaveBeenCalledWith('reset', { config: { ...DEFAULT_QUADRILLE_CONFIG, targetRounds: 7 } }),
    );
  });
});
