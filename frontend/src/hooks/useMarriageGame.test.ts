...
import { useMarriageGame } from './useMarriageGame';
...
const defaultState: MarriageResponse = {
  players: [
    {
      id: 0,
      isHuman: true,
...
describe('useMarriageGame', () => {
  ...
  it('handleConfigChange updates config with a valid number', async () => {
    ...
    expect(result.current.marriageConfig.playerCount).toBe(3);
  });
  ...
  it('handleConfigChange ignores NaN values', async () => {
    ...
    expect(result.current.marriageConfig.playerCount).toBe(5);
  });
