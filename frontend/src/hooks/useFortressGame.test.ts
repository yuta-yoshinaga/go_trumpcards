...
import { useFortressGame } from './useFortressGame';
...
const baseState: FortressResponse = {
...
describe('useFortressGame', () => {
  ...
  it('calls reset on mount', async () => {
    renderHook(() => useFortressGame(), { wrapper: makeWrapper() });
    await waitFor(() => expect(mockExec).toHaveBeenCalledWith('reset'));
  });

  it('handleReset / handleGiveUp / handleUndo forward to exec', async () => {
    const { result } = renderHook(() => useFortressGame(), { wrapper: makeWrapper() });
