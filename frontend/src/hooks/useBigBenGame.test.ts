...
import { useBigBenGame } from './useBigBenGame';
...
const baseState: BigBenResponse = {
...
describe('useBigBenGame', () => {
  ...
  it('calls reset on mount', async () => {
    renderHook(() => useBigBenGame(), { wrapper: makeWrapper() });
    await waitFor(() => expect(mockExec).toHaveBeenCalledWith('reset'));
  });

  it('forwards the simple commands', async () => {
    const { result } = renderHook(() => useBigBenGame(), { wrapper: makeWrapper() });
  ...
  it('handleHint stores the hint payload from the API', async () => {
    mockExec.mockResolvedValueOnce(baseState).mockResolvedValueOnce({
      ...baseState,
      hint: { fromZone: 'tableau', fromCol: 1, toZone: 'foundation', toIdx: 4 },
    });
    const { result } = renderHook(() => useBigBenGame(), { wrapper: makeWrapper() });
    await waitFor(() => expect(mockExec).toHaveBeenCalled());
    ...
    expect(result.current.hint).toEqual({ fromZone: 'tableau', fromCol: 1, toZone: 'foundation', toIdx: 4 });
  });
  ...
  it('clears the selection and the hint on a board-resetting action', async () => {
    mockExec.mockResolvedValueOnce(baseState).mockResolvedValueOnce({
      ...baseState,
      hint: { fromZone: 'tableau', fromCol: 1, toZone: 'tableau', toIdx: 4 },
    });
    const { result } = renderHook(() => useBigBenGame(), { wrapper: makeWrapper() });
    await waitFor(() => expect(mockExec).toHaveBeenCalled());
