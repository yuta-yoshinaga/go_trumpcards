...
import { type PreferenceConfigInput, preferenceApi } from '../api/gameApi';
import { useCardSelection } from './useCardSelection';
import { useGameApi } from './useGameApi';
import { useGameConfig } from './useGameConfig';

/** Default Préférence game configuration. */
export const DEFAULT_PREFERENCE_CONFIG: Required<PreferenceConfigInput> = {
  cpuDifficulty: 1,
  targetPoints: 30,
};

/** CPU difficulty level options for Préférence. */
...
/** Available target match-point options for Préférence (first player to reach wins). */
export const TARGET_POINTS_OPTIONS = [20, 30, 40] as const;

/**
 * Hook that manages Préférence game state, the bid action, the play action, and
...
export function usePreferenceGame() {
  ...
  const { config, handleConfigChange } = useGameConfig<Required<PreferenceConfigInput>>(DEFAULT_PREFERENCE_CONFIG);

  const onSuccess = useCallback(() => {
    clearSelection();
  }, [clearSelection]);

  const { state, loading, error, exec, retry } = useGameApi(preferenceApi.exec, { onSuccess });
