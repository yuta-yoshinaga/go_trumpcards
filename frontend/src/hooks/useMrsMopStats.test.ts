...
describe('applyMrsMopResult (pure reducer)', () => {
  it('a win increments plays and wins and sets initial best score/fewest moves', () => {
    const { stats, update } = applyMrsMopResult({}, { difficulty: 1, won: true, score: 500, moves: 42 });
  ...
  it('a loss increments plays only and leaves bests null', () => {
    const { stats, update } = applyMrsMopResult({}, { difficulty: 2, won: false, score: 120, moves: 90 });
  ...
  it('a better win updates best score and fewest moves', () => {
    const first = applyMrsMopResult({}, { difficulty: 1, won: true, score: 400, moves: 60 }).stats;
    const { stats, update } = applyMrsMopResult(first, { difficulty: 1, won: true, score: 550, moves: 45 });
    expect(stats['1']).toEqual({ plays: 2, wins: 2, bestScore: 550, fewestMoves: 45 });
    expect(update).toEqual({ newBestScore: true, newFewestMoves: true });
  });

  it('a worse win keeps existing bests', () => {
    const first = applyMrsMopResult({}, { difficulty: 1, won: true, score: 550, moves: 45 }).stats;
    const { stats, update } = applyMrsMopResult(first, { difficulty: 1, won: true, score: 300, moves: 80 });
    expect(stats['1']).toEqual({ plays: 2, wins: 2, bestScore: 550, fewestMoves: 45 });
    expect(update).toEqual({ newBestScore: false, newFewestMoves: false });
  });

  it('keeps difficulties separate', () => {
    const s1 = applyMrsMopResult({}, { difficulty: 1, won: true, score: 500, moves: 40 }).stats;
    const s2 = applyMrsMopResult(s1, { difficulty: 4, won: false, score: 100, moves: 30 }).stats;
...
describe('mrsMopWinRate', () => {
  it('is 0 with no plays', () => {
    expect(mrsMopWinRate(emptyMrsMopStat())).toBe(0);
  });

  it('rounds to a whole percentage', () => {
    expect(mrsMopWinRate({ plays: 3, wins: 1, bestScore: null, fewestMoves: null })).toBe(33);
  });
});

describe('readMrsMopStats', () => {
  it('returns {} when nothing stored', () => {
    expect(readMrsMopStats()).toEqual({});
  });

  it('ignores malformed json', () => {
    localStorage.setItem(MRSMOP_STATS_KEY, 'not-json');
    expect(readMrsMopStats()).toEqual({});
  });
...
describe('useMrsMopStats', () => {
  it('records a win, persists it, and reads it back', () => {
    ...
    expect(readMrsMopStats()['1']).toEqual({ plays: 1, wins: 1, bestScore: 480, fewestMoves: 55 });
  });
  ...
  it('getStat returns an empty stat for an untouched difficulty', () => {
    const { result } = renderHook(() => useMrsMopStats());
    expect(result.current.getStat(4)).toEqual(emptyMrsMopStat());
  });
});
