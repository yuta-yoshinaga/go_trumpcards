...
const baseState: BeleagueredCastleResponse = {
...
describe('useBeleagueredCastleGame', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('calls reset on mount', async () => {
    renderHook(() => useBeleagueredCastleGame(), { wrapper: makeWrapper() });
    await waitFor(() => expect(mockExec).toHaveBeenCalledWith('reset'));
  });

  it('handleReset / handleGiveUp / handleUndo forward to exec', async () => {
    const { result } = renderHook(() => useBeleagueredCastleGame(), { wrapper: makeWrapper() });
