// Type declarations for minchiate. Split out of card.ts (issue #4366);
// card.ts re-exports this file, so existing imports keep working.

import type { BaseGameResponse, Card } from '../common';

/** Minchiate game phase (0=Scarto 1=Play 2=TrickEnd 3=RoundEnd 4=GameEnd). */
export type MinchiatePhaseValue = 0 | 1 | 2 | 3 | 4;

/** A Minchiate player's public/own state. Cards are non-empty only for the human. */
export interface MinchiatePlayer {
  id: number;
  isHuman: boolean;
  cardCount: number;
  cards: Card[];
...
/** A card played into the current Minchiate trick. */
export interface MinchiateTrickCard {
  playerIdx: number;
  card: Card;
}

/** Minchiate game configuration. */
export interface MinchiateConfig {
  cpuDifficulty: number;
  /** Constrained to a multiple of the player count so the deal goes round once. */
  targetRounds: number;
}

/** A suggested hint for Minchiate, computed by the backend. */
export interface MinchiateHint {
  cardIndices: number[];
...
export interface MinchiateResponse extends BaseGameResponse {
  ...
  hint?: MinchiateHint | null;
  config: MinchiateConfig;
}

/** Cards the dealer must bury. 97 does not divide by 4, so 13 are always left over. */
export const MINCHIATE_SURPLUS = 13;

/** Number of players. Opposite seats partner into two fixed teams. */
export const MINCHIATE_PLAYER_COUNT = 4;

/** The team a seat belongs to. Mirrors `domain.MinchiateTeamOf`. */
export function minchiateTeamOf(seat: number): number {
  return seat % 2;
}
