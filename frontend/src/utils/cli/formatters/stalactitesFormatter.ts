...
/** Format a Stalactites game state as terminal text. */
export function formatStalactitesState(state: StalactitesResponse): string {
  const lines: string[] = [];

  lines.push(formatHeader('Stalactites'));

  // Free cells
  const cells = state.cells.map((c, i) => `[c${i}]${c ? formatCard(c) : '    '}`);
