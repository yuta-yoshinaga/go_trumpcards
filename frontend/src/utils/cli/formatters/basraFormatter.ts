...
/** Format a Basra (Bastra) game state as terminal text. */
export function formatBasraState(state: BasraResponse): string {
  const lines: string[] = [];

  lines.push(formatHeader('Basra'));
  lines.push(
  ...
  for (const p of state.players) {
    ...
    lines.push(`${name}: ${p.cardCount} cards  captured=${p.capturedCount}  basra=${p.basraCount}`);
