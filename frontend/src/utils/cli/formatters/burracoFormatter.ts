...
/** Format a Burraco game state as terminal text. */
export function formatBurracoState(state: BurracoResponse): string {
  const lines: string[] = [];

  lines.push(formatHeader('Burraco'));
  ...
  lines.push(`pozzetti remaining: ${state.pozzettoCount}`);
  lines.push('');

  for (const p of state.players) {
    const name = formatPlayerName(p.id, p.isHuman);
    const tags: string[] = [];
    if (p.hasBurraco) tags.push('Burraco');
    if (p.hasInitMeld) tags.push('Init Meld');
    if (p.tookPozzetto) tags.push('Pozzetto');
    ...
    if (p.melds.length > 0) {
      for (const m of p.melds) {
        const type = m.isBurraco ? '(burraco)' : m.isNatural ? '(natural)' : '';
