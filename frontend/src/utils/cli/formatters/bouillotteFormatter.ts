...
/** Format a Bouillotte game state as terminal text. */
export function formatBouillotteState(state: BouillotteResponse): string {
  const lines: string[] = [];

  lines.push(formatHeader('Bouillotte'));
  ...
  if (state.retourne) {
    lines.push(`retourne: ${formatCard(state.retourne)}`);
  }
  lines.push('');
