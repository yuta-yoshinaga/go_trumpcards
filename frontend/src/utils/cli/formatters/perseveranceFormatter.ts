...
/** Format a Perseverance game state as terminal text. */
export function formatPerseveranceState(state: PerseveranceResponse): string {
  const lines: string[] = [];

  lines.push(formatHeader('Perseverance'));
  ...
  lines.push(`moves: ${state.moveCount}  redeals:${state.redealsLeft}  undo:${state.canUndo ? 'yes' : 'no'}`);
