...
/** Format a Biriba game state as terminal text. */
export function formatBiribaState(state: BiribaResponse): string {
  const lines: string[] = [];

  lines.push(formatHeader('Biriba'));
  ...
  for (const p of state.players) {
    ...
    const tags: string[] = [];
    if (p.hasBiriba) tags.push('Biriba');
    ...
    if (p.melds.length > 0) {
      for (const m of p.melds) {
        const type = m.isBiriba ? '(biriba)' : m.isNatural ? '(natural)' : '';
