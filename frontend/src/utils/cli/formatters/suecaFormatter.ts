...
/** Format a Sueca game state as terminal text. */
export function formatSuecaState(state: SuecaResponse): string {
  const lines: string[] = [];

  lines.push(formatHeader('Sueca'));
  lines.push(
  ...
  lines.push(`game points: A=${state.teamGamePoints[0] ?? 0}  B=${state.teamGamePoints[1] ?? 0}`);
  lines.push('');
