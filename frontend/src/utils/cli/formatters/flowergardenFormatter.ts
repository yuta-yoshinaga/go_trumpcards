...
export function formatFlowerGardenState(state: FlowerGardenResponse): string {
  ...
  lines.push(formatHeader('Flower Garden'));

  // Foundations (4 suits)
  const fnd = state.foundation.map((pile) => (pile.length > 0 ? formatCard(pile[pile.length - 1]) : '[  ]'));
  lines.push(`foundation: ${fnd.join(' | ')}`);

  // Reserve (the bouquet) — 16 one-way cards; null cells render empty.
  const res = state.reserve.map((c) => (c ? formatCard(c) : '[  ]'));
  lines.push(`reserve: ${res.join(' | ')}`);
  lines.push('----------');
