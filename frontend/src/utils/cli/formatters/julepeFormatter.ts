...
const PHASE_NAMES: Record<number, string> = {
  [JulepePhase.DECIDE]: 'DECIDE',
  [JulepePhase.PLAY]: 'PLAY',
  [JulepePhase.ROUND_END]: 'ROUND END',
  [JulepePhase.GAME_END]: 'GAME END',
};
...
function statusStr(p: JulepePlayer): string {
  if (!p.decided) return '-';
  return p.inRound ? 'in' : 'out';
}

/** Format a Julepe game state as terminal text. */
export function formatJulepeState(state: JulepeResponse | null): string {
  if (!state) return 'Loading...';
  const lines: string[] = [];

  lines.push(formatHeader('Julepe'));
  lines.push(
