...
/** Format a Gin Rummy game state as terminal text. */
export function formatGinrummyState(state: GinRummyResponse): string {
  const lines: string[] = [];

  lines.push(formatHeader('Gin Rummy'));
  ...
  if (state.knockerIdx >= 0) {
    ...
    lines.push(`${knocker} knocked!${state.isGin ? ' (GIN!)' : ''}`);
