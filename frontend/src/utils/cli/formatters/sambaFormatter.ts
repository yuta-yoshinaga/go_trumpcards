import type { SambaMeldData, SambaResponse } from '../../../types/card';
import {
  formatCard,
  formatCardList,
  formatHeader,
...
function meldLabel(m: SambaMeldData): string {
  if (m.kind === 1) {
    return m.isSamba ? '(samba)' : '(sequence)';
  }
  if (m.isCanasta) return '(canasta)';
  return m.isNatural ? '(natural)' : '(set)';
}

/** Format a Samba game state as terminal text. */
export function formatSambaState(state: SambaResponse): string {
  const lines: string[] = [];

  lines.push(formatHeader('Samba'));
