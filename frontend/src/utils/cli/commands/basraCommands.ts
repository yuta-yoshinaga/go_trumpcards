...
type BasraArgs = Parameters<typeof basraApi.exec>;

const VALID_COMMANDS = ['p', 'play', 'n', 'next', 'nr', 'nextround', 'h', 'hint', 'r', 'reset', 'help', '?'];

/** Parse a Basra (Bastra) CLI command into API exec arguments. */
export function parseBasraCommand(input: string): CliParseResult<BasraArgs> {
...
/** Help text for Basra (Bastra) CLI mode. */
export const BASRA_HELP: string[] = [
