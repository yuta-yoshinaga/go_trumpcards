...
export function parseBeleagueredCastleCommand(input: string): CliParseResult<BeleagueredCastleArgs> {
...
function parseMoveCommand(args: string[]): CliParseResult<BeleagueredCastleArgs> {
  if (args.length < 2) {
