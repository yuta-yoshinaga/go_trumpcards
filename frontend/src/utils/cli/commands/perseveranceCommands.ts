...
/** Parse a Perseverance CLI command into API exec arguments. */
export function parsePerseveranceCommand(input: string): CliParseResult<PerseveranceArgs> {
...
function parseMoveCommand(args: string[]): CliParseResult<PerseveranceArgs> {
  if (args.length < 2) {
...
/** Help text for Perseverance CLI mode. */
export const PERSEVERANCE_HELP: string[] = [
  ...
  'rd/redeal       - Gather and re-deal (twice per game)',
