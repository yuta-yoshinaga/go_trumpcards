import type { fortressApi } from '../../../api/gameApi';
import { splitCommand, suggestCommand } from '../commandParserBase';
import type { CliParseResult } from '../types';

type FortressArgs = Parameters<typeof fortressApi.exec>;
...
/** Parse a Fortress CLI command into API exec arguments. */
export function parseFortressCommand(input: string): CliParseResult<FortressArgs> {
...
function parseMoveCommand(args: string[]): CliParseResult<FortressArgs> {
  if (args.length < 2) {
...
/** Help text for Fortress CLI mode. */
export const FORTRESS_HELP: string[] = [
