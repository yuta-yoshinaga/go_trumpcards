import type { bigtwoApi } from '../../../api/gameApi';
import type { BigTwoResponse } from '../../../types/card';
import { parseIntSlice, splitCommand, suggestCommand } from '../commandParserBase';
import type { CliParseResult } from '../types';

/** Args tuple accepted by bigtwoApi.exec. */
export type BigTwoCliArgs = Parameters<typeof bigtwoApi.exec>;

const VALID_COMMANDS = ['p', 'play', 'r', 'reset', 'help', '?'];

/** Parses a single CLI command line for the Big Two game. */
export function parseBigTwoCommand(input: string): CliParseResult<BigTwoCliArgs> {
...
/** Renders a Big Two game state as CLI-friendly text. */
export function formatBigTwoState(s: BigTwoResponse): string {
  const lines: string[] = [];
...
/** Help text shown in the CLI terminal for Big Two. */
export const BIGTWO_HELP = [
