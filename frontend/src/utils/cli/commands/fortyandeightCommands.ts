...
/** Parse a Forty and Eight CLI command into API exec arguments. */
export function parseFortyandeightCommand(input: string): CliParseResult<FortyAndEightArgs> {
...
function parseMoveCommand(args: string[]): CliParseResult<FortyAndEightArgs> {
  if (args.length < 2) {
...
/** Help text for Forty and Eight CLI mode. */
export const FORTYANDEIGHT_HELP: string[] = [
  'd/draw          - Draw a card from the stock',
  'rd/redeal       - Recycle waste into stock (once per game)',
