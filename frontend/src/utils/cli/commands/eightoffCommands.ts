import type { eightoffApi } from '../../../api/gameApi';
import { parseIntArg, splitCommand, suggestCommand } from '../commandParserBase';
import type { CliParseResult } from '../types';

type EightOffArgs = Parameters<typeof eightoffApi.exec>;
...
/** Parse an Eight Off CLI command into API exec arguments. */
export function parseEightOffCommand(input: string): CliParseResult<EightOffArgs> {
...
function parseMoveCommand(args: string[]): CliParseResult<EightOffArgs> {
  if (args.length < 2) return { error: 'Usage: m <from> <to> (e.g., m t0 f, m t0 3 t1, m t0 c4, m c0 t1)' };
...
/** Help text for Eight Off CLI mode. */
export const EIGHTOFF_HELP: string[] = [
  'm t<c> t<c> - Move tableau top to column (same suit only)',
  'm t<c> <i> t<c> - Move card at index to column',
  'm t<c> f    - Move to foundation',
  'm t<c> c<n> - Move to free cell (0-7)',
