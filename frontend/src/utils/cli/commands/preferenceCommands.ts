...
/** Args tuple accepted by preferenceApi.exec. */
export type PreferenceCliArgs = Parameters<typeof preferenceApi.exec>;
...
/**
 * Parses a single CLI command line for the Préférence game into
 * {@link preferenceApi}.exec arguments.
 *
 * Préférence has a bidding phase (`bid <0-4>` / `pass`) followed by trick play
 * (`play <idx>`). `sd <0-2>` resets the game with a new CPU difficulty because
 * config is only accepted on the `reset` command.
 */
export function parsePreferenceCommand(input: string): CliParseResult<PreferenceCliArgs> {
...
/** Help text shown in the CLI terminal for Préférence. */
