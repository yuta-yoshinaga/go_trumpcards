import type { streetsAndAlleysApi } from '../../../api/gameApi';
import { splitCommand, suggestCommand } from '../commandParserBase';
import type { CliParseResult } from '../types';

type StreetsAndAlleysArgs = Parameters<typeof streetsAndAlleysApi.exec>;
...
/** Parse a Streets and Alleys CLI command into API exec arguments. */
export function parseStreetsAndAlleysCommand(input: string): CliParseResult<StreetsAndAlleysArgs> {
...
function parseMoveCommand(args: string[]): CliParseResult<StreetsAndAlleysArgs> {
  if (args.length < 2) {
...
/** Help text for Streets and Alleys CLI mode. */
export const STREETSANDALLEYS_HELP: string[] = [
