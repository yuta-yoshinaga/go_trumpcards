...
/** Args tuple accepted by tarocchiniApi.exec. */
export type TarocchiniCliArgs = Parameters<typeof tarocchiniApi.exec>;
...
/**
 * Parses a single CLI command line for Tarocchini into
 * {@link tarocchiniApi}.exec arguments.
 *
 * Tarocchini has no bidding phase: the dealer buries the surplus (`scarto`)
 ...
 */
export function parseTarocchiniCommand(input: string): CliParseResult<TarocchiniCliArgs> {
  ...
  switch (cmd) {
    ...
    case 'discard': {
      ...
      // ときに案内だけ古くなる。
      if (args.length < TAROCCHINI_SURPLUS) {
        return { error: `Usage: scarto <i0> <i1> (${TAROCCHINI_SURPLUS} card indices)` };
      }
      const indices = args.slice(0, TAROCCHINI_SURPLUS).map((a) => Number.parseInt(a, 10));
      if (indices.some(Number.isNaN)) return { error: 'Usage: scarto <i0> <i1> (numeric indices)' };
...
/** Help text shown in the CLI terminal for Tarocchini. */
export const TAROCCHINI_HELP: string[] = [
