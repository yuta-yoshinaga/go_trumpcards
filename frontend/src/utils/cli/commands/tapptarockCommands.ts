...
/** Parse a TappTarock CLI command into API exec arguments. */
export function parseTappTarockCommand(input: string): CliParseResult<TappTarockArgs> {
  ...
  switch (cmd) {
    ...
    case 'bid': {
      ...
      if (arg !== 'dreier' && arg !== 'solo') return { error: 'Usage: bid dreier|solo' };
...
/** Help text for TappTarock (タップ・タロック) CLI mode. */
export const TAPPTAROCK_HELP: string[] = [
  'bid dreier|solo                  - Declare a contract (Trischaken only arises when everyone passes)',
