import type { seahaventowersApi } from '../../../api/gameApi';
import { parseIntArg, splitCommand, suggestCommand } from '../commandParserBase';
import type { CliParseResult } from '../types';

type SeahavenTowersArgs = Parameters<typeof seahaventowersApi.exec>;
...
/** Parse a Seahaven Towers CLI command into API exec arguments. */
export function parseSeahavenTowersCommand(input: string): CliParseResult<SeahavenTowersArgs> {
...
function parseMoveCommand(args: string[]): CliParseResult<SeahavenTowersArgs> {
  ...
  if (fromZone.startsWith('t')) {
    ...
    if (to.startsWith('c')) {
      ...
      return { args: ['move', { zone: 'tableau', col }, { zone: 'reserved', cell }] };
    }
    if (to.startsWith('t')) {
    ...
    return { error: 'Invalid target: use f (foundation), t<col> (tableau), or c<cell> (reserved cell)' };
  }
  ...
  if (fromZone.startsWith('c')) {
    ...
    const to = args[1].toLowerCase();
    if (to === 'f') return { args: ['move', { zone: 'reserved', cell }, { zone: 'foundation' }] };
    if (to.startsWith('t')) {
      const toCol = Number(to.slice(1));
      if (Number.isNaN(toCol)) return { error: 'Usage: m c<cell> t<col>' };
      return { args: ['move', { zone: 'reserved', cell }, { zone: 'tableau', col: toCol }] };
    }
    return { error: 'Invalid target: use f (foundation) or t<col> (tableau)' };
  }

  return { error: 'Invalid source: use t<col> (tableau) or c<cell> (reserved cell)' };
}

/** Help text for Seahaven Towers CLI mode. */
export const SEAHAVENTOWERS_HELP: string[] = [
  'm t<c> t<c> - Move tableau top to column',
  'm t<c> <i> t<c> - Move card at index to column',
  'm t<c> f    - Move to foundation',
  'm t<c> c<n> - Move to reserved cell',
  'm c<n> t<c> - Move from reserved cell to tableau',
  'm c<n> f    - Move from reserved cell to foundation',
  'ac          - Auto-complete',
