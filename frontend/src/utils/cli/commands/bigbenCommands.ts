...
type BigBenArgs = Parameters<typeof bigBenApi.exec>;
...
/** Parse a Big Ben CLI command into API exec arguments. */
export function parseBigBenCommand(input: string): CliParseResult<BigBenArgs> {
...
function parseMoveCommand(args: string[]): CliParseResult<BigBenArgs> {
  if (args.length < 2) {
...
/** Help text for Big Ben CLI mode. */
export const BIGBEN_HELP: string[] = [
  'd/deal          - Deal from the stock (tops every column up to three)',
