...
/** Args tuple accepted by bouillotteApi.exec. */
export type BouillotteCliArgs = Parameters<typeof bouillotteApi.exec>;
...
/**
 * Parses a single CLI command line for the Bouillotte game into
 * {@link bouillotteApi}.exec arguments.
 *
 * On the human's Betting turn: `call` (or `c`) matches the current bet,
 * `raise` (`ra` / `vie`) increases it by a fixed increment, and `fold` (`f`)
 * drops out of the round. `next` deals the next round. `sp <3-4>`, `sa <n>`,
 ...
 */
export function parseBouillotteCommand(input: string): CliParseResult<BouillotteCliArgs> {
  ...
  switch (cmd) {
    ...
    case 'setplayers': {
      ...
      if (Number.isNaN(n) || n < 3 || n > 4) return { error: 'Usage: sp <3-4> (player count)' };
...
/** Help text shown in the CLI terminal for Bouillotte. */
export const BOUILLOTTE_HELP: string[] = [
  'c / call            - Call (match the current bet)',
  'ra / raise / vie    - Raise (vie) by a fixed increment',
  'f / fold            - Fold (drop out of the round)',
  'n / next            - Next round',
  'sp <3-4>            - Set player count (resets game)',
