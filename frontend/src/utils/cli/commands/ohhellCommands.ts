...
type OhHellArgs = Parameters<typeof ohHellApi.exec>;

const EXTRA_COMMANDS = ['bid'];

/** Parse an Oh Hell CLI command into API exec arguments. */
export function parseOhhellCommand(input: string): CliParseResult<OhHellArgs> {
  ...
  return { args: [result.command as OhHellArgs[0], undefined, result.cardIndex] };
}

/** Help text for Oh Hell CLI mode. */
export const OHHELL_HELP: string[] = ['bid <n>     - Place bid', ...TRICK_HELP];
