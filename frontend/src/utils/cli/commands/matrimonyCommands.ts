...
/** Parse a Matrimony CLI command into API exec arguments. */
export function parseMatrimonyCommand(input: string): CliParseResult<MatrimonyArgs> {
...
function parseMoveCommand(args: string[]): CliParseResult<MatrimonyArgs> {
  if (args.length < 2) {
    return { error: 'Usage: m <from> <to> (e.g., m t0 f, m w f, m w t3, m s t3)' };
  }
  const fromTok = args[0].toLowerCase();
  const toTok = args[1].toLowerCase();

  let from: MatrimonyMoveZone;
  if (fromTok === 'w') {
    from = { zone: 'waste' };
  } else if (fromTok === 's') {
    from = { zone: 'stock' };
    ...
    return { error: 'Invalid source: use t<slot> (tableau), w (waste) or s (stock)' };
  }
  ...
  if (toTok.startsWith('t')) {
    ...
    // Tableau cards have nowhere to go but a foundation.
    if (from.zone === 'tableau') {
      return { error: 'Invalid move: a tableau card can only go to a foundation' };
    }
...
/** Help text for Matrimony CLI mode. */
