...
/**
 * Matador rank within Ombre's trump hierarchy:
...
/**
 * Identifies whether a card is one of Ombre's three matadors and returns its
 * rank, or `null` when it is an ordinary card or trump is not yet decided.
 *
 * Mirrors the Go domain's `ombreCardStrength` (internal/domain/Ombre.go)
 * exactly:
...
/** i18n key (under the `ombre` namespace) for each matador rank's display name. */
