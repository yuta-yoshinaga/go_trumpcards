...
/**
 * Returns a frontend {@link HintResult} for Rams, or null when no suggestion is
 * available.
 *
 ...
 */
export function getRamsHint(state: RamsResponse): HintResult | null {
  ...
  if (hint.cardIndex === undefined) {
    return {
      targetAction: hint.reason === 'ramsPlayIn' ? 'play-in' : 'pass-out',
  ...
  return {
    ...
    reason: `hint.${hint.reason}`,
    confidence: hint.reason === 'ramsTakeTrick' ? 'strong' : 'moderate',
  };
}
