...
export function getSpiderHint(state: SpiderResponse): HintResult | null {
  if (state.phase !== SpiderPhase.PLAYING) return null;
...
function hasNearCompleteSequence(tableau: SpiderTableauCard[][]): boolean {
...
function hasSameSuitBuildMove(tableau: SpiderTableauCard[][]): boolean {
  for (const col of tableau) {
...
function hasFaceDownToReveal(tableau: SpiderTableauCard[][]): boolean {
  for (const col of tableau) {
