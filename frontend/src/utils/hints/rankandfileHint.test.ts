...
import { getRankAndFileHint } from './rankandfileHint';

const state = (hint?: RankAndFileResponse['hint']): RankAndFileResponse => ({ hint }) as RankAndFileResponse;

describe('getRankAndFileHint', () => {
  it('returns null without a hint', () => {
    expect(getRankAndFileHint(state())).toBeNull();
  });

  it('names the destination column', () => {
    const s = state({ fromZone: 'tableau', fromCol: 1, cardIndex: 0, toZone: 'tableau', toCol: 5 });
    expect(getRankAndFileHint(s)).toEqual({
      targetAction: 'tableau-5',
      reason: 'frontendHint.rankandfileMove',
      confidence: 'moderate',
    });
  });
  ...
  it('keeps a move onto column zero', () => {
    ...
    expect(getRankAndFileHint(s)?.targetAction).toBe('tableau-0');
  });
  ...
  it('names a column-less destination by zone alone', () => {
    ...
    expect(getRankAndFileHint(s)?.targetAction).toBe('foundation');
  });
  ...
  it('tells the player to draw when the server says stock', () => {
    ...
    expect(getRankAndFileHint(s)).toEqual({
      targetAction: 'draw',
      reason: 'frontendHint.rankandfileDraw',
      confidence: 'moderate',
    });
  });
});
