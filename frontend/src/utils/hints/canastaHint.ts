...
/**
 * Returns a Canasta hint suggesting the right phase action for the human turn.
 ...
 */
export function getCanastaHint(state: CanastaResponse): HintResult | null {
  ...
  if (state.phase === CanastaPhase.DRAW) {
  ...
  if (state.phase === CanastaPhase.MELD) {
  ...
  if (state.phase === CanastaPhase.DISCARD) {
