...
describe('getMarriageHint', () => {
  ...
  it('returns null for round-end phase', () => {
    const state = makeState({ phase: MarriagePhase.ROUND_END, players: [player({ cards: [card('HEART', 5)] })] });
    expect(getMarriageHint(state)).toBeNull();
  });
  ...
  describe('draw phase', () => {
    it('suggests draw from discard when the top forms a set', () => {
      ...
      const hint = getMarriageHint(state);
    ...
    it('suggests draw from discard when the top extends a run', () => {
      ...
      expect(getMarriageHint(state)?.targetAction).toBe('drawDiscard');
    });
    ...
    it('suggests draw from stock when there is no discard top', () => {
      ...
      expect(getMarriageHint(state)?.targetAction).toBe('drawStock');
    });
