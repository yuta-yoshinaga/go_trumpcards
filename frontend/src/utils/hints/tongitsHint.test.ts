...
function base({ hand = [card('SPADE', 13), card('HEART', 12)], ...overrides }: Partial<TongitsResponse> & Extra = {}) {
  return {
    players: [
...
describe('getTongitsHint', () => {
  ...
  it('takes the discard when it completes a set', () => {
    ...
    // 3 枚目が来て初めてデッドウッドが減る。
    const state = base({
      phase: TongitsPhase.DRAW,
      hand: [card('SPADE', 5), card('HEART', 5), card('CLOVER', 13)],
      discardTop: card('DIAMOND', 5),
    });
    expect(getTongitsHint(state)?.targetAction).toBe('takeDiscard');
  });
  ...
  it('does not take a card that only makes a pair', () => {
    ...
    expect(getTongitsHint(state)?.targetAction).toBe('drawStock');
  });
  ...
  it('offers the knock only once the discarded hand is within the threshold', () => {
    ...
    expect(getTongitsHint(at)?.targetAction).toBe('knock');
    ...
    expect(getTongitsHint(edge)?.targetAction).toBe('knock');

    // 捨てたあとが 6 点。1 点超えただけでサーバは拒否する。
    const over = base({
      hand: [card('SPADE', 3), card('HEART', 3), card('DIAMOND', 3), card('CLOVER', 6), card('SPADE', 9)],
    });
    expect(getTongitsHint(over)?.targetAction).not.toBe('knock');
  });
  ...
  it('does not treat a pair as a meld when deciding to knock', () => {
    ...
    expect(getTongitsHint(state)?.targetAction).not.toBe('knock');
  });
  ...
  it('discards the card that leaves the least deadwood', () => {
    ...
    expect(getTongitsHint(state)?.targetAction).toBe('card-3');
  });
  ...
  it('scores an ace as a single point', () => {
    ...
    expect(getTongitsHint(state)?.targetAction).toBe('knock');
  });

  it('returns null when the human has no cards', () => {
    expect(getTongitsHint(base({ hand: [] }))).toBeNull();
  });
});
