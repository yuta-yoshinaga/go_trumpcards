...
import { getSpiderHint } from './spiderHint';
...
function tc(design: Card['design'], value: number, faceUp = true): SpiderTableauCard {
...
describe('getSpiderHint', () => {
  it('returns null when game is cleared', () => {
    expect(getSpiderHint(makeState({ phase: SpiderPhase.GAME_CLEAR }))).toBeNull();
  });

  it('returns null when game is over', () => {
    expect(getSpiderHint(makeState({ phase: SpiderPhase.GAME_OVER }))).toBeNull();
  });

  it('suggests completing suit for near-complete sequence', () => {
    // Build a 10-card same-suit sequence
    const longSeq: SpiderTableauCard[] = [];
    for (let v = 13; v >= 4; v--) {
      longSeq.push(tc(S, v));
    }
  ...
  it('returns null when no moves and no stock', () => {
    ...
    expect(getSpiderHint(state)).toBeNull();
  });
});
