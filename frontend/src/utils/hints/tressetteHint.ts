...
/**
 * Tressette trick strength (higher beats lower within the led suit).
...
/** Returns a frontend HintResult for Tressette, or null if no suggestion. */
export function getTressetteHint(state: TressetteResponse): HintResult | null {
  if (state.phase !== TressettePhase.PLAY) return null;
