...
import { getWillOTheWispHint } from './willothewispHint';
...
function tc(design: Card['design'], value: number, faceUp = true): WillOTheWispTableauCard {
  return { card: { design, value }, faceUp };
}

function makeState(overrides: Partial<WillOTheWispResponse> = {}): WillOTheWispResponse {
  return {
    tableau: [[tc(S, 10), tc(S, 9), tc(S, 8)], [tc(H, 5), tc(H, 7), tc(H, 6)], [], [], [], [], []],
    stockCount: 20,
    completedSuits: 0,
    phase: WillOTheWispPhase.PLAYING,
    moveCount: 0,
    canUndo: false,
    isStalemate: false,
    score: 500,
...
describe('getWillOTheWispHint', () => {
  it('returns null for null/undefined state', () => {
    expect(getWillOTheWispHint(null)).toBeNull();
    expect(getWillOTheWispHint(undefined)).toBeNull();
  });

  it('returns null when game is cleared', () => {
    expect(getWillOTheWispHint(makeState({ phase: WillOTheWispPhase.GAME_CLEAR }))).toBeNull();
  });

  it('returns null when game is over', () => {
    expect(getWillOTheWispHint(makeState({ phase: WillOTheWispPhase.GAME_OVER }))).toBeNull();
  });

  it('suggests completing suit for near-complete sequence', () => {
    // Build a 10-card same-suit descending sequence (K→4).
    const longSeq: WillOTheWispTableauCard[] = [];
    for (let v = 13; v >= 4; v--) {
      longSeq.push(tc(S, v));
    }
    ...
    const hint = getWillOTheWispHint(state);
  ...
  it('suggests using an empty column after build moves are unavailable', () => {
    const state = makeState({
      tableau: [[tc(S, 5), tc(H, 4)], [tc(D, 10)], [], [], [], [], []],
      stockCount: 0,
    });
    const hint = getWillOTheWispHint(state);
  ...
  it('returns null when no moves and no stock', () => {
    ...
    expect(getWillOTheWispHint(state)).toBeNull();
  });
});
