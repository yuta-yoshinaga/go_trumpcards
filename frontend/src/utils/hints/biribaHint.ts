...
/**
 * Returns a Biriba hint suggesting the right phase action for the human turn.
...
export function getBiribaHint(state: BiribaResponse): HintResult | null {
  ...
  if (state.phase === BiribaPhase.DRAW) {
  ...
  if (state.phase === BiribaPhase.MELD) {
  ...
  if (state.phase === BiribaPhase.DISCARD) {
