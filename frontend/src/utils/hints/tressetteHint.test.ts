...
import type { Card, TressetteResponse } from '../../types/card';
import { TressettePhase } from '../../types/phases';
import { getTressetteHint } from './tressetteHint';

const card = (design: Card['design'], value: number): Card => ({ design, value });

function makeState(overrides: Partial<TressetteResponse> = {}): TressetteResponse {
  return {
    players: [
      {
        id: 0,
...
describe('getTressetteHint', () => {
  it('returns null when not the play phase', () => {
    expect(getTressetteHint(makeState({ phase: TressettePhase.TRICK_END }))).toBeNull();
  });

  it('returns null when it is not the human turn', () => {
    expect(getTressetteHint(makeState({ currentPlayerIdx: 1 }))).toBeNull();
  });

  it('returns null when the human has no cards', () => {
    const s = makeState();
    s.players[0].cards = [];
    expect(getTressetteHint(s)).toBeNull();
  });

  it('suggests leading low when leading', () => {
    const hint = getTressetteHint(makeState());
    expect(hint?.reason).toBe('hint.leadLow');
  });

  it('suggests discarding low when void in led suit', () => {
    const hint = getTressetteHint(makeState({ currentTrick: [{ playerIdx: 1, card: card('HEART', 7) }] }));
  ...
  it('suggests giving the partner when the partner is winning', () => {
    ...
    const hint = getTressetteHint(makeState({ currentTrick: [{ playerIdx: 2, card: card('SPADE', 5) }] }));
  ...
  it('suggests winning when an opponent leads and the human can beat it', () => {
    ...
    const hint = getTressetteHint(makeState({ currentTrick: [{ playerIdx: 1, card: card('SPADE', 5) }] }));
  ...
  it('suggests ducking when an opponent leads and the human cannot win', () => {
    ...
    const hint = getTressetteHint(makeState({ currentTrick: [{ playerIdx: 1, card: card('SPADE', 3) }] }));
