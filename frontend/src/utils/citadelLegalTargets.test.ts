...
import type { CitadelTableauCard } from '../types/games/citadel';
import { citadelLegalTargets } from './citadelLegalTargets';

const card = (design: CardDesign, value: number): Card => ({ design, value });
const tc = (design: CardDesign, value: number): CitadelTableauCard => ({
  card: card(design, value),
  faceUp: true,
});

describe('citadelLegalTargets', () => {
  it('reports nothing without a selected card', () => {
    const targets = citadelLegalTargets([[tc('SPADE', 5)]], [[]], null);
  ...
  it('accepts any suit onto a tableau card one rank higher', () => {
    const targets = citadelLegalTargets(
  ...
  it('offers an empty column to any card', () => {
    const targets = citadelLegalTargets([[], [tc('SPADE', 9)]], [[], [], [], []], card('HEART', 5));
  ...
  it('requires the same suit going up on a foundation', () => {
    ...
    const targets = citadelLegalTargets([[tc('CLOVER', 9)]], foundation, card('SPADE', 5));
    expect([...targets.foundation]).toEqual([0]);
  });

  it('starts an empty foundation only with an ace', () => {
    expect(citadelLegalTargets([], [[]], card('SPADE', 1)).foundation.has(0)).toBe(true);
    expect(citadelLegalTargets([], [[]], card('SPADE', 2)).foundation.has(0)).toBe(false);
  });

  it('offers a tableau column and a foundation at once when both accept', () => {
    const targets = citadelLegalTargets([[tc('CLOVER', 6)]], [[card('SPADE', 4)], [], [], []], card('SPADE', 5));
    expect(targets.tableau.has(0)).toBe(true);
