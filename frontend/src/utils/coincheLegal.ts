...
/**
 * Converts a card's suit design string into the 1-4 suit number the Coinche
 * domain uses (`internal/domain/Coinche.go`): SPADE=1, CLOVER=2, HEART=3,
...
/**
 * Trump-suit strength ordering. Mirrors `coincheTrumpRank` in the Go domain:
 * J(11) > 9 > A(1) > 10 > K(13) > Q(12) > 8 > 7.
 */
function coincheTrumpRank(value: number): number {
  switch (value) {
    case 11:
      return 8;
    case 9:
...
/**
 * Non-trump strength ordering. Mirrors `coincheNonTrumpRank` in the Go domain:
 * A(1) > 10 > K(13) > Q(12) > J(11) > 9 > 8 > 7.
 */
function coincheNonTrumpRank(value: number): number {
  switch (value) {
    case 1:
      return 8;
    case 10:
...
function cardRank(card: Card, trumpSuit: number): number {
  if (suitToNum(card.design) === trumpSuit) {
    return 200 + coincheTrumpRank(card.value);
  }
  return 100 + coincheNonTrumpRank(card.value);
}
...
function highestTrumpInTrick(trick: CoincheTrickCard[], trumpSuit: number): number {
  let best = 0;
  for (const tc of trick) {
    if (suitToNum(tc.card.design) !== trumpSuit) continue;
    const r = coincheTrumpRank(tc.card.value);
    if (r > best) best = r;
  }
  return best;
}
...
function handCanBeatTrump(hand: Card[], trumpSuit: number, rank: number): boolean {
  return hand.some((c) => suitToNum(c.design) === trumpSuit && coincheTrumpRank(c.value) > rank);
}

/** Whether the trick already contains a trump card. Mirrors `trickContainsTrump`. */
function trickContainsTrump(trick: CoincheTrickCard[], trumpSuit: number): boolean {
...
function currentLeader(trick: CoincheTrickCard[], trumpSuit: number): number {
 ...
 * (`internal/domain/Coinche.go`): follow-suit obligation, trump obligation
 ...
 */
export function isCoincheLegalPlay(
  card: Card,
  hand: Card[],
  trick: CoincheTrickCard[],
  trumpSuit: number,
  playerIdx: number,
): boolean {
  ...
  if (leadSuit === trumpSuit) {
    ...
    if (hasLead) {
      ...
      if (canOverTrump && coincheTrumpRank(card.value) <= highest) return false;
      return true;
    }
 ...
 * @param trumpSuit - The trump suit number (1-4), or 0 for no trump.
 * @param playerIdx - The seat index of the player (used for the partner check).
 * @returns The subset of hand indices that {@link isCoincheLegalPlay} accepts.
 */
export function coincheLegalPlayIndices(
  hand: Card[],
  trick: CoincheTrickCard[],
  trumpSuit: number,
  playerIdx: number,
  ...
  hand.forEach((card, idx) => {
    if (isCoincheLegalPlay(card, hand, trick, trumpSuit, playerIdx)) indices.push(idx);
  });
  return indices;
}
