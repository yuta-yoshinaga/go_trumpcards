...
import { CitadelPage } from './CitadelPage';

vi.mock('../api/gameApi', () => ({
  citadelApi: { exec: vi.fn() },
  actionLogApi: { citadel: vi.fn() },
}));
...
const mockExec = vi.mocked(citadelApi.exec);

function makeTableau(cols: CitadelTableauCard[][]): CitadelTableauCard[][] {
  const result: CitadelTableauCard[][] = [];
  for (let i = 0; i < 8; i++) {
    result.push(cols[i] ?? []);
  }
  return result;
...
const playingState: CitadelResponse = {
  tableau: makeTableau([
    [
...
const gameClearState: CitadelResponse = {
  ...playingState,
  phase: 1,
  message: 'ゲームクリア！',
  messageCode: 'citadel.gameClear',
  messageParams: { moveCount: '42' },
};

const gameOverState: CitadelResponse = {
  ...playingState,
  phase: 2,
  message: 'ゲームオーバー',
  messageCode: 'citadel.gameOver',
};

describe('CitadelPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('enables and pulses the auto-complete button once a foundation builds past its ace', async () => {
    const readyState: CitadelResponse = {
      ...playingState,
  ...
  it('shows StalemateEscapeButton when stalemate flag is set', async () => {
    const stalemate: CitadelResponse = {
      ...playingState,
      isStalemate: true,
      undoToEscape: 2,
      canUndo: true,
  ...
  describe('legal target highlighting', () => {
    ...
    it('marks a foundation only for the card that continues it', async () => {
      mockExec.mockResolvedValue(playingState);
      const { unmount } = renderWithProviders(<CitadelPage />);
      fireEvent.click(await screen.findByRole('button', { name: '♠ 5' }));
      await waitFor(() => expect(markedColumns()).toContain('#1'));
...
// test. See issue #4429.
describe('CitadelPage keyboard shortcuts', () => {
  it.each([
    ['h', 'hint'],
    ['a', 'autocomplete'],
    ['z', 'undo'],
...
// 選ぶ前に行き先が見える (#4454)。
describe('CitadelPage destination preview', () => {
  const render = async () => {
    mockExec.mockResolvedValue(playingState);
    renderWithProviders(<CitadelPage />);
...
describe('CitadelPage selection status announcement', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(useGameHint).mockReturnValue({ hint: null, hintEnabled: false, setHintEnabled: vi.fn() });
    localStorage.clear();
  ...
  const mixedBoardState: CitadelResponse = {
    ...playingState,
    tableau: makeTableau([
