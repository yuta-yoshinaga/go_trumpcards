...
import type { BakersDozenMoveZone } from '../api/gameApi';
...
function BakersDozenPageContent() {
  ...
  const bdCliConfig = useMemo(
    () => ({
      gameName: 'bakersdozen' as const,
      parseCommand: (_cmd: string) => ({ error: 'CLI not supported' }) as const,
      formatResponse: (_res: BakersDozenResponse): string => '',
      helpText: [] as string[],
    }),
    [],
  );
  const { handleCommand } = useCliGame(exec, bdCliConfig, state, { addInput, addOutput, addError, clearLog });
