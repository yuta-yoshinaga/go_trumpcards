...
import { BigBenPage } from './BigBenPage';

vi.mock('../api/gameApi', () => ({
  bigBenApi: { exec: vi.fn() },
  actionLogApi: { bigben: vi.fn() },
}));
...
const mockExec = vi.mocked(bigBenApi.exec);

const card = (design: CardDesign, value: number): Card => ({ design, value });

function makeTableau(cols: BigBenTableauCard[][]): BigBenTableauCard[][] {
...
const playingState: BigBenResponse = {
  tableau: makeTableau([
    [
...
const gameClearState: BigBenResponse = {
  ...playingState,
  phase: 1,
  message: 'ゲームクリア！',
  messageCode: 'bigben.gameClear',
  messageParams: { moveCount: '42' },
};

const gameOverState: BigBenResponse = {
  ...playingState,
  phase: 2,
  message: 'ゲームオーバー',
  messageCode: 'bigben.gameOver',
};
...
describe('BigBenPage', () => {
  ...
  it('renders heading and move count', async () => {
    mockExec.mockResolvedValue(playingState);
    renderWithProviders(<BigBenPage />);
    await waitFor(() => expect(screen.getByText(/ビッグ・ベン/)).toBeInTheDocument());
  ...
  // **9 時始まり。**クローン元は 1 時始まりで、添字 0 が A を求めた。時刻の
  // 並びを直さないと、画面の時刻と目標ランクが全部ずれる。
  it('labels face 0 as nine o’clock and face 11 as eight', async () => {
    mockExec.mockResolvedValue(playingState);
    renderWithProviders(<BigBenPage />);
    await waitFor(() => expect(screen.getByLabelText(/文字盤0 \(9時\)/)).toBeInTheDocument());
    expect(screen.getByLabelText(/文字盤11 \(8時\)/)).toBeInTheDocument();
    // 負のコントロール: 1 時始まりの並びが残っていないこと。
    expect(screen.queryByLabelText(/文字盤0 \(1時\)/)).not.toBeInTheDocument();
  });
  ...
  it('marks a completed face aria-disabled but keeps it focusable', async () => {
    ...
    const face = screen.getByLabelText(/文字盤0 \(9時\)/);
  ...
  it('does not move a card onto a completed face', async () => {
    ...
    fireEvent.click(screen.getByLabelText(/文字盤0 \(9時\)/));
  ...
  it('renders giveup button when playing and hides it once cleared', async () => {
    mockExec.mockResolvedValue(playingState);
    const { unmount } = renderWithProviders(<BigBenPage />);
...
describe('BigBenPage keyboard shortcuts', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(useGameHint).mockReturnValue({ hint: null, hintEnabled: false, setHintEnabled: vi.fn() });
  });
  ...
  // **補充がこのゲームの逃げ道。**クローン元のグランドファーザーズ・クロックは
  // 52 枚を配り切るので山札を持たず、このボタンが存在しなかった。
  it('deals from the stock and shows how many are left', async () => {
    mockExec.mockResolvedValue(playingState);
    renderWithProviders(<BigBenPage />);
    const deal = await screen.findByTestId('bigben-deal-button');
    expect(deal).toHaveTextContent('52');
