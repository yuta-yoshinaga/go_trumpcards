...
import { useWhiteheadGame } from '../hooks/useWhiteheadGame';
...
import { parseWhiteheadCommand, WHITEHEAD_HELP } from '../utils/cli/commands/whiteheadCommands';
import { formatWhiteheadState } from '../utils/cli/formatters/whiteheadFormatter';
...
/** Renders the Whitehead solitaire game page with tableau, stock/waste, and foundation. */
export const WhiteheadPage = withTutorial(WhiteheadPageContent, 'whitehead', KL_TUTORIAL_STEPS);
/** Inner content of the Whitehead page, wrapped by TutorialProvider. */
...
function WhiteheadPageContent() {
  ...
  // CLI mode
  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('whitehead');
  const whiteheadCliConfig: CliGameConfig<WhiteheadResponse, Parameters<typeof whiteheadApi.exec>> = useMemo(
    () => ({
      gameName: 'whitehead',
      parseCommand: parseWhiteheadCommand,
      formatResponse: formatWhiteheadState,
      helpText: WHITEHEAD_HELP,
    }),
    [],
  );
  const { handleCommand } = useCliGame(exec, whiteheadCliConfig, state, { addInput, addOutput, addError, clearLog });

  const {
    hint: frontendHint,
    hintEnabled: frontendHintEnabled,
    setHintEnabled: setFrontendHintEnabled,
  } = useGameHint('whitehead', state);
  const { cardHeight, cardOverlap, cardWidth, isMobile } = useCardDimensions();
  const windowWidth = useWindowWidth();

  // Responsive card dimensions for Whitehead's 7-column layout on mobile
  const kl = useMemo(() => {
  ...
  const isPlayingForKbd = state?.phase === WhiteheadPhase.PLAYING;
  const { elapsedSeconds, resetTimer, timeBonus } = useWhiteheadTimer(isPlayingForKbd);
  ...
  const { getStat, recordResult } = useWhiteheadStats();
  ...
  useEffect(() => {
    const ended = currentPhase === WhiteheadPhase.GAME_CLEAR || currentPhase === WhiteheadPhase.GAME_OVER;
    if (!ended) {
    ...
    const won = currentPhase === WhiteheadPhase.GAME_CLEAR;
  ...
  const dispatchMove = useCallback(
    (source: WhiteheadMoveZone, target: WhiteheadMoveZone) => {
      void exec('move', source, target);
    },
    [exec],
  );
  const dnd = useSolitaireDragDrop<WhiteheadMoveZone>({
    onMove: dispatchMove,
  ...
  if (!state) return <GameSkeleton gameKey="whitehead" layout={{ kind: 'tableau', topRow: 6, tableau: 7 }} />;

  const isPlaying = state.phase === WhiteheadPhase.PLAYING;
  ...
  // Mirrors the backend's Whitehead.AllFaceUp() guard: stock must be empty AND every tableau card face-up.
  ...
  return (
    <GamePageShell
      title={tc('nav.whitehead')}
      gameThemeBg={gameTheme.whitehead.bg}
    ...
    >
      {cliEnabled ? (
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto pt-3 px-2 sm:px-4 lg:px-8">
            ...
            <div className="flex gap-1 sm:gap-2 mb-3" data-tutorial="kl-tableau">
              {state.tableau.map((col, colIdx) => {
                const tableauColZone: WhiteheadMoveZone = { zone: 'tableau', col: colIdx };
                return (
                  <div
                    ...
                    >
                      <div className="relative" style={{ minHeight: kl.ch }}>
                        {col.length === 0 ? (
                          ...
                          col.map((tc, cardIdx) => {
                            const cardZone: WhiteheadMoveZone = {
                              zone: 'tableau',
                              col: colIdx,
                              cardIndex: cardIdx,
                            };
            ...
            {/*
              ...
              られないことがある (#5955)。
            */}
            <div data-tutorial="kl-hint-display" data-testid="kl-hint-live" role="status" aria-live="polite">
              {hint &&
                (() => {
                  const fromCard =
                    hint.fromCol >= 0
          ...
          {/* Footer */}
          <GameFooter className={`${gameTheme.whitehead.footer} px-4 py-2.5`}>
            ...
            <div className="flex gap-2 items-center flex-wrap">
              ...
              {/* **Vegas を選んだプレイヤーは計算式を知る手段が無かった。** ヘッダーには
                  ...
                  数字は WhiteheadVegas から取る -- domain 側の定数と突き合わせる Go の
              ...
              <div data-testid="kl-stats-panel" className="w-full text-game-text-muted text-xs">
                {t('stats.winRate', { rate: whiteheadWinRate(currentStat) })} ({currentStat.wins}/{currentStat.plays})
            ...
            <ActionShortcutsPanel bindings={actionBindings} data-testid="whitehead-kbd-shortcuts" />
          </GameFooter>
        </>
      )}
    </GamePageShell>
