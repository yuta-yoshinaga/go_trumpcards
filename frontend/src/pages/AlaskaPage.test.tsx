...
vi.mock('../api/gameApi', () => ({
  ...
  actionLogApi: { alaska: vi.fn() },
}));

const mockExec = vi.mocked(alaskaHintApi.exec);

const card = (design: CardDesign, value: number): Card => ({ design, value });

const playingState: AlaskaResponse = {
  tableau: [
  ...
  isStalemate: false,
  message: '',
  messageCode: 'alaska.playing',
};

const gameClearState: AlaskaResponse = {
  ...playingState,
  phase: 1,
  moveCount: 42,
  messageCode: 'alaska.gameClear',
  messageParams: { moveCount: '42' },
};

const gameOverState: AlaskaResponse = {
  ...playingState,
  phase: 2,
  messageCode: 'alaska.gameOver',
};
...
describe('AlaskaPage', () => {
  ...
  it('dismisses the face-down rule note, persists it, and keeps it hidden on remount', async () => {
    ...
    expect(screen.queryByTestId('alaska-facedown-rule')).not.toBeInTheDocument();
    expect(localStorage.getItem('alaska-rules-dismissed')).toBe('true');
  ...
  it('embeds the hint in card aria-labels instead of a text panel', async () => {
    mockExec.mockResolvedValue({
      ...
      messageCode: 'alaska.hintAvailable',
    });
    renderWithProviders(<AlaskaPage />);
    await waitFor(() => expect(mockExec).toHaveBeenCalledWith('reset'));
...
describe('AlaskaPage block move announcement', () => {
  const blockState = {
    ...playingState,
    tableau: [
      [
