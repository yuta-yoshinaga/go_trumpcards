...
const mockExec = vi.mocked(julepeApi.exec);
...
const playing = (over: Partial<JulepeResponse> = {}) =>
  makeState({
    phase: 1,
...
describe('JulepePage', () => {
  ...
  it('shows the hint when one is enabled', async () => {
    vi.mocked(useGameHint).mockReturnValue({
      hint: { targetAction: 'play-in', reason: 'hint.julepePlayIn', confidence: 'moderate' },
      hintEnabled: true,
...
describe('JulepePage dealer badge', () => {
  beforeEach(() => {
    localStorage.clear();
    vi.clearAllMocks();
  });
