...
function AlaskaPageContent() {
  ...
  const {
    ...
    exec: apiExec,
    retry,
  } = useGameApi<AlaskaResponse, Parameters<typeof alaskaHintApi.exec>>((...args) => alaskaHintApi.exec(...args));
  ...
  const [rulesDismissed, setRulesDismissed] = useLocalStorageToggle('alaska-rules-dismissed', false);

  const {
    hint: frontendHint,
    hintEnabled: frontendHintEnabled,
    setHintEnabled: setFrontendHintEnabled,
  } = useGameHint('alaska', state);

  // CLI mode
  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('alaska');
  const alaskaHintCliConfig: CliGameConfig<AlaskaResponse, Parameters<typeof alaskaHintApi.exec>> = useMemo(
    () => ({
      gameName: 'alaska',
      parseCommand: parseAlaskaCommand,
      formatResponse: formatAlaskaState,
      helpText: RS_HELP,
    }),
    [],
  );
  const { handleCommand } = useCliGame(apiExec, alaskaHintCliConfig, state, {
    addInput,
    addOutput,
    addError,
    clearLog,
  ...
  const dispatchMove = useCallback(
    (source: AlaskaMoveZone, target: AlaskaMoveZone) => {
      void apiExec('move', source, target);
    },
    [apiExec],
  );
  const dnd = useSolitaireDragDrop<AlaskaMoveZone>({
    onMove: dispatchMove,
    isPlaying: state?.phase === AlaskaPhase.PLAYING,
    disabled: loading,
  });
  ...
  const handleHint = useCallback(async () => {
    const res = await alaskaHintApi.exec('hint');
  ...
  const isPlayingForKbd = state?.phase === AlaskaPhase.PLAYING;
  ...
  if (!state) return <GameSkeleton gameKey="alaska" layout={{ kind: 'tableau', topRow: 6, tableau: 7 }} />;

  const isPlaying = state.phase === AlaskaPhase.PLAYING;
  ...
  return (
    <GamePageShell
      title={tc('nav.alaska')}
      gameThemeBg={gameTheme.alaska.bg}
          ...
          <div className="flex-1 overflow-y-auto pt-3 px-2 sm:px-4 lg:px-8">
            ...
            {!rulesDismissed && (
              <div
                ...
                data-testid="alaska-facedown-rule"
                role="note"
              >
                  ...
                  {col.length === 0 ? (
                    ...
                    <div className="relative" style={{ width: rs.cw, height: rs.ch + (col.length - 1) * rs.co }}>
                      {col.map((tc, cardIdx) => {
                        ...
                        const zone: AlaskaMoveZone = { zone: 'tableau', col: colIdx, cardIndex: cardIdx };
