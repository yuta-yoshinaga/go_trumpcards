...
import { actionLogApi, courchevelApi } from '../api/gameApi';
...
vi.mock('../api/gameApi', () => ({
  courchevelApi: { exec: vi.fn() },
  actionLogApi: { courchevel: vi.fn() },
}));
...
const mockExec = vi.mocked(courchevelApi.exec);
...
describe('CourchevelPage', () => {
  ...
  // ---- Courchevel: the one card that is already face up ----

  // **プリフロップで場が 1 枚見えている。** これがこのゲームの全部で、
  // ページはサーバが送ってきた場をそのまま描くだけ ── 枚数をページ側で
  // 決め打つと、Big O に戻したときに気づけない。
  it('renders the single exposed board card during pre-flop', async () => {
    mockExec.mockResolvedValue({
      ...preFlopState,
      // 誰の手札とも重ならない札にする (同じ alt が 2 つあると引けない)。
      communityCards: [{ design: 'DIAMOND', value: 3 }],
    } as OmahaResponse);
    renderWithProviders(<CourchevelPage />);
    await waitFor(() => expect(screen.getByAltText('♦ 3')).toBeInTheDocument());
    // 場は 1 枚として描かれる (フロップの残り 2 枚はまだ出ていない)。
  ...
  it('labels the used hole cards (2) and the unused ones at showdown', async () => {
    ...
    expect(screen.getAllByTestId('courchevel-hole-used').length).toBe(2);
    expect(screen.getAllByTestId('courchevel-hole-unused').length).toBeGreaterThan(0);
  });
  ...
  it('omits used/unused labels when the human has folded', async () => {
    ...
    renderWithProviders(<CourchevelPage />);
    await waitFor(() => expect(screen.getByText('ツーペア')).toBeInTheDocument());
    expect(screen.queryByTestId('courchevel-hole-used')).not.toBeInTheDocument();
  });
  ...
  it('keeps the 5 hole cards on a single scrollable row on mobile (no wrap)', async () => {
    ...
    try {
      ...
      renderWithProviders(<CourchevelPage />);
      await waitFor(() => expect(screen.getByAltText('♠ A')).toBeInTheDocument());
      const hole = screen.getByTestId('courchevel-hole-cards');
  ...
  it('previews the current best hand during play', async () => {
    ...
    mockExec.mockResolvedValue(flopState);
    renderWithProviders(<CourchevelPage />);
    const badge = await screen.findByTestId('courchevel-live-besthand-name');
