...
import { useRankAndFileGame } from '../hooks/useRankAndFileGame';
 ...
 *
 * **空の山にスートを書かない。**`RankAndFile.MoveTableauToFoundation` は宛先
...
/** Rank and File tutorial step definitions. */
const FT_TUTORIAL_STEPS: TutorialStep[] = [
  {
    target: '[data-tutorial="rf-stock-waste"]',
    messageKey: 'tutorial.stockWaste',
    placement: 'bottom',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="rf-foundation"]',
    messageKey: 'tutorial.foundation',
    placement: 'bottom',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="rf-tableau"]',
    messageKey: 'tutorial.tableau',
    placement: 'top',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="rf-controls"]',
...
/** Renders the Rank and File solitaire game page with tableau, stock/waste, and foundation. */
export const RankAndFilePage = withTutorial(RankAndFilePageContent, 'rankandfile', FT_TUTORIAL_STEPS);
...
function RankAndFilePageContent() {
  ...
  // CLI mode
  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('rankandfile');
  const ftCliConfig = useMemo(
    () => ({
      gameName: 'rankandfile' as const,
      parseCommand: parseRankandfileCommand,
      formatResponse: formatRankandfileState,
      helpText: RANKANDFILE_HELP,
    }),
    [],
  );
  ...
  } = useGameHint('rankandfile', state);
  // Live longest-column length: shrinks the per-card vertical step on mobile so the tallest
  // tableau column fits within 375×667 without scrolling (#1861). Rank and File columns start
  // at 4 cards but accumulate quickly as alternating-colour descending sequences are built.
  const maxColCards = useMemo(
    () => state?.tableau.reduce((m, col) => (col.length > m ? col.length : m), 0) ?? 0,
    [state?.tableau],
  );
  // This page uses `px-2 sm:px-4 lg:px-8` (padX=16 on mobile) and `gap-1 sm:gap-2`
  // (gapPx=4 on mobile), which matches the hook's defaults — no overrides needed.
  const rf = useResponsiveTableau(10, { maxColCards });

  const isPlayingForKbd = state?.phase === RankAndFilePhase.PLAYING;

  const dispatchMove = useCallback(
    (source: RankAndFileMoveZone, target: RankAndFileMoveZone) => {
      void exec('move', source, target);
    },
    [exec],
  );
  const dnd = useSolitaireDragDrop<RankAndFileMoveZone>({
    onMove: dispatchMove,
  ...
  const isPlaying = state.phase === RankAndFilePhase.PLAYING;
  const isGameClear = state.phase === RankAndFilePhase.GAME_CLEAR;
  const isGameOver = state.phase === RankAndFilePhase.GAME_OVER;
  ...
  return (
    <GamePageShell
      title={tc('nav.rankandfile')}
      gameThemeBg={gameTheme.rankandfile.bg}
      phaseName={isGameClear ? t('phase.gameClear') : isGameOver ? t('phase.gameOver') : t('phase.playing')}
      gamePath="/rankandfile"
      gameEndFlag={isEnded}
      winShow={isGameClear}
      loading={loading}
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto pt-3 px-2 sm:px-4 lg:px-8">
            ...
            <div className="flex gap-1 sm:gap-2 mb-3 items-start flex-wrap">
              {/* Stock + Waste */}
              <div className="flex gap-1 sm:gap-2" data-tutorial="rf-stock-waste">
                ...
                <div className="text-center">
                  ...
                  {wasteDisplay.length > 0 ? (
                    ...
                    >
                      <AnimatedCard card={wasteDisplay[0]} width={rf.cw} draggable={false} />
                    </button>
                  ) : (
                    <div
                      style={{ width: rf.cw, height: rf.ch }}
              ...
              <div className="flex gap-1 sm:gap-2 flex-wrap" data-tutorial="rf-foundation">
                {state.foundation.map((pile, idx) => {
                  const foundationZone: RankAndFileMoveZone = { zone: 'foundation', col: idx };
                  return (
                    <div
                      key={`f-${idx.toString()}`}
            ...
            {/* Tableau */}
            <div className="flex gap-1 sm:gap-2 mb-3" data-tutorial="rf-tableau">
              {state.tableau.map((col, colIdx) => {
                const tableauColZone: RankAndFileMoveZone = { zone: 'tableau', col: colIdx };
                return (
                  <div
                  ...
                  >
                    ...
                    >
                      <div className="relative" style={{ minHeight: rf.ch }}>
                        {col.length === 0 ? (
                          <button
                            type="button"
                          ...
                          col.map((tc, cardIdx) => {
                            const cardZone: RankAndFileMoveZone = {
                              zone: 'tableau',
                              col: colIdx,
                              cardIndex: cardIdx,
                            };
                            ...
                            return (
                              ...
                              >
                                {tc.faceUp && tc.card ? (
                                  ...
                                  <AnimatedCardBack width={rf.cw} className="w-full" />
                                )}
                              </div>
                            );
                          })
                        )}
                        {col.length > 0 && <div style={{ height: (col.length - 1) * rf.co + rf.ch }} />}
                      </div>
                    </DropZone>
                  </div>
                );
            ...
            {/* Hint display */}
            <div data-tutorial="rf-hint-display" data-testid="rf-hint-display">
              {hint && (
                <div className="text-ds-warning text-sm mb-2">
                  ...
                  {hint.fromZone === 'stock' ? (
                    <>
                      {t('hintAvailable')}: {t('frontendHint.rankandfileDraw')}
                    </>
                  ) : (
                    <>
              ...
              <div className="sr-only" role="status" aria-live="polite" data-testid="rf-hint-announcement">
                {hint
                  ? hint.fromZone === 'stock'
                    ? t('frontendHint.rankandfileDraw')
          ...
          {/* Footer */}
          <GameFooter className={`${gameTheme.rankandfile.footer} px-4 py-2.5`}>
            <ErrorAlert message={error ?? hintError} onRetry={retry} />
            <div className="flex gap-2 items-center flex-wrap">
              {isPlaying && (
                <div data-tutorial="rf-controls">
                  <button
                    type="button"
