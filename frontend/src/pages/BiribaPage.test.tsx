...
vi.mock('../api/gameApi', () => ({
  ...
  actionLogApi: { biriba: vi.fn() },
}));
const mockExec = vi.mocked(biribaApi.exec);
...
const basePlayers: BiribaPlayerData[] = [
  {
    id: 0,
    isHuman: true,
    cardCount: 15,
...
const drawPhaseState: BiribaResponse = {
  players: basePlayers,
  phase: 0,
  roundNumber: 1,
  currentPlayerIdx: 0,
  ...
  winnerIdx: -1,
  message: '',
  messageCode: 'biriba.drawPhase',
  config: { cpuDifficulty: 1, pointLimit: 5000 },
};

const meldPhaseState: BiribaResponse = {
  ...drawPhaseState,
  phase: 1,
  messageCode: 'biriba.meldPhase',
};

const discardPhaseState: BiribaResponse = {
  ...drawPhaseState,
  phase: 2,
  messageCode: 'biriba.discardPhase',
};

const roundEndState: BiribaResponse = {
  ...drawPhaseState,
  phase: 3,
  messageCode: 'biriba.roundEnd',
};

const gameEndState: BiribaResponse = {
  ...drawPhaseState,
  phase: 4,
  gameEndFlag: true,
  winnerIdx: 0,
...
describe('BiribaPage', () => {
  ...
  it("executes Biriba's reset command from the page mount effect", async () => {
    renderWithProviders(<BiribaPage />);
    await waitFor(() => expect(mockExec).toHaveBeenCalled());
    expect(mockExec.mock.calls.some(([command]) => command === 'reset')).toBe(true);
  });
  ...
  it('shows HintTooltip when hint is enabled in draw phase', async () => {
    localStorage.setItem('hint_enabled_biriba', 'true');
  ...
  it('rings nothing while the hint is off', async () => {
    localStorage.setItem('hint_enabled_biriba', 'false');
    mockExec.mockResolvedValue({
      ...meldPhaseState,
      hint: { action: 'meld', reason: 'meldInitial', indices: [0, 2] },
    });
  ...
  it('shows the disabled reason for draw-from-discard when no cards are selected', async () => {
    ...
    expect(reason).toHaveTextContent('手札から捨て札のトップと一致するナチュラルカード2枚を選択してください');
  });
  ...
  it('persists the chosen sort mode to localStorage', async () => {
    ...
    expect(localStorage.getItem('biriba-sort-mode')).toBe('rank');
  });
  ...
  it('refuses a two-card selection the shared Canasta rules reject', async () => {
    localStorage.clear();
    mockExec.mockReset();
    // Biriba still uses two natural cards matching the discard top to take the pile.
    mockExec.mockResolvedValue(drawPhaseState);
    renderWithProviders(<BiribaPage />);
    await waitFor(() => expect(screen.getByRole('button', { name: '捨て札を取る' })).toBeInTheDocument());
    const handCards = screen.getAllByTestId(/^bu-hand-card-/);
    ...
    expect(screen.getByTestId('ca-draw-discard-reason')).toHaveTextContent('トップと一致');
  });
