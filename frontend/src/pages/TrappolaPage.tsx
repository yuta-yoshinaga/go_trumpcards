...
import type { trappolaApi } from '../api/gameApi';
...
import { CPU_DIFFICULTY_OPTIONS, TARGET_POINTS_OPTIONS, useTrappolaGame } from '../hooks/useTrappolaGame';
...
import { parseTrappolaCommand, TRAPPOLA_HELP } from '../utils/cli/commands/trappolaCommands';
import { formatTrappolaState } from '../utils/cli/formatters/trappolaFormatter';
import type { CliGameConfig } from '../utils/cli/types';
import { playerName } from '../utils/playerUtils';
import { hintCheckboxItem } from '../utils/settingsItems';

/** Trappola tutorial step definitions. */
...
const TRAPPOLA_PHASE_KEYS: Readonly<Record<number, string>> = {
  [TrappolaPhase.PLAY]: 'play',
  [TrappolaPhase.TRICK_END]: 'trickEnd',
  [TrappolaPhase.ROUND_END]: 'roundEnd',
  [TrappolaPhase.GAME_END]: 'gameEnd',
};
...
/** Renders the Trappola game page: no-trump must-follow trick play with team scoring. */
export const TrappolaPage = withTutorial(TrappolaPageContent, 'trappola', TR_TUTORIAL_STEPS);

/** Inner content of the Trappola page, wrapped by TutorialProvider. */
function TrappolaPageContent() {
  const { t, tc, actionLog, showActionLog, hideActionLog, confirmOpen, requestConfirm, confirmReset, cancelReset } =
    useGamePageSetup('trappola');
  const {
    state,
    loading,
    error,
  ...
  // CLI mode
  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('trappola');
  const trappolaCliConfig: CliGameConfig<TrappolaResponse, Parameters<typeof trappolaApi.exec>> = useMemo(
    () => ({
      gameName: 'trappola',
      parseCommand: parseTrappolaCommand,
      formatResponse: formatTrappolaState,
      helpText: TRAPPOLA_HELP,
    }),
    [],
  );
  const { handleCommand } = useCliGame(exec, trappolaCliConfig, state, { addInput, addOutput, addError, clearLog });

  const {
    hint: frontendHint,
    hintEnabled: frontendHintEnabled,
    setHintEnabled: setFrontendHintEnabled,
  } = useGameHint('trappola', state);
  const { cardWidth, isMobile } = useCardDimensions();

  const isPlayPhaseForKbd = state?.phase === TrappolaPhase.PLAY;
  ...
  const phaseNames = usePhaseNames('trappola', TRAPPOLA_PHASE_KEYS);

  const handleManualReset = useCallback(() => {
    hideActionLog();
    void exec('reset', undefined, undefined, {
      cpuDifficulty: trappolaConfig.cpuDifficulty,
      targetPoints: trappolaConfig.targetPoints,
    });
  }, [exec, hideActionLog, trappolaConfig.cpuDifficulty, trappolaConfig.targetPoints]);

  if (!state)
    return <GameSkeleton gameKey="trappola" layout={{ kind: 'trick-taking', trickArea: true, footerHandSize: 5 }} />;

  const humanPlayer = state.players.find((p) => p.isHuman);
  const isPlayPhase = state.phase === TrappolaPhase.PLAY;
  const isTrickEnd = state.phase === TrappolaPhase.TRICK_END;
  const isRoundEnd = state.phase === TrappolaPhase.ROUND_END;
  const isGameEnd = state.phase === TrappolaPhase.GAME_END || state.gameEndFlag;
  ...
  return (
    <GamePageShell
      title={tc('nav.trappola')}
      gameThemeBg={gameTheme.trappola.bg}
    ...
    >
      {cliEnabled ? (
        ...
        <>
                ...
                items: [
                  {
                    ...
                    value: trappolaConfig.cpuDifficulty,
                  ...
                  },
                  {
                    ...
                    value: trappolaConfig.targetPoints,
          ...
          {/* Footer */}
          <GameFooter className={`${gameTheme.trappola.footer} px-4 py-2.5`}>
            {humanPlayer && (
              <PlayerHandSection
            ...
            {/*
              ...
              られないことがある (#5955)。
            */}
            <div data-testid="trappola-hint-live" role="status" aria-live="polite">
              {hint && (
            ...
            <CardNavShortcutsPanel data-testid="trappola-kbd-shortcuts" />
          </GameFooter>
        </>
      )}
    </GamePageShell>
