...
import type { BoliviaResponse, Card } from '../types/card';
...
/**
 * Cards required to complete a canasta (7-card set) or bolivia (7-card sequence).
 * Matches the Go domain rule (`BoliviaMeld.IsCanasta`/`IsBolivia`: `len(Cards) >= 7`).
 */
const BOLIVIA_CANASTA_SIZE = 7;

const BOLIVIA_PHASE_KEYS: Readonly<Record<number, string>> = {
  [BoliviaPhase.DRAW]: 'draw',
  [BoliviaPhase.MELD]: 'meld',
  [BoliviaPhase.DISCARD]: 'discard',
  [BoliviaPhase.ROUND_END]: 'roundEnd',
  [BoliviaPhase.GAME_END]: 'gameEnd',
};

/** Bolivia tutorial step definitions. */
...
function BoliviaPageContent() {
  ...
  const cliConfig: CliGameConfig<BoliviaResponse, Parameters<typeof boliviaApi.exec>> = useMemo(
    () => ({
      gameName: 'bolivia',
      parseCommand: parseBoliviaCommand,
      formatResponse: formatBoliviaState,
      helpText: BOLIVIA_HELP,
  ...
  const isDrawPhase = state?.phase === BoliviaPhase.DRAW;
  const isMeldPhase = state?.phase === BoliviaPhase.MELD;
  const isDiscardPhase = state?.phase === BoliviaPhase.DISCARD;
  const isRoundEnd = state?.phase === BoliviaPhase.ROUND_END;
  ...
  const handleManualReset = useCallback(() => {
    ...
    void gameExec('reset', undefined, {
      cpuDifficulty: boliviaConfig.cpuDifficulty,
      pointLimit: boliviaConfig.pointLimit,
    });
  }, [gameExec, hideActionLog, boliviaConfig.cpuDifficulty, boliviaConfig.pointLimit]);
  const isHumanTurn =
  ...
  return (
    <GamePageShell
      title={tc('nav.bolivia')}
      gameThemeBg={gameTheme.bolivia.bg}
                ...
                items: [
                  {
                    ...
                    value: boliviaConfig.cpuDifficulty,
                  ...
                  },
                  {
                    ...
                    value: boliviaConfig.pointLimit,
