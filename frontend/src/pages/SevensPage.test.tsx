...
const humanTurnState: SevensResponse = {
  players: [
    { id: 0, isHuman: true, isFinished: false, rank: -1, cardCount: 3, passesUsed: 0, maxPasses: 5,
      cards: [
...
const passesExhaustedState: SevensResponse = {
  ...humanTurnState,
  players: [
    { ...humanTurnState.players[0], passesUsed: 5, maxPasses: 5 },
...
describe('SevensPage', () => {
  ...
  it('shows human action log after play', async () => {
    ...
    render(<SevensPage />)
    await waitFor(() =>
      expect(screen.getByText(/あなたが出しました/)).toBeInTheDocument()
    )
  })
  ...
  it('shows pass action log for human pass', async () => {
    ...
    render(<SevensPage />)
    await waitFor(() =>
      expect(screen.getByText(/あなたがパスしました/)).toBeInTheDocument()
    )
  })
