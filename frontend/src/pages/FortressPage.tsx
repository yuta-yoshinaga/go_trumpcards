...
import type { FortressMoveZone, fortressApi } from '../api/gameApi';
...
import { useFortressGame } from '../hooks/useFortressGame';
...
import { FORTRESS_HELP, parseFortressCommand } from '../utils/cli/commands/fortressCommands';
import { formatFortressState } from '../utils/cli/formatters/fortressFormatter';
import type { CliGameConfig } from '../utils/cli/types';
import { fortressLegalTargets } from '../utils/fortressLegalTargets';
...
const BC_TUTORIAL_STEPS: TutorialStep[] = [
  {
    target: '[data-tutorial="fortress-foundation"]',
    messageKey: 'tutorial.foundation',
    placement: 'bottom',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="fortress-tableau"]',
  ...
  {
    target: '[data-tutorial="fortress-controls"]',
...
/** Renders the Fortress solitaire game page with 10 tableau columns and 4 foundations. */
export const FortressPage = withTutorial(FortressPageContent, 'fortress', BC_TUTORIAL_STEPS);
...
function FortressPageContent() {
  ...
  } = useGameHint('fortress', state);
  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('fortress');
  const cliConfig: CliGameConfig<FortressResponse, Parameters<typeof fortressApi.exec>> = useMemo(
    () => ({
      gameName: 'fortress',
      parseCommand: parseFortressCommand,
      formatResponse: formatFortressState,
      helpText: FORTRESS_HELP,
    }),
    [],
  );
  ...
  const dispatchMove = useCallback(
    (source: FortressMoveZone, target: FortressMoveZone) => {
      void game.exec('move', source, target);
    },
    [game],
  );
  const dnd = useSolitaireDragDrop<FortressMoveZone>({
    onMove: dispatchMove,
  ...
  const preview = useDestinationPreview<FortressMoveZone>(selectedSource);

  if (!state) return <GameSkeleton gameKey="fortress" layout={{ kind: 'tableau', topRow: 4, tableau: 10 }} />;

  const isPlaying = state.phase === FortressPhase.PLAYING;
  const isGameClear = state.phase === FortressPhase.GAME_CLEAR;
  const isGameOver = state.phase === FortressPhase.GAME_OVER;
  ...
  const previewedCard =
    previewSource?.zone === 'tableau' && previewSource.col !== undefined && previewSource.cardIndex !== undefined
      ...
      : undefined;
  const legalTargets = fortressLegalTargets(state.tableau, state.foundation, previewedCard);
  ...
  const renderTableauColumn = (colIdx: number) => {
    ...
    const tableauColZone: FortressMoveZone = { zone: 'tableau', col: colIdx };
    return (
      <div
        ...
        >
          <div className="relative" style={{ minHeight: dims.ch }}>
            {col.length === 0 ? (
              ...
              col.map((tc, cardIdx) => {
                const cardZone: FortressMoveZone = {
                  zone: 'tableau',
                  col: colIdx,
                  cardIndex: cardIdx,
                };
  ...
  return (
    <GamePageShell
      title={tc('nav.fortress')}
      gameThemeBg={gameTheme.fortress.bg}
      ...
      {cliEnabled ? (
        ...
        <>
          <div className="flex-1 overflow-y-auto pt-3 px-2 sm:px-4 lg:px-8">
            ...
            */}
            <div
              data-tutorial="fortress-hint-display"
              data-testid="fortress-hint-live"
              role="status"
              aria-live="polite"
            >
              {hint && (
            ...
            {isGameOver && (
              <p data-testid="fortress-gameover-summary" className="text-ds-text-muted text-sm text-center mt-1">
                {t('gameOverSummary', {
          ...
          <GameFooter className={`${gameTheme.fortress.footer} px-4 py-2.5`}>
            <ErrorAlert message={error ?? hintError} onRetry={retry} />
            <div className="flex gap-2 items-center flex-wrap">
              {isPlaying && (
                <div data-tutorial="fortress-controls">
                  <button
                    type="button"
              ...
              <GameResetButton
                ...
                loading={loading}
                dataTutorial="fortress-reset-button"
              />
            </div>
            <ActionShortcutsPanel bindings={actionBindings} data-testid="fortress-kbd-shortcuts" />
          </GameFooter>
        </>
      )}
    </GamePageShell>
