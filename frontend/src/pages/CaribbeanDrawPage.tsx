...
import { outcomeFromResult, useCaribbeanDrawStats } from '../hooks/useCaribbeanDrawStats';
...
import { btnDanger, btnPrimary, btnSecondary, btnSuccess, btnWarning, focusRingAccent } from '../styles/buttonStyles';
...
/** Caribbean Draw Poker tutorial step definitions. */
const CD_TUTORIAL_STEPS: TutorialStep[] = [
  {
    target: '[data-tutorial="cd-bet-controls"]',
    messageKey: 'tutorial.betControls',
    placement: 'top',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="cd-draw-controls"]',
    messageKey: 'tutorial.drawControls',
    placement: 'top',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="cd-action-buttons"]',
    messageKey: 'tutorial.actionButtons',
    placement: 'top',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="cd-results"]',
...
/** Renders the Caribbean Draw Poker game page with betting, draw, action, and result display. */
export const CaribbeanDrawPage = withTutorial(CaribbeanDrawPageContent, 'caribbeandraw', CD_TUTORIAL_STEPS);
/** Inner content of the Caribbean Draw Poker page, wrapped by TutorialProvider. */
function CaribbeanDrawPageContent() {
  const { t, tc, actionLog, showActionLog, hideActionLog, confirmOpen, requestConfirm, confirmReset, cancelReset } =
    useGamePageSetup('caribbeandraw');

  const [anteAmount, setAnteAmount] = useState(100);
  const [jackpotAmount, setJackpotAmount] = useState(0);
  // Positions (0-based, as the API wants them) the player has picked to exchange.
  const [selectedIndices, setSelectedIndices] = useState<number[]>([]);

  const { cardWidth } = useCardDimensions();
  const { state, loading, error, exec: execApi, retry } = useGameApi(caribbeandrawApi.exec);
  const {
    hint: frontendHint,
    hintEnabled: frontendHintEnabled,
    setHintEnabled: setFrontendHintEnabled,
  } = useGameHint('caribbeandraw', state);
  // CLI mode
  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('caribbeandraw');
  const cliConfig: CliGameConfig<CaribbeanDrawResponse, Parameters<typeof caribbeandrawApi.exec>> = useMemo(
    () => ({
      gameName: 'caribbeandraw',
      parseCommand: parseCaribbeandrawCommand,
      formatResponse: formatCaribbeandrawState,
      helpText: CARIBBEANDRAW_HELP,
    }),
    [],
  );
  ...
  const isBetPhase = state?.phase === CaribbeanDrawPhase.BET;
  ...
  // Drop the picks whenever the draw phase ends, so a stale selection cannot be
  // replayed against the next hand's cards.
  const currentPhase = state?.phase;
  useEffect(() => {
    if (currentPhase !== CaribbeanDrawPhase.DRAW) setSelectedIndices([]);
  }, [currentPhase]);

  const toggleCard = useCallback((index: number) => {
    setSelectedIndices((prev) => {
      if (prev.includes(index)) return prev.filter((i) => i !== index);
      // The cap is the rule, not a nicety: the backend rejects a third index
      // outright, which would cost the player a round-trip to be told no.
      if (prev.length >= MAX_EXCHANGE) return prev;
      return [...prev, index];
    });
  }, []);
  ...
  const { tally, recordRound, clearHistory } = useCaribbeanDrawStats();
  ...
  const actionBindings = useMemo(
    () => [
      { key: 'b', action: () => execApi('bet', anteAmount, jackpotAmount), enabled: isBetPhase, label: 'bet' },
      {
        ...
        enabled: isDrawPhase && selectedIndices.length > 0,
        label: 'exchange',
      },
      {
        key: 's',
        action: () => execApi('draw', undefined, undefined, []),
        enabled: isDrawPhase,
        label: 'standPat',
      },
  ...
  const handleDraw = () => {
    execApi('draw', undefined, undefined, selectedIndices);
  };

  const handleStandPat = () => {
    execApi('draw', undefined, undefined, []);
  };
  ...
  return (
    <GamePageShell
      title={tc('nav.caribbeandraw')}
      gameThemeBg={gameTheme.caribbeandraw.bg}
      phaseName={phaseName}
      gamePath="/caribbeandraw"
      isHumanTurn={isBetPhase || isDrawPhase || isActionPhase}
      ...
      {cliEnabled ? (
        ...
        <>
          ...
          >
            ...
            {tally.hands > 0 && (
              ...
              >
                ...
                <div className="flex items-center justify-center gap-3 text-ds-text-muted">
                  <span data-testid="cd-session-tally">
                    {t('session.tally', {
                      wins: tally.wins,
                      losses: tally.losses,
                      pushes: tally.pushes,
            ...
            {isDrawPhase && (
              <div className="mx-auto mb-3 max-w-sm text-center text-sm" data-testid="cd-draw-guide">
                <p className="text-ds-text-primary">{t('draw.guide', { max: MAX_EXCHANGE })}</p>
                {/* The fee is charged on confirm, so it belongs here rather than
                    in the payout breakdown the player only reaches afterwards. */}
                <p className="text-ds-warning" data-testid="cd-draw-fee">
                  {t('draw.fee', { cost: drawFee })}
                </p>
                <p className="text-ds-text-muted" data-testid="cd-draw-selected">
                  {t('draw.selected', { count: selectedIndices.length, max: MAX_EXCHANGE })}
                </p>
              </div>
            )}

            {state.playerHand.length > 0 && (
              <div className="mb-4" data-tutorial="cd-results">
                <div className="text-ds-warning font-bold text-center mb-1">
                  <span aria-hidden="true">🟡</span> {t('player')}
                  {/* The rank is known from the deal onwards, and during the draw
                      it is the whole basis for deciding what to throw away. */}
                  {!isBetPhase && HAND_RANK_KEYS[state.playerHandRank] && (
                ...
                <div className="flex justify-center gap-2 flex-wrap">
                  {state.playerHand.map((card, i) => {
                    ...
                    if (!isDrawPhase) {
                      return <AnimatedCard key={`p-${card.design}-${card.value}-${i}`} card={card} width={cardWidth} />;
                    }
                    return (
                      <button
                        key={`p-${card.design}-${card.value}-${i}`}
                        type="button"
                        data-testid={`cd-player-card-${i}`}
                        aria-label={`${cardAlt(card)}${isSelected ? ` ${t('draw.cardSelected')}` : ''}`}
            ...
            {isEndPhase && (
              <div className="text-ds-text-primary text-center text-sm mb-2" data-testid="payout-breakdown">
                ...
                {/* A cost, never folded into totalPayout — without its own line the
                    chip count drops by an amount the breakdown cannot account for. */}
                {state.drawCost > 0 && (
                  <div data-testid="cd-draw-cost">
                    {t('payout.draw')}: -{state.drawCost}
                  </div>
                )}
          ...
          <GameFooter className={`${gameTheme.caribbeandraw.footer} px-4 pt-3`}>
            ...
            />
            {isBetPhase && (
              <div className="flex flex-col items-center gap-2 pb-2" data-tutorial="cd-bet-controls">
                <ChipBetInput
                  id="caribbeandraw-ante-amount"
                  label={t('label.ante')}
                  value={anteAmount}
                ...
                />
                <ChipBetInput
                  id="caribbeandraw-jackpot-amount"
              ...
              <div className="flex flex-col items-center gap-2 pb-2" data-tutorial="cd-draw-controls">
                <div className="flex justify-center gap-2">
                  ...
                  <button
                    ...
                    disabled={loading}
                    data-testid="cd-stand-pat-button"
                  >
                    {t('button.standPat')}
                  </button>
                </div>
            ...
            {isActionPhase && (
              <div className="flex justify-center gap-2 pb-2" data-tutorial="cd-action-buttons">
            ...
            <ActionShortcutsPanel bindings={actionBindings} data-testid="caribbean-draw-kbd-shortcuts" />
          </GameFooter>
        </>
      )}
    </GamePageShell>
