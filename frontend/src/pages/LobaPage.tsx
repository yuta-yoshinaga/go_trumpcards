...
import { useLobaGame } from '../hooks/useLobaGame';
...
const LB_TUTORIAL_STEPS: TutorialStep[] = [
  ...
  { target: '[data-tutorial="lb-melds"]', messageKey: 'tutorial.deck', placement: 'bottom', advanceOn: 'next' },
  { target: '[data-tutorial="lb-seats"]', messageKey: 'tutorial.score', placement: 'bottom', advanceOn: 'next' },
];

/** Renders the Loba page: 108 cards, melds needing three suits, out at 101. */
export const LobaPage = withTutorial(LobaPageContent, 'loba', LB_TUTORIAL_STEPS);

function LobaPageContent() {
  const { t, tc, actionLog, showActionLog, hideActionLog, confirmOpen, requestConfirm, confirmReset, cancelReset } =
    useGamePageSetup('loba');
  const game = useLobaGame();
  ...
  const cliConfig: CliGameConfig<LobaResponse, Parameters<typeof lobaApi.exec>> = useMemo(
    () => ({
      gameName: 'loba',
      parseCommand: parseLobaCommand,
      formatResponse: formatLobaState,
      helpText: LOBA_HELP,
    }),
    [],
  );
  ...
  } = useGameHint('loba', state);

  if (!state) {
    return <GameSkeleton gameKey="loba" layout={{ kind: 'tableau', topRow: 3, tableau: 4 }} />;
  }

  const ended = state.phase === LobaPhase.GAME_END;
  const drawing = state.phase === LobaPhase.DRAW;
  const acting = state.phase === LobaPhase.ACT;
  const roundOver = state.phase === LobaPhase.ROUND_END;
      ...
      headerExtra={
        <>
          <span className="text-sm text-ds-text-muted">
            {t('round')}: {state.roundNo + 1} / {t('stock')}: {state.stockCount} /{' '}
            {t('knockOut', { n: state.knockOut })}
          </span>
      ...
      {cliEnabled ? (
        ...
        <>
          <div className="flex-1 overflow-y-auto pt-3 px-2 sm:px-4 lg:px-8">
            ...
            <div className="mb-4" data-tutorial="lb-melds">
            ...
            <div className="text-center" data-tutorial="lb-hand">
              <div className="text-game-text-muted text-xs mb-1">
                {t('yourHand')}
                {' · '}
          ...
          <GameFooter className={`${gameTheme.loba.footer} px-4 py-2.5`}>
