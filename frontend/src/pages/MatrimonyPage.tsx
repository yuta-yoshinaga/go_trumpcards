...
import { useMatrimonyGame } from '../hooks/useMatrimonyGame';
import { useSolitaireDragDrop } from '../hooks/useSolitaireDragDrop';
import { btnDanger, btnPrimary, btnSuccess, focusRingWhite } from '../styles/buttonStyles';
import { gameTheme } from '../styles/gameTheme';
import type { MatrimonyMoveZone, MatrimonyResponse } from '../types/games/matrimony';
import { MatrimonyPhase } from '../types/games/matrimony';
import type { TutorialStep } from '../types/tutorial';
import { cardAlt } from '../utils/cardAlt';
import { MATRIMONY_HELP, parseMatrimonyCommand } from '../utils/cli/commands/matrimonyCommands';
import { formatMatrimonyState } from '../utils/cli/formatters/matrimonyFormatter';
import type { CliGameConfig } from '../utils/cli/types';
import { hintCheckboxItem } from '../utils/settingsItems';

const FOUNDATION_LABELS = ['Q♠↓', 'Q♠↓', 'J♦↑', 'J♦↑'] as const;
...
/** Renders the Matrimony page: sixteen tableau slots and four foundations. */
export const MatrimonyPage = withTutorial(MatrimonyPageContent, 'matrimony', CG_TUTORIAL_STEPS);
...
function MatrimonyPageContent() {
  ...
  } = useGameHint('matrimony', state);
  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('matrimony');
  const cliConfig: CliGameConfig<MatrimonyResponse, Parameters<typeof matrimonyApi.exec>> = useMemo(
    () => ({
      gameName: 'matrimony',
      parseCommand: parseMatrimonyCommand,
      formatResponse: formatMatrimonyState,
      helpText: MATRIMONY_HELP,
    }),
    [],
  );
  ...
  const isPlayingForKbd = state?.phase === MatrimonyPhase.PLAYING;

  const dispatchMove = useCallback(
    (source: MatrimonyMoveZone, target: MatrimonyMoveZone) => {
      // Tableau cards only move to foundations; empty slots are filled from stock or waste.
      // のは山札か捨て札だけ。クリック経路はボタンの disabled で防いでいるが、
      // **ドラッグ経路はここを通る**ので、同じ規則をここでも見る (#4906)。
      if (target.zone === 'tableau' && source.zone === 'tableau') return;
      void game.exec('move', source, target);
    },
    [game],
  );
  const dnd = useSolitaireDragDrop<MatrimonyMoveZone>({
    onMove: dispatchMove,
  ...
  if (!state) {
    return <GameSkeleton gameKey="matrimony" layout={{ kind: 'tableau', topRow: 8, tableau: SLOTS_PER_ROW }} />;
  }

  const isPlaying = state.phase === MatrimonyPhase.PLAYING;
  const isGameClear = state.phase === MatrimonyPhase.GAME_CLEAR;
  const isGameOver = state.phase === MatrimonyPhase.GAME_OVER;
  ...
  const wasteZone: MatrimonyMoveZone = { zone: 'waste' };
  const stockZone: MatrimonyMoveZone = { zone: 'stock' };

  // タブローは 1 枠 1 枚。重ねられないので、山を描くのではなく枠を描く。
  const renderSlot = (slot: number) => {
    const card = state.tableau[slot] ?? null;
    const slotZone: MatrimonyMoveZone = { zone: 'tableau', col: slot };
    return (
  ...
  return (
    <GamePageShell
      title={tc('nav.matrimony')}
      gameThemeBg={gameTheme.matrimony.bg}
    ...
    >
      ...
      {cliEnabled ? (
        ...
        <>
            ...
            <div className="flex flex-wrap justify-center items-start gap-3 sm:gap-6 mb-3">
              <div className="flex flex-wrap justify-center gap-1 sm:gap-2" data-tutorial="cg-foundation">
                {state.foundation.map((pile, idx) => {
                  const foundationZone: MatrimonyMoveZone = { zone: 'foundation', col: idx };
                  return (
                    <div key={`f-${idx.toString()}`} className="text-center">
                      <div className="text-game-text-muted text-xs mb-1">{FOUNDATION_LABELS[idx]}</div>
                      <DropZone
          ...
          <GameFooter className={`${gameTheme.matrimony.footer} px-4 py-2.5`}>
            ...
            <ActionShortcutsPanel bindings={actionBindings} data-testid="matrimony-kbd-shortcuts" />
          </GameFooter>
        </>
      )}
    </GamePageShell>
