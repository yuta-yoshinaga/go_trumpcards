...
describe('TablanetPage', () => {
  ...
  it('announces the tablanet-prompt from the always-mounted live region', async () => {
    mockExec.mockResolvedValue(playPhaseState);
    renderWithProviders(<TablanetPage />);

    const live = await screen.findByTestId('tablanet-prompt-live');
    expect(live).toHaveAttribute('role', 'status');
    expect(live).toHaveAttribute('aria-live', 'polite');
    // 隣に置いただけの実装は属性の検査を通る。**中にあること**を見る。
    expect(live).toContainElement(await screen.findByTestId('tablanet-prompt'));
  });
});
