...
import type { SevensResponse, SevensPlayerData, SevensAction, Card, CardDesign } from '../types/card'

// Design → suit index (matches Go backend: 1=SPADE, 2=CLOVER, 3=HEART, 4=DIAMOND)
const designToSuit: Record<CardDesign, number> = {
  SPADE: 1, CLOVER: 2, HEART: 3, DIAMOND: 4, JOKER: 0,
}
  ...
  return (
    <div style={style}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
        {!player.isFinished && Array.from({ length: showCount }).map((_, i) => (
    ...
    <div style={style}>
      ...
      {!player.isFinished && (
        <div style={{ color: '#ccc', fontSize: '0.85em', marginBottom: 4 }}>
          {player.cardCount}枚　パス: {player.passesUsed}/{player.maxPasses}
          {isCurrentTurn && (
            <span style={{ marginLeft: 8, color: '#cfc' }}>出せるカードをクリック</span>
          )}
        </div>
      )}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
    ...
    <div style={tableStyle}>
      ...
      {state.humanAction && (
        <div style={{
          ...
          color: '#cfc', padding: '8px 14px', margin: '8px 0', fontSize: '0.85em',
        }}>
      ...
      {state.cpuActions && state.cpuActions.length > 0 && (
        <div style={{
          background: 'rgba(0,0,0,0.4)', borderRadius: 8,
          color: '#ccc', padding: '8px 14px', margin: '8px 0',
          whiteSpace: 'pre-line', fontSize: '0.85em',
        }}>
      ...
      {state.message && (
        <div style={{
          ...
          padding: '12px 20px', fontSize: '1.3em', fontWeight: 'bold', margin: '10px 0',
        }}>
          {state.message}
        </div>
      )}
