...
const SS_PHASE_KEYS: Readonly<Record<number, string>> = {
  [SimpleSimonPhase.PLAYING]: 'playing',
  [SimpleSimonPhase.GAME_CLEAR]: 'gameClear',
  [SimpleSimonPhase.GAME_OVER]: 'gameOver',
};
...
export const SimpleSimonPage = withTutorial(SimpleSimonPageContent, 'simplesimon', SS_TUTORIAL_STEPS);

/** Inner content of the Simple Simon page, wrapped by TutorialProvider. */
function SimpleSimonPageContent() {
  ...
  const isClear = state.phase === SimpleSimonPhase.GAME_CLEAR;
  const isEnd = isClear || state.phase === SimpleSimonPhase.GAME_OVER;
  const canAct = !isEnd;
  ...
  return (
    <GamePageShell
      title={tc('nav.simplesimon')}
      gameThemeBg={gameTheme.simplesimon.bg}
      phaseName={phaseName}
      gamePath="/simplesimon"
      gameEndFlag={isEnd}
      winShow={isClear}
      loading={loading}
