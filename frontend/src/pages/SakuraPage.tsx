...
function SakuraPageContent() {
  ...
  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('sakura');
  const cliConfig: CliGameConfig<NonNullable<typeof state>, Parameters<typeof sakuraApi.exec>> = useMemo(
    () => ({
      gameName: 'sakura',
      parseCommand: parseSakuraCommand,
      formatResponse: formatSakuraState,
      helpText: SAKURA_HELP,
    }),
    [],
  );
