...
const mockExec = vi.mocked(bigOApi.exec);
...
describe('BigOPage', () => {
  ...
  it('handles action log visibility and API fetch', async () => {
    ...
    vi.mocked(actionLogApi.bigo).mockResolvedValueOnce({ entries: [] });
    fireEvent.click(screen.getByText('棋譜を見る'));

    await waitFor(() => expect(actionLogApi.bigo).toHaveBeenCalledTimes(1));
