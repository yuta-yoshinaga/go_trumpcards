...
import { FollowTheQueenPage } from './FollowTheQueenPage';

vi.mock('../api/gameApi', () => ({
  followTheQueenApi: { exec: vi.fn() },
  actionLogApi: { followthequeen: vi.fn() },
}));
...
const cpuPlayer = (id: number, overrides: Partial<import('../types/card').FollowTheQueenPlayerData> = {}) => ({
  id,
  isHuman: false,
  holeCards: [],
  doorCards: [
...
const initState: FollowTheQueenResponse = {
  ...baseState,
  players: [],
  phase: 0,
  message: '',
...
const thirdStreetState: FollowTheQueenResponse = {
  ...baseState,
...
const thirdStreetWithBetState: FollowTheQueenResponse = {
  ...thirdStreetState,
  lastBet: 40,
...
/** SHOWDOWN (phase 6) */
const showdownState: FollowTheQueenResponse = {
  ...baseState,
  players: [
...
describe('FollowTheQueenPage', () => {
  ...
  it('disables buttons while loading', async () => {
    ...
    let resolve!: (value: FollowTheQueenResponse) => void;
    const slowPromise = new Promise<FollowTheQueenResponse>((res) => {
      resolve = res;
    });
  ...
  it('wraps settings in collapsible details element', async () => {
    mockExec.mockResolvedValue(initState);
    const { container } = renderWithProviders(<FollowTheQueenPage />);
...
const kbdCases: [string, unknown[], FollowTheQueenResponse][] = [
...
describe('FollowTheQueenPage keyboard shortcuts', () => {
...
describe('FollowTheQueenPage HUD stats', () => {
...
describe('FollowTheQueenPage wild rank', () => {
  // **ワイルドが読めなければ役が数えられない。** サーバが送る wildRank を
  // そのまま出しているか、未確定のときに «まだ無し» と言い切っているかを見る。
  const wildState = (wildRank: number): FollowTheQueenResponse => ({
    ...thirdStreetState,
    wildRank,
  });

  it('names the wild rank the server sent', async () => {
    mockExec.mockResolvedValue(wildState(7));
    renderWithProviders(<FollowTheQueenPage />);
    await waitFor(() => expect(screen.getByTestId('ftq-wild-rank')).toHaveTextContent('7'));
  });

  it('renders a face rank as its letter, not its number', async () => {
    mockExec.mockResolvedValue(wildState(12));
    renderWithProviders(<FollowTheQueenPage />);
    await waitFor(() => expect(screen.getByTestId('ftq-wild-rank')).toHaveTextContent('Q'));
    expect(screen.getByTestId('ftq-wild-rank')).not.toHaveTextContent('12');
  });

  it('says none while no queen has set one', async () => {
    mockExec.mockResolvedValue(wildState(0));
    renderWithProviders(<FollowTheQueenPage />);
