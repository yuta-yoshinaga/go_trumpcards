...
export const CourchevelHiLoPage = withTutorial(CourchevelHiLoPageContent, 'courchevelhilo', BIGOHL_TUTORIAL_STEPS);
/** Inner content of the Courchevel Hi-Lo page, wrapped by TutorialProvider. */
function CourchevelHiLoPageContent() {
  const {
    t,
    tc,
    actionLog,
  ...
  return (
    <GamePageShell
      title={tc('nav.courchevelhilo')}
      gameThemeBg={gameTheme.courchevelhilo.bg}
      phaseName={phaseNames[phase] ?? t('phase.init')}
      isHumanTurn={canAct}
      gamePath="/courchevelhilo"
    ...
    >
      {cliEnabled ? (
        ...
        <>
          ...
          <div className={`relative flex-1 overflow-y-auto pt-4 px-5 lg:px-8 ${lgCardAreaConstraint}`}>
                  ...
                  <div className="mb-1.5 flex flex-wrap items-center gap-2">
                    ...
                    {phase >= OmahaPhase.FLOP && phase <= OmahaPhase.RIVER && (
                      <BoardLowBadge
                        ...
                        t={t}
                        testId="courchevelhilo-board-low-badge"
                      />
                    )}
                  </div>
                      ...
                      ? state.communityCards.map((card, idx) => {
                          ...
                          return (
                            <div
                              ...
                              data-testid={inLo ? 'courchevelhilo-lo-card' : undefined}
                            >
            ...
            {isShowdown &&
              (() => {
                ...
                return (
                  <div className="mb-2 text-center text-sm" data-testid="courchevelhilo-split">
                    ...
                    {scoopers.length > 0 && (
                      <div className="mb-1.5 flex flex-wrap justify-center gap-2" role="status">
                        {scoopers.map((s) => (
                          <span
                            key={`scoop-${s.name}`}
                            data-testid="courchevelhilo-scoop-badge"
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                      {hiWinners.map((w) => (
                        <span
                          key={`hi-${w.name}`}
                          data-testid="courchevelhilo-hi-badge"
                      ...
                      {loWinners.map((w) => (
                        <span
                          key={`lo-${w.name}`}
                          data-testid="courchevelhilo-lo-badge"
                    ...
                    {loWinners.length === 0 && hiWinners.length > 0 && (
                      <div className="mt-1 text-xs text-ds-text-muted" data-testid="courchevelhilo-hi-takes-all">
                        {t('hiLo.hiTakesAll')}
                      </div>
                    )}
                  </div>
          ...
          <GameFooter className={`${gameTheme.courchevelhilo.footer} px-5 py-3`}>
            {/* Human player */}
            {humanPlayer && (
              <div className="mb-2" data-tutorial="bohl-player-hand">
                ...
                <div
                  ...
                  data-testid="courchevelhilo-rule-badge"
            ...
            {canAct && (
              <div data-tutorial="bohl-action-buttons">
                <BettingControls
                  inputId="courchevelhiloBetAmount"
                  betAmount={betAmount}
