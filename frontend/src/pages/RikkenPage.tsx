...
const RK_TUTORIAL_STEPS: TutorialStep[] = [
  { target: '[data-tutorial="rk-bid"]', messageKey: 'tutorial.bid', placement: 'top', advanceOn: 'next' },
  { target: '[data-tutorial="rk-hand"]', messageKey: 'tutorial.hand', placement: 'top', advanceOn: 'next' },
  { target: '[data-tutorial="rk-score"]', messageKey: 'tutorial.score', placement: 'bottom', advanceOn: 'next' },
];

/** The four trump choices. Suits are 1..4; no-trump is not chosen here. */
const TRUMP_CHOICES = [
...
/** Renders the Rikken game page (#5230). */
...
function RikkenPageContent() {
  ...
  const cliConfig: CliGameConfig<RikkenResponse, Parameters<typeof rikkenApi.exec>> = useMemo(
    () => ({
      gameName: 'rikken',
      parseCommand: parseRikkenCommand,
      formatResponse: formatRikkenState,
      helpText: RIKKEN_CLI_HELP,
    }),
    [],
  );
  ...
  const isBidPhase = phase === RikkenPhase.BID;
  const isCallPhase = phase === RikkenPhase.CALL;
  const isPlayPhase = phase === RikkenPhase.PLAY;
  const isRoundEnd = phase === RikkenPhase.ROUND_END;
  ...
  } = useGameHint('rikken', state);

  if (!state) {
    return <GameSkeleton gameKey="rikken" layout={{ kind: 'trick-taking', trickArea: true, footerHandSize: 13 }} />;
  }
  ...
  const contractLabel = t(RIKKEN_CONTRACTS.find((c) => c.contract === state.contract)?.key ?? 'contract.none');
  const trumpLabel =
    state.trumpSuit === RIKKEN_NO_TRUMP
      ? t('trump.none')
  ...
  return (
    <GamePageShell
      title={tc('nav.rikken')}
      gameThemeBg={gameTheme.rikken.bg}
      phaseName={phaseName}
      gamePath="/rikken"
        ...
        <>
          <div data-testid="card-area" className={`overflow-y-auto pt-3 px-4 lg:px-8 flex-1 ${lgCardAreaConstraint}`}>
            ...
            <div className="text-ds-text-primary text-center text-sm mb-2 space-y-1">
              <div data-testid="rikken-contract">
                {t('label.contract')}: {contractLabel}
                {state.contract !== RikkenContract.NONE && ` / ${t('label.trump')}: ${trumpLabel}`}
              </div>
              {state.declarerIdx >= 0 && (
                <div data-testid="rikken-declarer">
            ...
            {state.currentTrick.length > 0 && (
              <div className="mb-4" data-testid="rikken-trick">
            ...
            {human && human.cards.length > 0 && (
              <div data-tutorial="rk-hand">
                ...
                <div className="flex justify-center gap-1 flex-wrap" data-testid="rikken-hand">
          ...
          <GameFooter className={`${gameTheme.rikken.footer} px-4 pt-3`}>
