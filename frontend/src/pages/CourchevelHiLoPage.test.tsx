...
import { CourchevelHiLoPage } from './CourchevelHiLoPage';

vi.mock('../api/gameApi', () => ({
  courchevelHiLoApi: { exec: vi.fn() },
  actionLogApi: { courchevelhilo: vi.fn() },
}));
...
const mockExec = vi.mocked(courchevelHiLoApi.exec);
...
describe('CourchevelHiLoPage', () => {
  ...
  it('shows green Hi and blue Lo split badges (Lo omitted when none qualifies)', async () => {
    ...
    renderWithProviders(<CourchevelHiLoPage />);
    await waitFor(() => expect(screen.getByTestId('courchevelhilo-split')).toBeInTheDocument());
    expect(screen.getByTestId('courchevelhilo-hi-badge')).toHaveClass('text-ds-success');
    expect(screen.getByTestId('courchevelhilo-lo-badge')).toHaveClass('text-ds-info');
  });
  ...
  it('highlights the human qualifying low (blue) and lists the low card ranks', async () => {
    ...
    renderWithProviders(<CourchevelHiLoPage />);
    await waitFor(() => expect(screen.getByTestId('courchevelhilo-split')).toBeInTheDocument());
    expect(screen.getAllByTestId('courchevelhilo-lo-card').length).toBeGreaterThan(0);
    expect(screen.getByTestId('courchevelhilo-lo-badge')).toHaveTextContent('A 5 8 2 5');
  });
  ...
  it('states Hi scoops the pot when no low qualifies', async () => {
    ...
    renderWithProviders(<CourchevelHiLoPage />);
    await waitFor(() => expect(screen.getByTestId('courchevelhilo-hi-badge')).toBeInTheDocument());
    expect(screen.getByTestId('courchevelhilo-hi-takes-all')).toBeInTheDocument();
  });
  ...
  it('shows a scoop badge when one player wins both Hi and Lo', async () => {
    ...
    renderWithProviders(<CourchevelHiLoPage />);
    const badge = await screen.findByTestId('courchevelhilo-scoop-badge');
  ...
  it('does not show a scoop badge on a split win (different Hi and Lo winners)', async () => {
    ...
    renderWithProviders(<CourchevelHiLoPage />);
    await waitFor(() => expect(screen.getByTestId('courchevelhilo-split')).toBeInTheDocument());
    expect(screen.queryByTestId('courchevelhilo-scoop-badge')).not.toBeInTheDocument();
  });
  ...
  it('handles action log visibility and API fetch', async () => {
    ...
    vi.mocked(actionLogApi.courchevelhilo).mockResolvedValueOnce({ entries: [] });
    fireEvent.click(screen.getByText('棋譜を見る'));

    await waitFor(() => expect(actionLogApi.courchevelhilo).toHaveBeenCalledTimes(1));
  ...
  it('shows whether the board can still make a low', async () => {
    ...
    mockExec.mockResolvedValue(flopState);
    renderWithProviders(<CourchevelHiLoPage />);
    await waitFor(() => expect(screen.getByTestId('courchevelhilo-board-low-badge')).toBeInTheDocument());
  });
  ...
  it('withholds the board-low badge before the flop', async () => {
    ...
    expect(screen.queryByTestId('courchevelhilo-board-low-badge')).not.toBeInTheDocument();
  });
