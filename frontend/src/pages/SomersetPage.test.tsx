...
import { SomersetPage } from './SomersetPage';

vi.mock('../api/gameApi', () => ({
  somersetApi: { exec: vi.fn() },
  actionLogApi: { somerset: vi.fn() },
}));
...
const playingState: SomersetResponse = {
  tableau: makeTableau([
    [
...
const gameClearState: SomersetResponse = {
  ...playingState,
  phase: 1,
  message: 'ゲームクリア！',
  messageCode: 'somerset.gameClear',
  messageParams: { moveCount: '42' },
};

const gameOverState: SomersetResponse = {
  ...playingState,
  phase: 2,
  message: 'ゲームオーバー',
  messageCode: 'somerset.gameOver',
};
...
describe('SomersetPage', () => {
  ...
  it('does not show the progress summary on game clear', async () => {
    ...
    expect(screen.queryByTestId('somerset-gameover-summary')).not.toBeInTheDocument();
  });
  ...
  it('enables and pulses the auto-complete button once a foundation builds past its ace', async () => {
    const readyState: SomersetResponse = {
      ...playingState,
  ...
  it('shows StalemateEscapeButton when stalemate flag is set', async () => {
    const stalemate: SomersetResponse = {
      ...playingState,
      isStalemate: true,
      undoToEscape: 2,
      canUndo: true,
...
// test. See issue #4429.
describe('SomersetPage keyboard shortcuts', () => {
  it.each([
    ['h', 'hint'],
    ['a', 'autocomplete'],
    ['z', 'undo'],
...
// 選ぶ前に行き先が見える (#4454)。
describe('SomersetPage destination preview', () => {
  const render = async () => {
    mockExec.mockResolvedValue(playingState);
    renderWithProviders(<SomersetPage />);
