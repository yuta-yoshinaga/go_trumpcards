...
import { StalactitesPage } from './StalactitesPage';

vi.mock('../api/gameApi', () => ({
  stalactitesApi: { exec: vi.fn() },
  actionLogApi: { stalactites: vi.fn() },
}));
...
const playingState: StalactitesResponse = {
  ...
  isStalemate: false,
  message: '',
  messageCode: 'stalactites.playing',
};

const gameClearState: StalactitesResponse = {
  ...playingState,
  phase: 1,
  message: 'ゲームクリア！',
  messageCode: 'stalactites.gameClear',
  messageParams: { moveCount: '42' },
};

const gameOverState: StalactitesResponse = {
  ...playingState,
  phase: 2,
  message: 'ゲームオーバー',
  messageCode: 'stalactites.gameOver',
};

const withFoundationState: StalactitesResponse = {
  ...playingState,
  foundation: [[card('SPADE', 1)], [], [card('HEART', 1), card('HEART', 2)], []],
};

const withHintState: StalactitesResponse = {
  ...playingState,
  hint: { fromZone: 'stalactites', fromCol: -1, cardIndex: -1, toZone: 'tableau', toCol: 3 },
};

const withHintFromColState: StalactitesResponse = {
  ...playingState,
  hint: { fromZone: 'tableau', fromCol: 2, cardIndex: 0, toZone: 'foundation', toCol: -1 },
};

const withStalactitesCardState: StalactitesResponse = {
  ...playingState,
  baseRank: 1,
  cells: [card('DIAMOND', 7), null, null, null],
};
...
describe('StalactitesPage', () => {
  ...
  it('renders stalactites with card occupied', async () => {
    mockExec.mockResolvedValue(withStalactitesCardState);
    renderWithProviders(<StalactitesPage />);
    await waitFor(() => expect(screen.getByTestId('phase-indicator')).toBeInTheDocument());
    // The occupied stalactites should show a card image
    expect(screen.getByAltText('♦ 7')).toBeInTheDocument();
    // 3 empty cells remain
    const emptyButtons = screen.getAllByText('空');
  ...
  it('shows the auto-complete-ready badge and pulses the button when the board is solvable', async () => {
    ...
    expect(screen.getByTestId('stalactites-autocomplete-ready-badge')).toBeInTheDocument();
  });
  ...
  it('does not pulse or show the badge when a column blocks auto-complete', async () => {
    ...
    expect(screen.queryByTestId('stalactites-autocomplete-ready-badge')).not.toBeInTheDocument();
  });

  it('enables the auto-complete button (no tooltip) when the board is ready (#3035)', async () => {
    renderWithProviders(<StalactitesPage />); // default playingState is ready
  ...
  it('card selection via handleSelectSource on stalactites card click', async () => {
    mockExec.mockResolvedValue(withStalactitesCardState);
    renderWithProviders(<StalactitesPage />);
  ...
  it('double-clicking a free-cell card that can reach a foundation issues the foundation move', async () => {
    mockExec.mockResolvedValue({
      ...playingState,
      baseRank: 1,
      cells: [card('SPADE', 1), null, null, null],
      foundation: [[], [], [], []],
    });
    renderWithProviders(<StalactitesPage />);
    ...
    fireEvent.doubleClick(cardButton);
    await waitFor(() =>
      expect(mockExec).toHaveBeenCalledWith('move', { zone: 'stalactites', cell: 0 }, { zone: 'foundation', col: 0 }),
    );
  });
  ...
  it('hint display when hint is set', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/セル.*→.*タブロー 3/)).toBeInTheDocument());
  });

  // #5494: ゾーン識別子 ("tableau"/"stalactites"/"foundation") をそのまま i18n キーに
  ...
  it('action log button fetches and shows log', async () => {
    mockExec.mockResolvedValue(gameClearState);
    const mockLogApi = vi.mocked(actionLogApi.stalactites);
  ...
  it('reset hides the action log panel if open', async () => {
    mockExec.mockResolvedValue(gameClearState);
    vi.mocked(actionLogApi.stalactites).mockResolvedValueOnce({
      entries: [{ turnNumber: 1, playerIdx: 0, actionType: 'move', detail: 'tableau→foundation' }],
    });
  ...
  it('empty stalactites buttons have aria-label', async () => {
    renderWithProviders(<StalactitesPage />);
    await waitFor(() => expect(screen.getByTestId('phase-indicator')).toBeInTheDocument());

    for (let i = 0; i < 4; i++) {
      expect(screen.getByRole('button', { name: `セル ${i} (空)` })).toBeInTheDocument();
    }
  });
  ...
  it('renders HintTooltip when hintEnabled is true and hint is available', async () => {
    vi.mocked(useGameHint).mockReturnValue({
      hint: { targetAction: 'move', reason: 'frontendHint.useCells', confidence: 'moderate' },
      hintEnabled: true,
  ...
  describe('drag and drop', () => {
    ...
    it('marks only the deepest movable cards as draggable when free cells + empty cols are exhausted', async () => {
      ...
      const tightState: StalactitesResponse = {
        ...playingState,
        tableau: [
        ...
        baseRank: 1,
        cells: [card('DIAMOND', 8), card('CLOVER', 9), card('SPADE', 10), card('HEART', 11)],
    ...
    it('highlights the in-limit block under the cursor', async () => {
      const looseState: StalactitesResponse = {
        ...playingState,
        tableau: [
          [card('SPADE', 13), card('HEART', 12), card('CLOVER', 11)],
          [card('DIAMOND', 1)],
        ...
        baseRank: 1,
        cells: [card('DIAMOND', 8), card('CLOVER', 9), null, null],
      };
...
describe('StalactitesPage empty-column move limit', () => {
  ...
  it('marks the empty column as out of reach for a stack that only the lower limit blocks', async () => {
    ...
    // **このボタンには名前が無く、中身は文字の `K` だけだった** ── 支援技術には
    // 「K」としか読まれず、列も特定できず、超過の手掛かりも届かなかった (#6814)。
    const label = emptyCol.getAttribute('aria-label') ?? '';
    expect(label).toContain('空き列 1');
  ...
  it('leaves the empty column usable when the stack fits the lower limit', async () => {
    ...
    // 名前は付くが理由は足さない。列番号だけは常に読み上げられること。
    const label = emptyCol.getAttribute('aria-label') ?? '';
    expect(label).toContain('空き列 1');
    expect(label).not.toContain('空き列へ動かせるのは');
  });
});
