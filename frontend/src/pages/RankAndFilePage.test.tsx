...
import { actionLogApi, rankAndFileApi } from '../api/gameApi';
import { useGameHint } from '../hooks/useGameHint';
import { flushPendingDispatch } from '../test/flushPendingDispatch';
import { renderWithProviders } from '../test/renderWithProviders';
import type { Card, CardDesign, RankAndFileResponse, RankAndFileTableauCard } from '../types/card';
import { RankAndFilePage } from './RankAndFilePage';

vi.mock('../api/gameApi', () => ({
  rankAndFileApi: { exec: vi.fn() },
  actionLogApi: { rankandfile: vi.fn() },
}));
...
function makeTableau(cols: RankAndFileTableauCard[][]): RankAndFileTableauCard[][] {
  const result: RankAndFileTableauCard[][] = [];
  for (let i = 0; i < 10; i++) {
    result.push(cols[i] ?? []);
  }
  return result;
...
const playingNoWasteState: RankAndFileResponse = {
  ...playingState,
  waste: [],
};

const playingEmptyStockState: RankAndFileResponse = {
  ...playingState,
  stockCount: 0,
};

const gameClearState: RankAndFileResponse = {
  ...playingState,
  phase: 1,
  message: 'ゲームクリア！',
  messageCode: 'rankandfile.gameClear',
  messageParams: { moveCount: '42' },
};

const gameOverState: RankAndFileResponse = {
  ...playingState,
  phase: 2,
  message: 'ゲームオーバー',
  messageCode: 'rankandfile.gameOver',
};

const withFoundationState: RankAndFileResponse = {
  ...playingState,
...
const withDrawHintState: RankAndFileResponse = {
  ...playingState,
...
describe('RankAndFilePage', () => {
  ...
  it('clicking auto complete button dispatches autocomplete when ready', async () => {
    const readyState: RankAndFileResponse = {
      ...playingState,
      stockCount: 0,
      waste: [],
    };
  ...
  it('double-clicking a foundation-playable tableau top card sends it to a foundation', async () => {
    const aceState: RankAndFileResponse = {
      ...playingState,
    ...
    mockExec.mockClear();
    mockExec.mockResolvedValue(aceState);
    fireEvent.dblClick(screen.getByTestId('rf-tableau-top-0'));
  ...
  it('double-clicking a foundation-playable waste card sends it to a foundation', async () => {
    const aceWasteState: RankAndFileResponse = {
      ...playingState,
      tableau: makeTableau([]),
      waste: [card('SPADE', 1)],
      foundation: [[], [], [], [], [], [], [], []],
    ...
    renderWithProviders(<RankAndFilePage />);
    await waitFor(() => expect(screen.getByTestId('rf-waste-top')).toBeInTheDocument());

    mockExec.mockClear();
    mockExec.mockResolvedValue(aceWasteState);
    fireEvent.dblClick(screen.getByTestId('rf-waste-top'));
  ...
  it('announces the hinted card and destination in a polite live region', async () => {
    ...
    const region = await screen.findByTestId('rf-hint-announcement');
  ...
  it('shows the draw hint as a sentence, not as a move with column -1', async () => {
    ...
    expect(screen.getByTestId('rf-hint-display')).toHaveTextContent('山札から引きましょう');
    expect(screen.getByTestId('rf-hint-display').textContent).not.toContain('-1');
  });
  ...
  it('renders an empty hint announcement region when there is no hint', async () => {
    ...
    expect(screen.getByTestId('rf-hint-announcement')).toHaveTextContent('');
  });
  ...
  it('announces a tableau-origin hint by resolving the card at [fromCol][cardIndex]', async () => {
    ...
    const tableauHintState: RankAndFileResponse = {
      ...playingState,
  ...
  it('announces with an empty card name when a tableau hint points at a missing card', async () => {
    ...
    const missingCardHintState: RankAndFileResponse = {
      ...playingState,
  ...
  it('displays message with messageCode', async () => {
    mockExec.mockResolvedValue({
      ...
      messageCode: 'rankandfile.playing',
    });
    renderWithProviders(<RankAndFilePage />);
...
// test. See issue #4429.
describe('RankAndFilePage keyboard shortcuts', () => {
  it.each([
    ['d', 'draw'],
    ['h', 'hint'],
    ['a', 'autocomplete'],
...
// **配りは各列3枚を伏せる。**クローン元 (Forty Thieves) は40枚すべて表向きなので、
// そのままだと伏せ札を描く経路がテストされない ── fixture がクローン元の前提に
// 一致してしまい、契約を守らなくなる。
describe('RankAndFilePage face-down cards', () => {
  const withHidden: RankAndFileResponse = {
    ...playingState,
    tableau: makeTableau([
      [
        { card: card('DIAMOND', 9), faceUp: false },
        { card: card('SPADE', 6), faceUp: true },
      ],
    ]),
  };

  it('renders a face-down card as a card back, not as its face', async () => {
    mockExec.mockResolvedValue(withHidden);
    renderWithProviders(<RankAndFilePage />);
  ...
  it('does not let a face-down card be picked up', async () => {
    mockExec.mockResolvedValue(withHidden);
    renderWithProviders(<RankAndFilePage />);
    await waitFor(() => expect(screen.getByRole('button', { name: /♠ 6/ })).toBeInTheDocument());
