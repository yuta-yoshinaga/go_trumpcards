...
import { useWillOTheWispGame } from '../hooks/useWillOTheWispGame';
...
import { parseWillOTheWispCommand, WILLOTHEWISP_HELP } from '../utils/cli/commands/willothewispCommands';
import { formatWillOTheWispState } from '../utils/cli/formatters/willothewispFormatter';
import type { CliGameConfig } from '../utils/cli/types';
import { hintCheckboxItem } from '../utils/settingsItems';
import { isTableauAllFaceUp } from '../utils/solitaireUtils';

const WILLOTHEWISP_TUTORIAL_STEPS: TutorialStep[] = [
  {
    target: '[data-tutorial="willothewisp-stock-pile"]',
    messageKey: 'tutorial.stockPile',
    placement: 'bottom',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="willothewisp-tableau"]',
    messageKey: 'tutorial.tableau',
    placement: 'top',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="willothewisp-completed-suits"]',
    messageKey: 'tutorial.completedSuits',
    placement: 'bottom',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="willothewisp-controls"]',
    messageKey: 'tutorial.controls',
    placement: 'top',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="willothewisp-reset-button"]',
...
/** Renders the WillOTheWisp game page (Klondike setup + Spider rules with a single deck). */
...
function WillOTheWispPageContent() {
  const {
    ...
    confirmGiveUp,
    cancelGiveUp,
  } = useGamePageSetup('willothewisp');
  const game = useWillOTheWispGame();
  const {
    state,
    loading,
    error,
  ...
  } = useGameHint('willothewisp', state);

  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('willothewisp');
  const cliConfig: CliGameConfig<WillOTheWispResponse, Parameters<typeof willothewispApi.exec>> = useMemo(
    () => ({
      gameName: 'willothewisp',
      parseCommand: parseWillOTheWispCommand,
      formatResponse: formatWillOTheWispState,
      helpText: WILLOTHEWISP_HELP,
    }),
    [],
  );
  ...
  if (!state)
    return <GameSkeleton gameKey="willothewisp" layout={{ kind: 'tableau', topRow: 1, tableau: TABLEAU_COLS }} />;
  ...
  const isPlaying = state.phase === WillOTheWispPhase.PLAYING;
  const isGameClear = state.phase === WillOTheWispPhase.GAME_CLEAR;
  const isGameOver = state.phase === WillOTheWispPhase.GAME_OVER;
  ...
  return (
    <GamePageShell
      title={tc('nav.willothewisp')}
      gameThemeBg={gameTheme.willothewisp.bg}
      phaseName={isGameClear ? t('phase.gameClear') : isGameOver ? t('phase.gameOver') : t('phase.playing')}
      gamePath="/willothewisp"
      gameEndFlag={isEnded}
      winShow={isGameClear}
      loading={loading}
      ...
      headerExtra={
        <>
          ...
          <span className="relative inline-flex ml-3" ref={scoreRef}>
            <button
              type="button"
              data-testid="willothewisp-score"
            ...
            {scoreOpen && (
              ...
              <section
                data-testid="willothewisp-score-popover"
                aria-label={t('score')}
      ...
      headerEnd={
        <span className="ml-3" data-tutorial="willothewisp-completed-suits">
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto pt-3 px-4 lg:px-8">
            <div className="flex gap-2 mb-3 items-start">
              <div className="text-center" data-tutorial="willothewisp-stock-pile">
                ...
                {state.tableau.map((col, colIdx) => {
                  ...
                  return (
                    <div
                      ...
                      data-testid={`willothewisp-col-${colIdx.toString()}`}
                      className={
                                ...
                                >
                                  {tc.faceUp && tc.card ? (
                                    <button
                                      ...
                                      data-testid={`willothewisp-card-${colIdx.toString()}-${cardIdx.toString()}`}
            ...
            {/*
              ...
              られないことがある (#5955)。
            */}
            <div data-testid="willothewisp-hint-live" role="status" aria-live="polite">
              {hint && (
          ...
          <GameFooter className={`${gameTheme.willothewisp.footer} px-4 py-2.5`}>
            <ErrorAlert message={error ?? hintError} onRetry={retry} />
            <div className="flex gap-2 items-center flex-wrap">
              {isPlaying && (
                <div data-tutorial="willothewisp-controls">
                  <button
                    type="button"
