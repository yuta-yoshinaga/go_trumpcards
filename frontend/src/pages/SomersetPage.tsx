...
import { parseSomersetCommand, SOMERSET_HELP } from '../utils/cli/commands/somersetCommands';
import { formatSomersetState } from '../utils/cli/formatters/somersetFormatter';
import type { CliGameConfig } from '../utils/cli/types';
import { hintCheckboxItem } from '../utils/settingsItems';
import { somersetLegalTargets } from '../utils/somersetLegalTargets';

const FOUNDATION_SUITS = ['♠', '♣', '♥', '♦'] as const;

const BC_TUTORIAL_STEPS: TutorialStep[] = [
  {
    target: '[data-tutorial="somerset-foundation"]',
    messageKey: 'tutorial.foundation',
    placement: 'bottom',
    advanceOn: 'next',
  },
  {
    target: '[data-tutorial="somerset-tableau"]',
  ...
  {
    target: '[data-tutorial="somerset-controls"]',
...
/** Renders the Somerset solitaire game page with 10 tableau columns and 4 foundations. */
export const SomersetPage = withTutorial(SomersetPageContent, 'somerset', BC_TUTORIAL_STEPS);
...
function SomersetPageContent() {
  ...
  } = useGameHint('somerset', state);
  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('somerset');
  const cliConfig: CliGameConfig<SomersetResponse, Parameters<typeof somersetApi.exec>> = useMemo(
    () => ({
      gameName: 'somerset',
      parseCommand: parseSomersetCommand,
      formatResponse: formatSomersetState,
      helpText: SOMERSET_HELP,
    }),
    [],
  );
  ...
  const dnd = useSolitaireDragDrop<SomersetMoveZone>({
    onMove: dispatchMove,
  ...
  const preview = useDestinationPreview<SomersetMoveZone>(selectedSource);

  if (!state) return <GameSkeleton gameKey="somerset" layout={{ kind: 'tableau', topRow: 4, tableau: 10 }} />;

  const isPlaying = state.phase === SomersetPhase.PLAYING;
  ...
  const previewedCard =
    previewSource?.zone === 'tableau' && previewSource.col !== undefined && previewSource.cardIndex !== undefined
      ...
      : undefined;
  const legalTargets = somersetLegalTargets(state.tableau, state.foundation, previewedCard);
  ...
  const renderTableauColumn = (colIdx: number) => {
    ...
    const tableauColZone: SomersetMoveZone = { zone: 'tableau', col: colIdx };
    return (
      <div
        ...
        >
          <div className="relative" style={{ minHeight: dims.ch }}>
            {col.length === 0 ? (
              ...
              col.map((tc, cardIdx) => {
                const cardZone: SomersetMoveZone = {
                  zone: 'tableau',
                  col: colIdx,
                  cardIndex: cardIdx,
                };
  ...
  return (
    <GamePageShell
      title={tc('nav.somerset')}
      gameThemeBg={gameTheme.somerset.bg}
      ...
      {cliEnabled ? (
        ...
        <>
          <div className="flex-1 overflow-y-auto pt-3 px-2 sm:px-4 lg:px-8">
            ...
            */}
            <div
              data-tutorial="somerset-hint-display"
              data-testid="somerset-hint-live"
              role="status"
              aria-live="polite"
            >
              {hint && (
          ...
          <GameFooter className={`${gameTheme.somerset.footer} px-4 py-2.5`}>
            ...
            <div className="flex gap-2 items-center flex-wrap">
              ...
              <GameResetButton
                ...
                loading={loading}
                dataTutorial="somerset-reset-button"
              />
            </div>
            <ActionShortcutsPanel bindings={actionBindings} data-testid="somerset-kbd-shortcuts" />
          </GameFooter>
        </>
      )}
    </GamePageShell>
