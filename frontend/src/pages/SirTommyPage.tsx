...
import { SirTommyPhase } from '../types/phases';
...
type ApiArgs = Parameters<typeof sirtommyApi.exec>;

type Source = { kind: 'stock' } | { kind: 'waste'; idx: number };

function parseSirTommyCommand(input: string): CliParseResult<ApiArgs> {
...
function formatSirTommyState(state: SirTommyResponse): string {
  const lines: string[] = [];
  const phase =
    state.phase === SirTommyPhase.GAME_CLEAR ? 'CLEAR' : state.phase === SirTommyPhase.GAME_OVER ? 'OVER' : 'Playing';
...
export const SirTommyPage = withTutorial(SirTommyPageContent, 'sirtommy', CA_TUTORIAL_STEPS);
function SirTommyPageContent() {
  const {
    t,
    tc,
    actionLog,
  ...
  const {
    ...
    exec: runApi,
    retry,
  } = useGameApi<SirTommyResponse, ApiArgs>((...args) => sirtommyApi.exec(...args));
  ...
  } = useGameHint('sirtommy', state);

  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('sirtommy');
  const cliConfig: CliGameConfig<SirTommyResponse, ApiArgs> = useMemo(
    () => ({
      gameName: 'sirtommy',
      parseCommand: parseSirTommyCommand,
      formatResponse: formatSirTommyState,
      helpText: [
  ...
  if (!state) return <GameSkeleton gameKey="sirtommy" layout={{ kind: 'tableau', topRow: 6, tableau: 7 }} />;

  const isPlaying = state.phase === SirTommyPhase.PLAYING;
  ...
  return (
    <GamePageShell
      title={tc('nav.sirtommy')}
      gameThemeBg={gameTheme.sirtommy.bg}
      phaseName={phaseName}
      gamePath="/sirtommy"
      gameEndFlag={isEnded}
      winShow={isGameClear}
    ...
    >
      {cliEnabled ? (
        ...
        <>
          ...
          <GameFooter className={`${gameTheme.sirtommy.footer} px-4 py-2.5`}>
