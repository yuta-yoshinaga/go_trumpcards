...
import { useCitadelGame } from '../hooks/useCitadelGame';
...
import { citadelLegalTargets } from '../utils/citadelLegalTargets';
import { CITADEL_HELP, parseCitadelCommand } from '../utils/cli/commands/citadelCommands';
import { formatCitadelState } from '../utils/cli/formatters/citadelFormatter';
...
/** Renders the Citadel solitaire game page with 8 tableau columns and 4 foundations. */
export const CitadelPage = withTutorial(CitadelPageContent, 'citadel', BC_TUTORIAL_STEPS);
...
function CitadelPageContent() {
  const {
    ...
    confirmGiveUp,
    cancelGiveUp,
  } = useGamePageSetup('citadel');
  const game = useCitadelGame();
  ...
  } = useGameHint('citadel', state);
  const { cliEnabled, toggleCli, logEntries, addInput, addOutput, addError, clearLog } = useCliMode('citadel');
  const cliConfig: CliGameConfig<CitadelResponse, Parameters<typeof citadelApi.exec>> = useMemo(
    () => ({
      gameName: 'citadel',
      parseCommand: parseCitadelCommand,
      formatResponse: formatCitadelState,
      helpText: CITADEL_HELP,
    }),
    [],
  );
  ...
  const isPlayingForKbd = state?.phase === CitadelPhase.PLAYING;

  const dispatchMove = useCallback(
    (source: CitadelMoveZone, target: CitadelMoveZone) => {
      void game.exec('move', source, target);
    },
    [game],
  );
  const dnd = useSolitaireDragDrop<CitadelMoveZone>({
    onMove: dispatchMove,
  ...
  const preview = useDestinationPreview<CitadelMoveZone>(selectedSource);

  if (!state) return <GameSkeleton gameKey="citadel" layout={{ kind: 'tableau', topRow: 4, tableau: 8 }} />;

  const isPlaying = state.phase === CitadelPhase.PLAYING;
  const isGameClear = state.phase === CitadelPhase.GAME_CLEAR;
  const isGameOver = state.phase === CitadelPhase.GAME_OVER;
  ...
  const renderTableauColumn = (colIdx: number) => {
    ...
    const tableauColZone: CitadelMoveZone = { zone: 'tableau', col: colIdx };
    return (
      <div
        ...
        >
          <div className="relative" style={{ minHeight: dims.ch }}>
            {col.length === 0 ? (
              ...
              col.map((tc, cardIdx) => {
                const cardZone: CitadelMoveZone = {
                  zone: 'tableau',
                  col: colIdx,
                  cardIndex: cardIdx,
                };
  ...
  return (
    <GamePageShell
      title={tc('nav.citadel')}
      gameThemeBg={gameTheme.citadel.bg}
    ...
    >
      ...
      {cliEnabled ? (
        ...
        <>
          ...
          <GameFooter className={`${gameTheme.citadel.footer} px-4 py-2.5`}>
