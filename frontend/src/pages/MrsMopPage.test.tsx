...
import { actionLogApi, mrsMopApi } from '../api/gameApi';
import { useGameHint } from '../hooks/useGameHint';
import { flushPendingDispatch } from '../test/flushPendingDispatch';
import { renderWithProviders } from '../test/renderWithProviders';
import type { Card, CardDesign, MrsMopResponse, MrsMopTableauCard } from '../types/card';
import { MrsMopPage } from './MrsMopPage';

vi.mock('../api/gameApi', () => ({
  mrsMopApi: { exec: vi.fn() },
  actionLogApi: { mrsmop: vi.fn() },
}));
...
const playingState: MrsMopResponse = {
  tableau: makeTableau([
...
const gameClearState: MrsMopResponse = {
  ...playingState,
  phase: 1,
  message: 'ゲームクリア！',
  messageCode: 'mrsmop.gameClear',
  messageParams: { moveCount: '42', score: '500' },
};

const gameOverState: MrsMopResponse = {
  ...playingState,
  phase: 2,
  message: 'ゲームオーバー',
  messageCode: 'mrsmop.gameOver',
};
...
describe('MrsMopPage', () => {
  ...
  it('announces and flashes when a new suit is completed', async () => {
    ...
    mockExec.mockResolvedValue({ ...filled, canUndo: true }); // completedSuits 0
    renderWithProviders(<MrsMopPage />);
    ...
    mockExec.mockResolvedValue({ ...filled, completedSuits: 1, canUndo: true });
  ...
  it('clicking auto complete button dispatches autocomplete when ready', async () => {
    const readyState: MrsMopResponse = {
      ...playingState,
      stockCount: 0,
  ...
  it('action log button fetches and shows log', async () => {
    mockExec.mockResolvedValue(gameClearState);
    const mockLogApi = vi.mocked(actionLogApi.mrsmop);
  ...
  it('announces the hint through a region that was already mounted', async () => {
    ...
    renderWithProviders(<MrsMopPage />);
    await waitFor(() => expect(screen.getByRole('button', { name: 'ヒント' })).toBeInTheDocument());
    const region = screen.getByTestId('mrsMop-hint-live');
  ...
  it('reset hides the action log panel if open', async () => {
    mockExec.mockResolvedValue(gameClearState);
    vi.mocked(actionLogApi.mrsmop).mockResolvedValueOnce({
      entries: [{ turnNumber: 1, playerIdx: 0, actionType: 'move', detail: 'tableau→tableau' }],
    });
  ...
  describe('per-difficulty stats (#3062)', () => {
    ...
    it('records a win to localStorage and shows the best badge', async () => {
      ...
      renderWithProviders(<MrsMopPage />);
      await waitFor(() => {
        const stored = JSON.parse(localStorage.getItem('trumpcards-mrsMop-stats') ?? '{}');
