...
import en from './locales/en/spider.json';
import ja from './locales/ja/spider.json';

// スパイダーの配置規則は2段構えで、チュートリアルはそれを取り違えていた (#5503)。
//
//   canPlaceOnTableau   -- 1枚を1枚の上に置くのは「値が1つ小さい」だけが条件。**スート不問**
//   isValidSpiderSequence -- 同スート降順が要るのは**まとめて動かすとき**だけ
//
// 旧文言は「同じスートの降順でカードを並べます」で、1枚配置にまで同スート制限が
// あるかのように読める。実際には合法な異スートへの配置をプレイヤーが避けてしまう。
// docs/manual/web/spider.md は最初から正しく書き分けているので、そちらが基準。
describe('spider tutorial text matches the actual placement rules', () => {
