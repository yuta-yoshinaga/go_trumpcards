...
| `useGameHint` | `src/hooks/useGameHint.ts` | Frontend hints; registry-driven via `hintFactories` (covers nearly all games; currently 205). The supported set is the `HintGameName` union (`keyof typeof hintFactories`) — add a game by registering its factory there |
