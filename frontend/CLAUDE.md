...
| `useGameHint` | `src/hooks/useGameHint.ts` | Frontend hints; registry-driven via `hintFactories` (covers ~all games, currently 134). The supported set is the `HintGameName` union (`keyof typeof hintFactories`) — add a game by registering its factory there |
