...
async function emittedCodes() {
  ...
  // The third emission path: a domain error that names a key instead of a
  // finished phrase. `NewDomainErrorCode(ErrInvalidPlay, "missmilligan.errX", …)`
  // is forwarded by the presenter into the same messageCode field, so an
  ...
  for (const name of domainFiles) {
    ...
    for (const m of text.matchAll(/NewDomainErrorCode\(\s*[A-Za-z0-9_.]+\s*,\s*"([a-zA-Z0-9_.]+)"/g)) {
      const code = m[1];
      if (!out.has(code)) out.set(code, { literals: new Set(), files: new Set() });
      out.get(code).literals.add('<domain-error>');
      out.get(code).files.add(name);
    }
  }
  return out;
