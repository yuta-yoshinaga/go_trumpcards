...
// TestSambaWebMinMeldMatchesTheDomain guards the web's copy of the initial-meld
...
func TestSambaWebMinMeldMatchesTheDomain(t *testing.T) {
	root := filepath.Join("..", "..", "..")
	path := filepath.Join(root, "frontend", "src", "utils", "sambaScore.ts")
	src, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("read %s: %v", path, err)
	}

	body := regexp.MustCompile(`(?s)export function sambaMinMeld\(cumulativeScore: number\): number \{(.*?)\n\}`).
		FindStringSubmatch(string(src))
	if body == nil {
		t.Fatal("sambaScore.ts no longer defines sambaMinMeld as a plain function")
	}
	...
	if len(branches) == 0 || len(fallback) == 0 {
		t.Fatalf("sambaMinMeld no longer states its thresholds as literals: %q", body[1])
	}
