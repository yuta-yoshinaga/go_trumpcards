...
var gameDescriptions = map[string]string{
	...
	"courchevelhilo":      "Courchevel Hi-Lo (クールシュヴェル ハイロー)",
	...
	"speculation":         "Speculation (スペキュレーション)",
	"dramaha":             "Dramaha (ドラマハ)",
	...
	"unsunkaruta":         "うんすんカルタ（八人メリ。75枚5スート、丸物の数札は1が最強、メリ/モンチの宣言でフォロー義務）",
	"quodlibet":           "クオドリベット（オーストリアの複合ゲーム。32枚・4人、3つの輪で12種目を1回ずつ打ち、罰点が最も少ない人が勝ち）",
	"dehlapakad":          "デーラ・パカド（インドの4人2対2。10（デーラ）の争奪。札は同じ席が2トリック続けて取ってはじめて手に入る）",
	"sutda":               "ソッタ（韓国の花札20枚。手札2枚で役を作り、ベットして競う。最上位は38光ッタン）",
	"cirulla":             "チルッラ（リグーリアのフィッシング。Scopa の捕獲に「合計15」とアッソの総取りが加わり、配札ボーナスもある）",
	"diloti":              "ディロティ（ギリシャのフィッシング。同ランクと合計一致で取り、宣言で束を作る。1 枚で場を払うと「クセリ」10点）",
	"comet":               "コメット（stops 系の祖。スートを無視してランクだけで昇り、9♦「コメット」がどのランクの代わりにもなる）",
	"costlycolours":       "コストリー・カラーズ（Cribbage の祖先。15/25/31 で数え、色とスートの梯子「コストリー」が加わる）",
	"bolivia":             "ボリビア（カナスタ系。ワイルドだけの「ボリビア」を認め、上がりにはワイルド無しの連番「エスカレラ」が要る）",
	"baccaratbanque":      "バカラ・バンク（3組156枚。左右2つの子に同時配当し、バンクは負けても動かずシューを配り切るまで続く）",
	"continentalrummy":    "コンチネンタル・ラミー（セットは組にならず、15枚すべてを同スート連番だけで一度に並べて上がる）",
