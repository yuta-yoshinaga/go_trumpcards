...
func TestGameManager_NewGameManager_AllGamesRegistered(t *testing.T) {
	mgr := NewGameManager("blackjack")
	// Verify that all 40 games are registered.
	for _, name := range []string{"canasta", "bridge", "pineapple", "gofish", "pinochle", "pigtail", "sevencardstud", "clocksolitaire", "durak"} {
		assert.Contains(t, mgr.games, name, "game %q should be registered", name)
	}
	assert.Equal(t, 40, len(mgr.games))
}
