...
// Ramsのフェーズ定数
const (
	// RamsPhaseDecide 参加するか降りるかの選択中
	RamsPhaseDecide RamsPhase = iota
	// RamsPhasePlay プレイ中
	RamsPhasePlay
	// RamsPhaseRoundEnd ラウンド終了（配当確定、次ラウンド待ち）
	RamsPhaseRoundEnd
	// RamsPhaseGameEnd ゲーム終了
	RamsPhaseGameEnd
)

// RamsHandSize 各プレイヤーの手札枚数
const RamsHandSize = 5

// RamsTricksPerRound 1 ラウンドのトリック数
const RamsTricksPerRound = RamsHandSize

// RamsStartingChips 開始時の持ちチップ
const RamsStartingChips = 60

// RamsAnte 各ラウンドの開始時に全員がポットへ出すチップ
const RamsAnte = 3

// RamsMissPenalty 参加したのに 1 トリックも取れなかったときの追加支払い
const RamsMissPenalty = 5

// ramsMaxSliceLen caps slice sizes during deserialisation.
const ramsMaxSliceLen = 1000
...
//
//   - **手札の枚数が書かれていない。** ラムスは 5 枚が標準で、流用元に指定された
//     Loo も 5 枚 (`LooHandSize`)。5 人でも 25 枚しか要らず 32 枚に収まる。
//   - **切り札の規定が無い。** ラムスは配り切ったあとの 1 枚を表向きにして切り札を
...
// NewRams コンストラクタ
func NewRams(trumpCards *TrumpCards, players []*RamsPlayer, config RamsConfig) *Rams {
	return &Rams{trumpCards: trumpCards, players: players, config: config, winnerIdx: -1}
}

// NewDefaultRams 既定構成（人間 1 + CPU 3）のコンストラクタ
func NewDefaultRams() *Rams {
	cfg := DefaultRamsConfig()
	return NewRams(NewTrumpCardsBelote(), newRamsPlayers(cfg.PlayerCnt), cfg)
}

// newRamsPlayers 指定人数のプレイヤーを作る（0 番だけが人間）
func newRamsPlayers(n int) []*RamsPlayer {
	players := make([]*RamsPlayer, 0, n)
	for i := range n {
		players = append(players, NewRamsPlayer(i == 0))
	}
	return players
}
...
func (r *Rams) Reset() {
	...
	if len(r.players) != r.config.PlayerCnt {
		r.players = newRamsPlayers(r.config.PlayerCnt)
	}
	r.roundNumber = 1
	r.dealerIdx = 0
	r.gameEndFlag = false
...
func (r *Rams) sortAllHands() {
	for _, p := range r.players {
		sortPlayerHand(p, func(ci, cj *Card) bool {
			...
			return ramsRank(ci) < ramsRank(cj)
		})
	}
}

// Decide 人間プレイヤーが参加 (play) するか降りる (pass) かを選ぶ
func (r *Rams) Decide(play bool) error {
	if r.gameEndFlag {
		return errors.New("game has ended")
	}
	if r.phase != RamsPhaseDecide {
...
func (r *Rams) setDecision(idx int, play bool) {
	p := r.players[idx]
	p.SetDecided(true)
	p.SetInRound(play)
	action := "pass"
...
func (r *Rams) cpuWantsToPlay(idx int) bool {
	p := r.players[idx]
	strength := 0
	for i := range p.GetCardsSize() {
		c := p.GetCard(i)
		switch {
		case c.GetDesign() == r.trumpSuit && ramsRank(c) >= CardValueMax:
			strength += 3 // 切り札の A / K
		case c.GetDesign() == r.trumpSuit:
			strength++
		case ramsRank(c) > CardValueMax:
			strength += 2 // 平のエース
		}
	}
	return strength >= 3
...
func (r *Rams) firstActiveFrom(idx int) int {
	for i := range len(r.players) {
...
func (r *Rams) nextActiveAfter(idx int) int {
	return r.firstActiveFrom((idx + 1) % len(r.players))
}

// PlayerPlay 人間プレイヤーが手札の cardIndex を出す
func (r *Rams) PlayerPlay(cardIndex int) error {
	if r.gameEndFlag {
		return errors.New("game has ended")
	}
	if r.phase != RamsPhasePlay {
...
// CpuPlay CPU が 1 枚出す
func (r *Rams) CpuPlay() {
	if r.gameEndFlag || r.phase != RamsPhasePlay || r.currentPlayerIdx == 0 {
		return
	}
...
// play 指定プレイヤーが 1 枚出す
func (r *Rams) play(playerIdx, cardIndex int) error {
...
func (r *Rams) canPlay(playerIdx int, card *Card) bool {
...
func (r *Rams) GetValidPlayIndices(playerIdx int) []int {
...
func (r *Rams) resolveTrick() {
	...
	if r.trickNumber >= RamsTricksPerRound {
		r.finishRound()
	}
}
...
func (r *Rams) NextRound() {
	if r.gameEndFlag || r.phase != RamsPhaseRoundEnd {
		return
	}
	r.roundNumber++
...
func (r *Rams) trickWinner() int {
	...
	for _, tc := range r.currentTrick[1:] {
		if ramsBeats(tc.Card, best, leadSuit, r.trumpSuit) {
...
// ramsBeats challenger が currentBest に勝つか
func ramsBeats(challenger, currentBest *Card, leadSuit, trumpSuit int) bool {
	...
	if cTrump && bTrump {
		return ramsRank(challenger) > ramsRank(currentBest)
	}
...
// ramsRank 札の強さ。A が最強、7 が最弱。
func ramsRank(c *Card) int {
	if c == nil {
		return 0
	}
	if v := c.GetValue(); v == 1 {
...
func (r *Rams) chooseCpuCard(playerIdx int) int {
	...
	if len(r.currentTrick) == 0 {
		...
		bestIdx, bestRank := valid[0], ramsRank(p.GetCard(valid[0]))
		for _, i := range valid[1:] {
			if rank := ramsRank(p.GetCard(i)); rank > bestRank {
...
func (r *Rams) pickWinning(p *RamsPlayer, valid []int) (int, bool) {
	leadSuit := r.currentTrick[0].Card.GetDesign()
	...
	for _, i := range valid {
		c := p.GetCard(i)
		if !ramsBeats(c, best, leadSuit, r.trumpSuit) {
			continue
		}
		if rank := ramsRank(c); bestIdx < 0 || rank < bestRank {
			bestIdx, bestRank = i, rank
		}
	}
...
func (r *Rams) GetHint() *RamsHint {
	if r.gameEndFlag {
		return nil
	}
	// **選択フェーズでは出す札ではなく、参加するかどうかを助言する。**
	if r.phase == RamsPhaseDecide && !r.players[0].GetDecided() {
		if r.cpuWantsToPlay(0) {
			return &RamsHint{Reason: "ramsPlayIn"}
		}
		return &RamsHint{Reason: "ramsPassOut"}
	}
	if !r.IsHumanTurn() || r.players[0].GetCardsSize() == 0 {
		return nil
	}
	idx := r.chooseCpuCard(0)
	reason := "ramsTakeTrick"
	if r.players[0].GetRoundTricks() > 0 {
		reason = "ramsAlreadySafe"
	}
	return &RamsHint{CardIndex: &idx, Reason: reason}
}

// --- Getters ---

// GetPhase 現在のフェーズ
func (r *Rams) GetPhase() RamsPhase { return r.phase }

// GetConfig 現在の設定
func (r *Rams) GetConfig() RamsConfig { return r.config }

// SetConfig 設定を差し替える
func (r *Rams) SetConfig(c RamsConfig) { r.config = c }

// GetRoundNumber 現在のラウンド番号（1 起点）
func (r *Rams) GetRoundNumber() int { return r.roundNumber }

// GetTrickNumber 現在のトリック番号（0 起点）
func (r *Rams) GetTrickNumber() int { return r.trickNumber }

// GetPot 場に積まれているチップ
func (r *Rams) GetPot() int { return r.pot }

// GetTrumpSuit 切り札のスート
func (r *Rams) GetTrumpSuit() int { return r.trumpSuit }

// GetUpCard 切り札を決めた表向きの 1 枚
func (r *Rams) GetUpCard() *Card { return r.upCard }
...
func (r *Rams) GetLeadPlayerIdx() int { return r.leadPlayerIdx }

// GetDealerIdx ディーラー
func (r *Rams) GetDealerIdx() int { return r.dealerIdx }

// GetActiveCount このラウンドの参加者数
func (r *Rams) GetActiveCount() int { return r.activeCount() }

// GetPlayerCnt プレイヤー数
func (r *Rams) GetPlayerCnt() int { return len(r.players) }

// GetPlayer 指定インデックスのプレイヤー
func (r *Rams) GetPlayer(i int) *RamsPlayer {
...
func (r *Rams) GetGameEndFlag() bool { return r.gameEndFlag }

// GetWinnerIdx 勝者（-1: 未確定または同点）
func (r *Rams) GetWinnerIdx() int { return r.winnerIdx }

// IsHumanTurn 人間の手番か
func (r *Rams) IsHumanTurn() bool {
	return !r.gameEndFlag && r.phase == RamsPhasePlay && r.currentPlayerIdx == 0 && r.players[0].GetInRound()
}

// IsDecidePhase 参加するかどうかの選択中か
func (r *Rams) IsDecidePhase() bool {
	return !r.gameEndFlag && r.phase == RamsPhaseDecide && !r.players[0].GetDecided()
}
...
func (r *Rams) UnmarshalJSON(data []byte) error {
	var j ramsJSON
	if err := json.Unmarshal(data, &j); err != nil {
		return err
	}
	if j.Phase < RamsPhaseDecide || j.Phase > RamsPhaseGameEnd {
		return fmt.Errorf("invalid phase: %d", j.Phase)
	}
	if j.TrickNumber < 0 || j.TrickNumber > RamsTricksPerRound {
		return fmt.Errorf("invalid trick number: %d", j.TrickNumber)
	}
	if j.RoundNumber < 1 || j.RoundNumber > RamsRoundsMax {
