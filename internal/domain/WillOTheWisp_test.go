...
func TestNewWillOTheWisp(t *testing.T) {
	tc := NewTrumpCards(0)
	s := NewWillOTheWisp(tc)
	assert.NotNil(t, s)
	assert.Equal(t, WillOTheWispPhasePlaying, s.GetPhase())
}

func TestNewDefaultWillOTheWisp(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	assert.NotNil(t, s)
}

func TestWillOTheWispReset(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	...
	// Will o' the Wisp starts with three face-up cards in every column.
	tableau := s.GetTableau()
	dealt := 0
	for i := range WillOTheWispTableauCnt {
		assert.Len(t, tableau[i], WillOTheWispInitialPerColumn, "col %d", i)
		dealt += WillOTheWispInitialPerColumn
		for j, tc := range tableau[i] {
			assert.True(t, tc.FaceUp, "card %d in col %d should be face up", j, i)
		}
	}
	assert.Equal(t, 21, dealt)
	assert.Equal(t, 31, s.GetStockCount(), "stock should hold the remaining 52-21 cards")
	assert.True(t, s.AllFaceUp())
}

func TestWillOTheWispDeal(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()

	for deal := 1; deal <= 4; deal++ {
		require.NoError(t, s.Deal())
		assert.Equal(t, 31-deal*WillOTheWispDealCnt, s.GetStockCount())
	}
	assert.Equal(t, 4, s.GetMoveCount())
	assert.Equal(t, 496, s.GetScore())

	require.NoError(t, s.Deal())
	assert.Equal(t, 0, s.GetStockCount())
	tableau := s.GetTableau()
	for i := range WillOTheWispTableauCnt {
		expected := WillOTheWispInitialPerColumn + 4
		if i < 3 {
			expected++
		}
		assert.Len(t, tableau[i], expected, "col %d", i)
		for j, tc := range tableau[i] {
			assert.True(t, tc.FaceUp, "card %d in col %d should be face up", j, i)
		}
	}
	assert.Equal(t, 5, s.GetMoveCount())
	assert.Equal(t, 495, s.GetScore())
	assert.True(t, s.AllFaceUp())
}

func TestWillOTheWispDeal_NotPlayingPhase(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	s.SetPhase(WillOTheWispPhaseGameOver)
	err := s.Deal()
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "not in playing phase")
}

func TestWillOTheWispDeal_NoStock(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	s.SetStock(nil)
	err := s.Deal()
...
// TestWillOTheWispDeal_Partial verifies that a final deal with fewer cards than
// columns still distributes the remaining cards from the left (#1676 review).
func TestWillOTheWispDeal_Partial(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	// Give every column one card so none is empty, then leave 3 cards in stock.
	var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
	for i := range WillOTheWispTableauCnt {
		tableau[i] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 5, false), FaceUp: true}}
	}
	...
	for i := 3; i < WillOTheWispTableauCnt; i++ {
		assert.Len(t, result[i], 1, "col %d untouched on partial deal", i)
	}
}

// TestWillOTheWispStalemate_PartialStockIsDealable verifies that a non-empty
...
func TestWillOTheWispStalemate_PartialStockIsDealable(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	...
	// predicate is internal. Use TestWillOTheWispDeal_Partial-style coverage:
	...
	require.NoError(t, s.Deal())
	// After Deal: stock has 0 left (3 dealt), so re-running checkWillOTheWispStalemate
...
// TestWillOTheWispUndo_RestoresActionLog verifies that undo truncates the
// action log so the undone entry is removed (#1676 review).
func TestWillOTheWispUndo_RestoresActionLog(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
	for i := range WillOTheWispTableauCnt {
		tableau[i] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 5+i, false), FaceUp: true}}
	}
	s.SetTableau(tableau)
...
func TestWillOTheWispDeal_EmptyColumn(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
	for i := range WillOTheWispTableauCnt {
...
func TestWillOTheWispMoveTableauToTableau(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	...
	log := s.GetActionLog()
	assert.Equal(t, "willothewisp.log.move", log[0].DetailCode)
...
func TestWillOTheWispMoveTableauToTableau_Shorthand(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
	for i := range WillOTheWispTableauCnt {
...
func TestWillOTheWispMoveTableauToTableau_Errors(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	...
	t.Run("not playing", func(t *testing.T) {
		...
		err := s.MoveTableauToTableau(0, 0, 1)
		assert.Error(t, err)
		s.SetPhase(WillOTheWispPhasePlaying)
	})

	t.Run("invalid from col", func(t *testing.T) {
		assert.Error(t, s.MoveTableauToTableau(-1, 0, 1))
		assert.Error(t, s.MoveTableauToTableau(WillOTheWispTableauCnt, 0, 1))
	})

	t.Run("invalid to col", func(t *testing.T) {
		assert.Error(t, s.MoveTableauToTableau(0, 0, -1))
		assert.Error(t, s.MoveTableauToTableau(0, 0, WillOTheWispTableauCnt))
	})
	...
	t.Run("face down card", func(t *testing.T) {
		var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
		for i := range WillOTheWispTableauCnt {
			tableau[i] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 5, false), FaceUp: false}}
		}
		s.SetTableau(tableau)
		assert.Error(t, s.MoveTableauToTableau(0, 0, 1))
	})
	...
	t.Run("not valid sequence", func(t *testing.T) {
		var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
		tableau[0] = []*WillOTheWispTableauCard{
			{Card: NewCard(CardDesignSpade, 5, false), FaceUp: true},
			{Card: NewCard(CardDesignHeart, 3, false), FaceUp: true},
		}
		tableau[1] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 6, false), FaceUp: true}}
		for i := 2; i < WillOTheWispTableauCnt; i++ {
			tableau[i] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 1, false), FaceUp: true}}
		}
		s.SetTableau(tableau)
		err := s.MoveTableauToTableau(0, 0, 1)
		assert.Error(t, err)
	...
	t.Run("cannot place on tableau", func(t *testing.T) {
		var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
		tableau[0] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 5, false), FaceUp: true}}
		tableau[1] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 3, false), FaceUp: true}}
		for i := 2; i < WillOTheWispTableauCnt; i++ {
			tableau[i] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 1, false), FaceUp: true}}
		}
		s.SetTableau(tableau)
...
func TestWillOTheWispMoveToEmptyColumn(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
...
func TestWillOTheWispCompletedSuit(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()

	var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
	col0 := make([]*WillOTheWispTableauCard, 0, CardValueMax)
	for v := CardValueMax; v >= 2; v-- {
		col0 = append(col0, &WillOTheWispTableauCard{Card: NewCard(CardDesignSpade, v, false), FaceUp: true})
	}
	tableau[0] = col0
	tableau[1] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 1, false), FaceUp: true}}
	for i := 2; i < WillOTheWispTableauCnt; i++ {
		tableau[i] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignClover, 1, false), FaceUp: true}}
	}
	s.SetTableau(tableau)
	s.SetStock(nil)
...
func TestWillOTheWispGameClear(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()

	s.SetCompletedSuits(WillOTheWispFoundationCnt - 1)
	var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
	col0 := make([]*WillOTheWispTableauCard, 0, CardValueMax)
	for v := CardValueMax; v >= 2; v-- {
		col0 = append(col0, &WillOTheWispTableauCard{Card: NewCard(CardDesignSpade, v, false), FaceUp: true})
	...
	require.NoError(t, s.MoveTableauToTableau(1, 0, 0))
	assert.Equal(t, WillOTheWispPhaseGameClear, s.GetPhase())
	assert.True(t, s.GetGameEndFlag())
}

func TestWillOTheWispGiveUp(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	s.GiveUp()
	assert.Equal(t, WillOTheWispPhaseGameOver, s.GetPhase())
	// No-op when not playing
	s.GiveUp()
	assert.Equal(t, WillOTheWispPhaseGameOver, s.GetPhase())
}

func TestWillOTheWispGetHint(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	...
	t.Run("finds general hint", func(t *testing.T) {
		var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
		tableau[0] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 3, false), FaceUp: true}}
		tableau[1] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 4, false), FaceUp: true}}
		for i := 2; i < WillOTheWispTableauCnt; i++ {
			tableau[i] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignClover, 1, false), FaceUp: true}}
		}
		s.SetTableau(tableau)
	...
	t.Run("skip move-to-empty when source column would become empty", func(t *testing.T) {
		...
		tableau[1] = nil
		for i := 2; i < WillOTheWispTableauCnt; i++ {
			tableau[i] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignClover, 5, false), FaceUp: true}}
		}
		s.SetTableau(tableau)
		s.SetStock(nil)
...
func TestWillOTheWispAutoComplete(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	...
	t.Run("all cards are face up from the start", func(t *testing.T) {
	...
	t.Run("removes completed suit", func(t *testing.T) {
		...
		tableau[0] = col0
		for i := 1; i < WillOTheWispTableauCnt; i++ {
			tableau[i] = nil
		}
		s.SetTableau(tableau)
		s.SetStock(nil)
...
func TestWillOTheWispUndo(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
...
func TestWillOTheWispUndo_Errors(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
...
func TestWillOTheWispUndoN(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	...
	tableau[0] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 5, false), FaceUp: true}}
	tableau[1] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 6, false), FaceUp: true}}
	tableau[2] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignSpade, 7, false), FaceUp: true}}
	for i := 3; i < WillOTheWispTableauCnt; i++ {
		tableau[i] = []*WillOTheWispTableauCard{{Card: NewCard(CardDesignClover, 1, false), FaceUp: true}}
	}
	s.SetTableau(tableau)
...
func TestWillOTheWispUndoN_Error(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	err := s.UndoN(1)
	assert.Error(t, err)
...
func TestWillOTheWispUndoToEscape(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
...
func TestWillOTheWispAllFaceUp(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	assert.True(t, s.AllFaceUp())
}

func TestWillOTheWispJSONRoundTrip(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	require.NoError(t, s.Deal())
...
func TestWillOTheWispJSONRoundTrip_WithHistory(t *testing.T) {
	s := NewDefaultWillOTheWisp()
	s.Reset()
	var tableau [WillOTheWispTableauCnt][]*WillOTheWispTableauCard
	for i := range WillOTheWispTableauCnt {
...
func TestWillOTheWispUnmarshalRejectsOversize(t *testing.T) {
	// Build a malicious payload: tableau column too long.
	overlong := make([]any, willOTheWispMaxSliceLen+1)
	for i := range overlong {
		overlong[i] = map[string]any{"f": true}
	}
	payload, err := json.Marshal(map[string]any{
		"tb": [WillOTheWispTableauCnt][]any{overlong},
	})
	require.NoError(t, err)
	var s WillOTheWisp
...
func TestWillOTheWisp_GetDealsRemaining(t *testing.T) {
	stock := func(n int) *WillOTheWisp {
		s := NewWillOTheWisp(NewTrumpCards(0))
		s.Reset()
		cards := make([]*Card, n)
		for i := range cards {
	...
	t.Run("counts a partial deal as a full one", func(t *testing.T) {
		...
		assert.Equal(t, 1, stock(WillOTheWispDealCnt-1).GetDealsRemaining())
	})

	t.Run("counts one deal per full batch", func(t *testing.T) {
		assert.Equal(t, 1, stock(WillOTheWispDealCnt).GetDealsRemaining())
		assert.Equal(t, 2, stock(WillOTheWispDealCnt+1).GetDealsRemaining())
		assert.Equal(t, 3, stock(WillOTheWispDealCnt*3).GetDealsRemaining())
	})

	// **実際に配り切るまでの回数と一致すること。**回数だけ別に数えると、
	// 表示と実際に押せる回数がずれる。
	t.Run("matches how many batches the stock actually splits into", func(t *testing.T) {
		total := WillOTheWispDealCnt*2 + 3
		s := stock(total)
		want := s.GetDealsRemaining()
		got, left := 0, total
		for left > 0 {
			left -= WillOTheWispDealCnt
			if left < 0 {
				left = 0
			}
			got++
