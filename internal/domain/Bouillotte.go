...
func (g *Bouillotte) startRound() {
	...
	g.appendLog(-1, "deal", "bouillotte.log.deal", map[string]string{"round": strconv.Itoa(g.state.roundNumber), "ante": strconv.Itoa(g.config.Ante), "pot": strconv.Itoa(g.state.pot)}, nil)
...
func (g *Bouillotte) applyCall(idx int) {
	...
	g.state.actionCount++
	g.appendLog(idx, "call", "bouillotte.log.call", map[string]string{"player": playerName(g.players, idx), "amount": strconv.Itoa(need), "pot": strconv.Itoa(g.state.pot)}, nil)
...
func (g *Bouillotte) applyRaise(idx int) {
	...
	g.state.actionCount++
	g.appendLog(idx, "raise", "bouillotte.log.raise", map[string]string{"player": playerName(g.players, idx), "newBet": strconv.Itoa(newBet), "amount": strconv.Itoa(need), "pot": strconv.Itoa(g.state.pot)}, nil)
	g.advanceOrClose(idx)
}
...
func (g *Bouillotte) applyFold(idx int) {
	...
	g.state.actionCount++
	g.appendLog(idx, "fold", "bouillotte.log.fold", map[string]string{"player": playerName(g.players, idx)}, nil)
	g.advanceOrClose(idx)
}
...
func (g *Bouillotte) endGame() {
	...
	g.appendLog(g.state.matchWinnerIdx, "game_end", "bouillotte.log.gameEnd", map[string]string{"player": playerName(g.players, g.state.matchWinnerIdx)}, nil)
}
...
func (g *Bouillotte) appendLog(playerIdx int, actionType, detailCode string, detailParams map[string]string, cards []*Card) {
