...
func (s *SevenCardStud) runCpuActions() error {
	if s.gameEndFlag {
		return nil
	}
	const maxIterations = 500
	iterations := 0
	for !s.gameEndFlag && s.phase >= SevenCardStudPhaseThirdStreet && s.phase <= SevenCardStudPhaseSeventhStreet {
		iterations++
		...
		s.cpuActions = append(s.cpuActions, SevenCardStudCpuAction{
...
func (s *SevenCardStud) handleCpuActionError(playerIdx, action int, err error) {
	...
	if callAmt > 0 {
		_ = s.executeAction(playerIdx, SevenCardStudActionFold, 0)
	} else {
		_ = s.executeAction(playerIdx, SevenCardStudActionCheck, 0)
	}
}

// cpuDecide CPUプレイヤーの意思決定
func (s *SevenCardStud) cpuDecide(idx int) (int, int) {
	p := s.players[idx]
	...
	if style == HoldemStyleGTO {
		...
		if maxRaises > 0 && s.raiseCount >= maxRaises {
			if action == SevenCardStudActionRaise || action == SevenCardStudActionBet {
				if callAmount > 0 {
					return SevenCardStudActionCall, 0
				}
				return SevenCardStudActionCheck, 0
			}
		}
	...
	if s.config.CpuMetaAI && s.humanProfile != nil && s.lastHumanPlayMs > 0 {
		if action == SevenCardStudActionFold && callAmount > 0 {
			...
			if adjustedCall > 0 && rand.Float64() < adjustedCall { //nolint:gosec // non-crypto random for game AI
				action = SevenCardStudActionCall
				amount = 0
			}
		}
	}
...
func (s *SevenCardStud) cpuDecideThirdStreet(idx int, params cpuStyleParams, callAmount int) (int, int) {
	...
	if strength < params.preFlopFoldThreshold {
		if params.preFlopFoldCompound {
			if callAmount > s.config.SmallBet*params.preFlopFoldCallMult {
				return SevenCardStudActionFold, 0
			}
		} else {
	...
	if rand.Intn(100) < params.bluffRate { //nolint:gosec
		...
		if betAmt > p.GetChips() {
			return SevenCardStudActionAllIn, 0
		}
		return SevenCardStudActionBet, betAmt
	}
	return SevenCardStudActionCheck, 0
}
...
func (s *SevenCardStud) cpuDecideThirdStreetGTO(idx, callAmount int) (int, int) {
	p := s.players[idx]
	strength := s.evalThirdStreetStrength(idx)
...
func (s *SevenCardStud) cpuPotBet(potPct int) int {
