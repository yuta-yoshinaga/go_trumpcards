...
func TestOmaha_CpuDecidePostFlopGTO_PairedBoard(t *testing.T) {
	t.Run("paired board suppression branch", func(t *testing.T) {
		// Run many iterations to statistically guarantee the paired board suppression
		// branch is hit (bt.paired && decision==2 && category<=gtoHandMedium && rand<30)
		o := newInternalTestOmaha()
		o.phase = OmahaPhaseFlop
		o.lastBet = 0
		o.pot = 50
		...
		gtoPlayer := NewOmahaPlayer(false, HoldemStyleGTO)
		gtoPlayer.SetChips(1000)
		// Weak/medium hand — two pair level
		gtoPlayer.AddCard(NewCard(CardDesignSpade, 9, false))
		gtoPlayer.AddCard(NewCard(CardDesignHeart, 10, false))
		gtoPlayer.AddCard(NewCard(CardDesignClover, 9, false))
		gtoPlayer.AddCard(NewCard(CardDesignDiamond, 4, false))
		o.players[1] = gtoPlayer

		// Run 1000 times without breaking to ensure coverage of all random branches
		for attempt := 0; attempt < 1000; attempt++ {
			o.cpuDecidePostFlopGTO(1, 0)
		}
	})
}

func TestOmaha_CpuDecidePostFlopGTO_HighCardBoard(t *testing.T) {
	t.Run("high card board suppression branch", func(t *testing.T) {
		// Run many iterations to statistically guarantee the high card board suppression
		// branch is hit (bt.highCards>=3 && decision==2 && category<=gtoHandWeak && rand<40)
		o := newInternalTestOmaha()
		o.phase = OmahaPhaseFlop
		o.lastBet = 0
		o.pot = 50
		...
		gtoPlayer := NewOmahaPlayer(false, HoldemStyleGTO)
		gtoPlayer.SetChips(1000)
		// Weak hand (high card or one pair)
		gtoPlayer.AddCard(NewCard(CardDesignDiamond, 2, false))
		gtoPlayer.AddCard(NewCard(CardDesignSpade, 3, false))
		gtoPlayer.AddCard(NewCard(CardDesignHeart, 5, false))
		gtoPlayer.AddCard(NewCard(CardDesignClover, 6, false))
		o.players[1] = gtoPlayer
...
func TestOmaha_RunCpuActions_GameEndDuringLoop(t *testing.T) {
	// Set up a scenario where all CPUs fold during runCpuActions, triggering gameEndFlag.
	// Post-flop decision is based on EvalBestHand, not preflop strength.
	// Give CPUs HighCard only hands with a huge callAmount so all styles fold.
	o := newInternalTestOmaha()
	o.phase = OmahaPhaseFlop
	o.lastBet = 500 // Very high bet forces folds
	o.pot = 1000
	o.minRaise = 10
	o.config.BigBlind = 10
	o.actedFlags = make([]bool, 4)
	o.actedFlags[0] = true // human already acted
	o.startingChips = []int{1000, 1000, 1000, 1000}
	// Community: high cards that don't pair with hole cards
	o.communityCards = []*Card{
		NewCard(CardDesignSpade, 1, false),
		NewCard(CardDesignHeart, 13, false),
		NewCard(CardDesignClover, 12, false),
	...
	// CPU hole cards: low values that don't match community → HighCard only
	// TAG: aggressive. HighCard < TwoPair → no raise. bluffRate=15% → sometimes bluff.
	//   fallbackFold=true: condCallRank=OnePair (1). HighCard(0) < OnePair(1) → foldOrCheck(500) → fold
	// LAP: passive. HighCard(0) <= HighCard(0) → postFlopPassFoldMult=3, 500 > 10*3=30 → fold
	// LAG: aggressive. HighCard < OnePair → no raise.
	//   fallbackFold=false: callAmt=500>0, HighCard<=HighCard, 500>10*4=40 → fold
	// bluffRate may prevent fold sometimes, but with high callAmount the fold path dominates
	hitGameEnd := false
	for attempt := 0; attempt < 100; attempt++ {
		o2 := newInternalTestOmaha()
		o2.phase = OmahaPhaseFlop
		o2.lastBet = 500
		o2.pot = 1000
		o2.minRaise = 10
		o2.config.BigBlind = 10
		o2.actedFlags = make([]bool, 4)
		o2.actedFlags[0] = true
		o2.startingChips = []int{1000, 1000, 1000, 1000}
		o2.communityCards = []*Card{
			NewCard(CardDesignSpade, 1, false),
			NewCard(CardDesignHeart, 13, false),
			NewCard(CardDesignClover, 12, false),
		}
		for i := 1; i < 4; i++ {
			o2.players[i].Reset()
			o2.players[i].AddCard(NewCard(CardDesignDiamond, 2, false))
			o2.players[i].AddCard(NewCard(CardDesignClover, 4, false))
			o2.players[i].AddCard(NewCard(CardDesignHeart, 6, false))
			o2.players[i].AddCard(NewCard(CardDesignSpade, 8, false))
		}
		o2.players[0].Reset()
		o2.players[0].AddCard(NewCard(CardDesignSpade, 10, false))
		o2.players[0].AddCard(NewCard(CardDesignHeart, 11, false))
		o2.players[0].AddCard(NewCard(CardDesignClover, 9, false))
		o2.players[0].AddCard(NewCard(CardDesignDiamond, 7, false))
		o2.currentTurn = 1

		err := o2.runCpuActions()
		assert.NoError(t, err)
		if o2.gameEndFlag {
			hitGameEnd = true
			break
	...
	assert.True(t, hitGameEnd, "all CPUs should fold eventually")
}
...
func TestOmaha_CpuDecidePreFlopGTO_AllBranches(t *testing.T) {
	t.Run("GTO preflop fold branch", func(t *testing.T) {
		hitFold := false
		for attempt := 0; attempt < 1000; attempt++ {
			o := newInternalTestOmaha()
			o.phase = OmahaPhasePreFlop
			...
			gtoPlayer := NewOmahaPlayer(false, HoldemStyleGTO)
			gtoPlayer.SetChips(1000)
			// Weak hand → trash/weak GTO category
			gtoPlayer.AddCard(NewCard(CardDesignSpade, 2, false))
			gtoPlayer.AddCard(NewCard(CardDesignHeart, 4, false))
			gtoPlayer.AddCard(NewCard(CardDesignClover, 7, false))
			gtoPlayer.AddCard(NewCard(CardDesignDiamond, 3, false))
			o.players[1] = gtoPlayer

			action, _ := o.cpuDecidePreFlopGTO(1, 50)
			if action == OmahaActionFold {
				hitFold = true
				break
			}
		}
		assert.True(t, hitFold, "GTO should sometimes fold weak hand preflop")
	})

	t.Run("GTO preflop raise branch", func(t *testing.T) {
		hitRaise := false
		for attempt := 0; attempt < 1000; attempt++ {
			o := newInternalTestOmaha()
			o.phase = OmahaPhasePreFlop
			...
			action, _ := o.cpuDecidePreFlopGTO(1, 10)
			if action == OmahaActionRaise || action == OmahaActionBet {
				hitRaise = true
				break
			}
		}
		assert.True(t, hitRaise, "GTO should sometimes raise strong hand preflop")
	})
}

func TestOmaha_CpuDecidePostFlopGTO_FoldBranch(t *testing.T) {
	t.Run("GTO postflop fold", func(t *testing.T) {
		hitFold := false
		for attempt := 0; attempt < 1000; attempt++ {
			o := newInternalTestOmaha()
			o.phase = OmahaPhaseFlop
			...
			gtoPlayer.SetChips(1000)
			gtoPlayer.AddCard(NewCard(CardDesignDiamond, 3, false))
			gtoPlayer.AddCard(NewCard(CardDesignSpade, 5, false))
			gtoPlayer.AddCard(NewCard(CardDesignHeart, 4, false))
			gtoPlayer.AddCard(NewCard(CardDesignClover, 6, false))
			o.players[1] = gtoPlayer

			action, _ := o.cpuDecidePostFlopGTO(1, 50)
			if action == OmahaActionFold {
				hitFold = true
				break
			}
		}
		assert.True(t, hitFold, "GTO should sometimes fold weak hand postflop")
	})
}
...
func TestOmaha_CpuDecidePostFlopGTO_BetRaiseBranch(t *testing.T) {
	t.Run("GTO postflop bet/raise branch", func(t *testing.T) {
		hitBetRaise := false
		for attempt := 0; attempt < 1000; attempt++ {
			o := newInternalTestOmaha()
			o.phase = OmahaPhaseFlop
			...
			gtoPlayer := NewOmahaPlayer(false, HoldemStyleGTO)
			gtoPlayer.SetChips(1000)
			// Strong hand that should trigger bet/raise
			gtoPlayer.AddCard(NewCard(CardDesignSpade, 1, false))
			gtoPlayer.AddCard(NewCard(CardDesignHeart, 1, false))
			gtoPlayer.AddCard(NewCard(CardDesignClover, 13, false))
			gtoPlayer.AddCard(NewCard(CardDesignDiamond, 13, false))
			...
			action, _ := o.cpuDecidePostFlopGTO(1, 0)
			if action == OmahaActionBet || action == OmahaActionRaise {
				hitBetRaise = true
				break
			}
		}
		assert.True(t, hitBetRaise, "GTO should sometimes bet/raise strong hand postflop")
	})
}

func TestOmaha_CpuDecide_GTO_MaxBetAmountCap(t *testing.T) {
	// Specifically test the GTO maxBetAmount > 0 && amount > maxBetAmount branch
	t.Run("GTO bet capped by pot limit", func(t *testing.T) {
		hitCapped := false
		for attempt := 0; attempt < 1000; attempt++ {
			o := newInternalTestOmaha()
			o.phase = OmahaPhaseFlop
		...
		assert.True(t, hitCapped, "GTO bet should be capped by pot limit")
	})
}
