...
// WhistGame ホイストゲームインタフェース
type WhistGame interface {
	BaseGame
	// Reset ゲームを初期化する
	Reset()
	...
	SetConfig(cfg domain.WhistConfig)

	// GetGameEndFlag ゲーム終了フラグを取得する
	GetGameEndFlag() bool
	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.WhistPhase
	...
	GetPlayer(i int) *domain.WhistPlayer
	// GetHint ヒントを取得する
	GetHint() *domain.WhistHint
}
