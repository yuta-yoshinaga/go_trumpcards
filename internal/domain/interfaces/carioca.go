...
// CariocaGame カリオカゲームインタフェース
type CariocaGame interface {
	BaseGame
	// Reset ゲームを初期化する
	Reset()
	...
	SetConfig(cfg domain.CariocaConfig)

	// GetGameEndFlag ゲーム終了フラグを取得する
	GetGameEndFlag() bool
	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.CariocaPhase
	...
	GetPlayer(i int) *domain.CariocaPlayer
