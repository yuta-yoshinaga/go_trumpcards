...
// EasthavenGame イーストヘイブンゲームインタフェース
type EasthavenGame interface {
	SolitaireGame
	...
	// GetHint ヒントを取得する
	GetHint() *domain.EasthavenHint
	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.EasthavenPhase
	// GetMoveCount 移動回数を取得する
	GetMoveCount() int
	// GetStockCount 山札の残り枚数を取得する
	GetStockCount() int
	// GetTableau タブローを取得する
	GetTableau() [domain.EasthavenTableauCnt][]*domain.KlondikeTableauCard
	// GetFoundation ファンデーションを取得する
	GetFoundation() [domain.EasthavenFoundationCnt][]*domain.Card
