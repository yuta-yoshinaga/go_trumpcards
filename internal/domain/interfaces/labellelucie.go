...
// LaBelleLucieGame ラ・ベル・ルーシー (ファン・ソリティア) のゲームインタフェース。
type LaBelleLucieGame interface {
	BaseGame
	...
	UndoN(n int) error
	// GetHint ヒントを取得する。
	GetHint() *domain.LaBelleLucieHint

	// GetPhase 現在のフェーズを取得する。
	GetPhase() domain.LaBelleLuciePhase
	...
	GetFoundation() [domain.LaBelleLucieFoundationCnt][]*domain.Card
}
