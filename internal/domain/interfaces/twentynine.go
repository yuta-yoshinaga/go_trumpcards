...
// TwentyNineGame トゥエンティナイン (29) のゲームインタフェース
type TwentyNineGame interface {
	BaseGame
	// Reset ゲームを初期化する
	Reset()
	...
	PlayerBid(bid domain.TwentyNineBid) error
	...
	SetConfig(cfg domain.TwentyNineConfig)

	// GetGameEndFlag ゲーム終了フラグを取得する
	GetGameEndFlag() bool
	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.TwentyNinePhase
	...
	GetCurrentTrick() []*domain.TwentyNineTrickCard
	...
	GetContract() domain.TwentyNineBid
	// GetBids 各プレイヤーの入札を取得する
	GetBids() [domain.TwentyNinePlayerCnt]domain.TwentyNineBid
