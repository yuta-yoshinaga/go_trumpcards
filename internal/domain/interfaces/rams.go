...
// RamsGame ラムス (Rams) ゲームインタフェース
type RamsGame interface {
	BaseGame
	// Reset ゲームを初期化する
	Reset()
	...
	SetConfig(cfg domain.RamsConfig)

	// GetGameEndFlag ゲーム終了フラグを取得する
	GetGameEndFlag() bool
	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.RamsPhase
	...
	GetPlayer(i int) *domain.RamsPlayer
	// GetWinnerIdx 勝者プレイヤーインデックスを取得する (-1: 未確定/同点)
	GetWinnerIdx() int
	// GetHint ヒントを取得する
	GetHint() *domain.RamsHint
}
