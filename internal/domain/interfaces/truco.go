...
// TrucoGame トゥルコゲームインタフェース
type TrucoGame interface {
	BaseGame
	// Reset マッチを初期化する
	Reset()
	// PlayerPlay 人間プレイヤーがカードを出す
	PlayerPlay(cardIndex int) error
	// DeclareTruco 人間プレイヤーが Truco を宣言 (または再引き上げ) する
	DeclareTruco() error
	// RespondTruco 人間プレイヤーが Truco 宣言に応答する (true=受諾 / false=拒否)
	RespondTruco(accept bool) error
	...
	SetConfig(cfg domain.TrucoConfig)

	// GetGameEndFlag マッチ終了フラグを取得する
	GetGameEndFlag() bool
	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.TrucoPhase
	// IsHumanTurn 現在の手番 (プレイまたは応答) が人間かを返す
	IsHumanTurn() bool
	// CanDeclareTruco 現在の手番の人間が Truco 宣言可能かを返す
	CanDeclareTruco() bool
	...
	GetPendingLevel() int
	// GetTrucoCallerIdx 応答待ちの宣言者インデックスを取得する (-1: なし)
	GetTrucoCallerIdx() int
	...
	GetPlayer(i int) *domain.TrucoPlayer
	// GetValidPlayIndices プレイ可能なカードのインデックスリストを返す
	GetValidPlayIndices(playerIdx int) []int
	// GetHint ヒントを取得する
	GetHint() *domain.TrucoHint
}
