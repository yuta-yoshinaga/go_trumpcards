...
// BurracoGame ブラーコゲームインタフェース
type BurracoGame interface {
	BaseGame
	// Reset ゲームを初期化する
	Reset()
	...
	SetConfig(cfg domain.BurracoConfig)

	// GetGameEndFlag ゲーム終了フラグを取得する
	GetGameEndFlag() bool
	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.BurracoPhase
	...
	// GetPozzettoCount 残っているポゼット（予備手札）の山の数を取得する
	...
	GetPlayer(i int) *domain.BurracoPlayer
