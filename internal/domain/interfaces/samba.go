...
// SambaGame サンバゲームインタフェース
type SambaGame interface {
	BaseGame
	// Reset ゲームを初期化する
	Reset()
	...
	SetConfig(cfg domain.SambaConfig)

	// GetGameEndFlag ゲーム終了フラグを取得する
	GetGameEndFlag() bool
	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.SambaPhase
	...
	GetPlayer(i int) *domain.SambaPlayer
	// GetTeamCount チーム数を取得する
	GetTeamCount() int
	// GetTeamScore チームの累積スコアを取得する
	GetTeamScore(team int) int
	// GetTeamCompletedMeldCount チームの完成カナスタ/サンバ数を取得する
	GetTeamCompletedMeldCount(team int) int
