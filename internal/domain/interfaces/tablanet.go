...
// TablanetGame はタブラネット (Tablanet / Bastra) のゲームインタフェース。
type TablanetGame interface {
	BaseGame
	// Reset ゲームを初期化する
	Reset()
	...
	SetConfig(cfg domain.TablanetConfig)

	// GetGameEndFlag ゲーム終了フラグを取得する
	GetGameEndFlag() bool
	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.TablanetPhase
	...
	GetLastDealDetail() *domain.TablanetScoreDetail
	// GetPlayerCnt プレイヤー数を取得する
	GetPlayerCnt() int
	// GetPlayer 指定インデックスのプレイヤーを取得する
	GetPlayer(i int) *domain.TablanetPlayer
