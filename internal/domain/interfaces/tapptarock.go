...
// TappTarockGame タップ・タロック (TappTarock) のゲームインタフェース
type TappTarockGame interface {
	BaseGame
	// Reset ゲームを初期化する
	Reset()
	...
	PlayerBid(bid domain.TappTarockBid) error
	...
	SetConfig(cfg domain.TappTarockConfig)

	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.TappTarockPhase
	...
	GetOutcome() domain.TappTarockOutcome
	// GetBreakdown 直近ディールの精算内訳を取得する
	GetBreakdown() *domain.TappTarockBreakdown
	// GetPlayerCnt 席数を取得する
	GetPlayerCnt() int
	// GetPlayer 指定した席を取得する
	GetPlayer(i int) *domain.TappTarockPlayer
