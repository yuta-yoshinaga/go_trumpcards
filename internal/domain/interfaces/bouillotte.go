...
// BouillotteGame はブイヨット (Bouillotte) のゲームインタフェース。
type BouillotteGame interface {
	BaseGame
	...
	SetConfig(cfg domain.BouillotteConfig)

	// GetPhase 現在のフェーズを取得する
	GetPhase() domain.BouillottePhase
	...
	// GetAnte アンティ額を取得する
	GetAnte() int
	// GetRetourne 共有カード (retourne) を取得する (未配札時は nil)
	GetRetourne() *domain.Card
	// GetWinnerIdx 直近ラウンドの勝者を取得する (-1 = なし)
	GetWinnerIdx() int
	// GetMatchWinnerIdx ゲーム全体の勝者を取得する (-1 = 未確定)
	GetMatchWinnerIdx() int
	// GetResult 人間から見たラウンド結果を取得する
	GetResult() domain.BouillotteResult
	// GetPlayerCnt プレイヤー数を取得する
	GetPlayerCnt() int
	// GetPlayer 指定インデックスのプレイヤーを取得する
	GetPlayer(i int) *domain.BouillottePlayer
