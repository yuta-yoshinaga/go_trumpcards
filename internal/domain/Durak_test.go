...
func TestDurak_BoutLimitEndsTheGame(t *testing.T) {
	...
	t.Run("a normal game finishes well inside the limit", func(t *testing.T) {
		...
		for range games {
			d.Reset()
			turns := 0
			// **打ち切り局は 500 手では終わらない。** 上限は 200 バウトで、
			// 1 バウトは複数手かかるので、その 200 バウトに到達した局は
			// 500 を超えることがある ── そこで `require` が先に落ち、
			// 「上限に達した局もちゃんと終局している」という、この subtest が
			// 見たかったはずの事象そのものが失敗として報告されていた。
			//
			// 実測 (2026-08-31、20 万局): 500 手を超えたのは 1 局、最長は 800 手、
			// 上限に達したのは 14 局。つまり 200 局ループあたり約 0.1%、
			// おおよそ 1000 回に 1 回この require が落ちる。
			// 2000 は観測最大 800 の 2.5 倍で、普通の揺れでは届かず、
			// 「終わらない局」という本物の退行は依然として捕まえられる。
			for !d.GetGameEndFlag() && turns < 2000 {
				d.CpuPlay()
				turns++
			}
