...
func TestBriscolaPlayer_New(t *testing.T) {
	p := domain.NewBriscolaPlayer(true)
	if !p.GetIsHuman() {
	...
	p2 := domain.NewBriscolaPlayer(false)
	if p2.GetIsHuman() {
		t.Error("expected non-human player")
	}
}

func TestBriscolaPlayer_ResetGame(t *testing.T) {
	p := domain.NewBriscolaPlayer(true)
...
func TestBriscolaPlayer_JSONRoundtrip(t *testing.T) {
	p := domain.NewBriscolaPlayer(true)
