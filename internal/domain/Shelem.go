...
// ShelemMinBid 競りの最低入札
const ShelemMinBid = 100

// ShelemMaxBid 競りの上限（1 ラウンドのカード点の合計）
const ShelemMaxBid = 165
...
// で、トリックは 13 ではなく **12**。
//
// **競るのはトリック数ではなく点数そのもの。** 100 から 5 刻みで競り上げ、
// 落札者はウィドウ 4 枚を取り込んで 4 枚捨て、切り札を決める。落札した点数を
// カード点で取り切れれば加点、届かなければ同額を失点する。
//
// **点になるのは 3 ランクだけ。** A と 10 が 10 点、5 が 5 点で、1 ラウンドの
// 合計はちょうど 100 点。だから入札の下限が 100 で、それを超える入札は
// 「相手にも点を取らせない」宣言に近づく。
//
