...
func makeFiveCardStudForPresenter() (*domain.FiveCardStud, []*domain.FiveCardStudPlayer) {
	tc := domain.NewTrumpCards(0)
	players := []*domain.FiveCardStudPlayer{
		domain.NewFiveCardStudPlayer(true, domain.HoldemStyleTAG),
		domain.NewFiveCardStudPlayer(false, domain.HoldemStyleLAP),
		domain.NewFiveCardStudPlayer(false, domain.HoldemStyleTAP),
		domain.NewFiveCardStudPlayer(false, domain.HoldemStyleGTO),
	}
...
func TestFiveCardStudWebPresenter_Output(t *testing.T) {
	p := new(presenter.FiveCardStudWebPresenter)

	setup := func() (*domain.FiveCardStud, []*domain.FiveCardStudPlayer) {
		return makeFiveCardStudForPresenter()
	}
	...
	t.Run("initial state", func(t *testing.T) {
		...
		assert.Equal(t, domain.FiveCardStudPhaseSecondStreet, out.Phase)
	...
	t.Run("human hole cards visible", func(t *testing.T) {
		...
		assert.Len(t, human.HoleCards, 1)
	...
	t.Run("community card present when card shortage", func(t *testing.T) {
		s, _ := setup()
		s.SetPhase(domain.FiveCardStudPhaseFifthStreet)
	...
	t.Run("CPU actions", func(t *testing.T) {
		...
		s.SetCpuActions([]domain.FiveCardStudCpuAction{
			{PlayerIdx: 1, Action: domain.FiveCardStudActionCall, Amount: 10},
			{PlayerIdx: 2, Action: domain.FiveCardStudActionFold, Amount: 0},
		})
		...
		assert.Equal(t, domain.FiveCardStudActionCall, out.CpuActions[0].Action)
	})
