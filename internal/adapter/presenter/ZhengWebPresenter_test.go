...
func makeZhengPresenterPlayers() []*domain.ZhengPlayer {
	return []*domain.ZhengPlayer{
		domain.NewZhengPlayer(true),
		domain.NewZhengPlayer(false),
		domain.NewZhengPlayer(false),
		domain.NewZhengPlayer(false),
	}
}
...
func setupZhengWebMock() (*interfaces.MockZhengGame, []*domain.ZhengPlayer) {
	...
	m.On("GetTablePlayType").Return(domain.ZhengPlayInvalid)
...
func TestZhengWebPresenter_Output(t *testing.T) {
	...
	t.Run("initial state hides CPU cards", func(t *testing.T) {
		...
		require.Len(out.Players[0].Cards, 2) // human cards visible (incl. joker)
		require.Len(out.Players[1].Cards, 0) // CPU cards hidden
		require.Equal(1, out.Players[1].CardCount)
		require.Equal(0, out.CurrentTurn)
		require.Equal(-1, out.LastPlayPlayerIdx)
		require.Equal(int(domain.ZhengPlayInvalid), out.TablePlayType)
	})
	...
	t.Run("human action included", func(t *testing.T) {
		...
		m.On("GetHumanAction").Return(&domain.ZhengAction{
	...
	t.Run("game end builds rankings", func(t *testing.T) {
		...
		assert.Equal(t, "zheng.result.rankings", out.MessageCode)
		...
		assert.Contains(t, out.MessageParams["rankings"], "CPU 1:2位")
