...
func TestQuinzeWebPresenter_Output(t *testing.T) {
	...
	t.Run("keeps the round result after replenishment", func(t *testing.T) {
		...
		result := parseQuinzeOutput(t, new(QuinzeWebPresenter).Output(g, nil))
		assert.Equal(t, "quinze.log.result", result.MessageCode)
	...
	t.Run("round over", func(t *testing.T) {
		...
		g.On("GetLastResultCode").Return("quinze.log.result")
		g.On("GetLastResultParams").Return(map[string]string{"points": "6.5"})
	...
	t.Run("the bank passing has its own message", func(t *testing.T) {
		...
		g.On("GetLastResultCode").Return("quinze.log.resultBust")
		g.On("GetLastResultParams").Return(map[string]string{"points": "18"})

		result := parseQuinzeOutput(t, new(QuinzeWebPresenter).Output(g, nil))
		assert.Equal(t, "quinze.bankPasses", result.MessageCode)
		assert.Equal(t, "quinze.log.resultBust", result.MessageParams["resultKey"])
		assert.Equal(t, "18", result.MessageParams["points"])
	})
}
