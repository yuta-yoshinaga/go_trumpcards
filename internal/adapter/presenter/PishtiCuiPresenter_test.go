...
// pishtiSetField は JSON 経由で Pishti の内部状態を差し替える (テスト用)。
func pishtiSetField(g *domain.Pishti, fields map[string]any) {
...
// 場のトップと自分の手札を照合させていた。
func TestPishtiCuiPresenter_MarksCapturingCards(t *testing.T) {
	p := new(presenter.PishtiCuiPresenter)
	card := func(d, v int) *domain.Card { return domain.NewCard(d, v, false) }

	seed := func(hand []*domain.Card, pileTop *domain.Card, turn int) *domain.Pishti {
		g := newPishtiForPresenter()
		human := g.GetPlayer(0)
		human.Reset()
		for _, c := range hand {
