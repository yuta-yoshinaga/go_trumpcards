...
func setupBoliviaCuiMock() *interfaces.MockBoliviaGame {
...
func setupBoliviaCuiMockWithPlayers() (*interfaces.MockBoliviaGame, []*domain.BoliviaPlayer) {
...
func TestBoliviaCuiPresenter_Output(t *testing.T) {
	...
	t.Run("frozen tag and frozen draw help", func(t *testing.T) {
		...
		assert.Contains(t, result, i18n.T("bolivia.promptDrawHelpFrozen"))
	})
	...
	// 「サンバ」と呼ぶが、ボリビアでその名前が付くのはワイルド 7 枚のほう。
	t.Run("a seven-card sequence is labelled an escalera, not a bolivia", func(t *testing.T) {
		m, players := setupBoliviaCuiMockWithPlayers()
		cards := []*domain.Card{
		...
		players[0].AddMeld(&domain.BoliviaMeld{Cards: cards, Kind: domain.BoliviaMeldEscalera, IsNatural: true})
		result := p.Output(m, nil)
		assert.Contains(t, result, "シーケンス")
		assert.Contains(t, result, "エスカレラ")
		assert.NotContains(t, result, "＝ボリビア", "エスカレラをボリビアと呼んでいる")
		assert.Contains(t, result, "★エスカレラ")
	})
	...
	t.Run("set meld shows canasta label", func(t *testing.T) {
		...
		players[0].AddMeld(&domain.BoliviaMeld{Cards: cards, Kind: domain.BoliviaMeldSet, IsNatural: true})
		result := p.Output(m, nil)
	...
	t.Run("meld phase commands", func(t *testing.T) {
		...
		m.On("GetPhase").Return(domain.BoliviaPhaseMeld)
		result := p.Output(m, nil)
...
func TestBoliviaCuiPresenter_ActionLogOutput(t *testing.T) {
	origNoColor := color.NoColor()
	color.SetNoColor(true)
	defer color.SetNoColor(origNoColor)
	p := new(presenter.BoliviaCuiPresenter)
...
func TestBoliviaCuiPresenter_ShowsTheInitialMeldRequirement(t *testing.T) {
	origNoColor := color.NoColor()
	color.SetNoColor(true)
	defer color.SetNoColor(origNoColor)
	p := new(presenter.BoliviaCuiPresenter)

	meldTurn := func(minValue int, melded bool) *interfaces.MockBoliviaGame {
		m, players := setupBoliviaCuiMockWithPlayers()
		m.ExpectedCalls = removeMockCall(m.ExpectedCalls, "GetPhase")
		m.On("GetPhase").Return(domain.BoliviaPhaseMeld)
	...
	for _, minValue := range []int{15, 50, 90, 120} {
		...
		assert.Contains(t, out, i18n.Tf("bolivia.promptMeldMinimum",
