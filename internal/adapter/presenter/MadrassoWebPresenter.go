...
// MadrassoWebPresenter マドラッソのWebプレゼンタークラス
type MadrassoWebPresenter struct{}

// Output ゲーム状態をJSON出力
func (p *MadrassoWebPresenter) Output(g interfaces.MadrassoGame, lastErr error) string {
	...
	//
	// **フェーズと手番はここでは見ない。**Madrasso.GetHint() が自分で
...
func (p *MadrassoWebPresenter) buildBase(g interfaces.MadrassoGame) *controller.MadrassoWebOutput {
	resObj := new(controller.MadrassoWebOutput)
	...
	resObj.TeamRoundPoints = pts[:]
	resObj.TrumpSuit = g.GetTrumpSuit()
	resObj.PlayableIndices = p.playableIndices(g)

	cfg := g.GetConfig()
	resObj.Config = controller.MadrassoWebOutputConfig{
...
func (p *MadrassoWebPresenter) buildLastTrickOutput(g interfaces.MadrassoGame) ([]*controller.WebOutputTrickCard, int) {
	empty := make([]*controller.WebOutputTrickCard, 0)
	// ラウンド最初のトリックがプレイ中は、この局に確定済みトリックが無いため空を返す。
	if g.GetPhase() == domain.MadrassoPhasePlay && g.GetTrickNumber() <= 1 {
		return empty, -1
	}
	...
	if len(plays) < domain.MadrassoPlayerCnt {
		return empty, -1
	}
	plays = plays[len(plays)-domain.MadrassoPlayerCnt:]
...
func (p *MadrassoWebPresenter) playableIndices(g interfaces.MadrassoGame) []int {
	if g.GetPhase() != domain.MadrassoPhasePlay || !g.IsHumanTurn() {
		return make([]int, 0)
	}
...
func (p *MadrassoWebPresenter) buildPlayersOutput(g interfaces.MadrassoGame) []*controller.MadrassoWebOutputPlayer {
	out := make([]*controller.MadrassoWebOutputPlayer, 0)
	for i := 0; i < g.GetPlayerCnt(); i++ {
		player := g.GetPlayer(i)
		if player == nil {
			continue
		}
		out = append(out, &controller.MadrassoWebOutputPlayer{
			ID:         i,
...
func (p *MadrassoWebPresenter) buildMessage(g interfaces.MadrassoGame, lastErr error) (string, string, map[string]string) {
	...
	case domain.MadrassoPhasePlay:
		if len(g.GetCurrentTrick()) == 0 {
			return "", "madrasso.playPhase.lead", nil
		}
		return "", "madrasso.playPhase.follow", nil
...
func (p *MadrassoWebPresenter) winnerMessage(g interfaces.MadrassoGame) (string, string, map[string]string) {
	winnerTeam := g.GetWinnerTeam()
	humanTeam := 0
	for i := 0; i < g.GetPlayerCnt(); i++ {
		if player := g.GetPlayer(i); player != nil && player.GetIsHuman() {
			humanTeam = domain.MadrassoTeamOf(i)
			break
		}
	}
	teamLabel := "A"
	...
	if winnerTeam == humanTeam {
		return fmt.Sprintf("ゲーム終了！ チーム%sの勝ち！", teamLabel), "madrasso.result.humanTeamWin", params
	}
	return fmt.Sprintf("ゲーム終了！ チーム%sの勝ち！", teamLabel), "madrasso.result.cpuTeamWin", params
}

// HintOutput ヒント情報をJSON出力する
func (p *MadrassoWebPresenter) HintOutput(g interfaces.MadrassoGame) string {
	hint := g.GetHint()
	resObj := p.buildBase(g)
	if hint != nil {
		resObj.Hint = cardHint(hint.CardIndices, hint.Reason)
	...
	if hint != nil {
		resObj.MessageCode = "madrasso.hintRequested"
	} else {
		resObj.MessageCode = "madrasso.noHint"
	}
	return marshalOrError(resObj)
}
