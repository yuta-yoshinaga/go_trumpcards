...
func setupCanastaWebMock() *interfaces.MockCanastaGame {
	...
	m.On("GetConfig").Return(domain.DefaultCanastaConfig())
	m.On("GetActionLog").Return(([]*domain.ActionLogEntry)(nil))
	return m
}

func makeCanastaPlayers() []*domain.CanastaPlayer {
	return []*domain.CanastaPlayer{
		domain.NewCanastaPlayer(true),
		domain.NewCanastaPlayer(false),
	}
}

func setupCanastaWebMockWithPlayers() (*interfaces.MockCanastaGame, []*domain.CanastaPlayer) {
	m := setupCanastaWebMock()
...
func TestCanastaWebPresenter_Output(t *testing.T) {
	...
	t.Run("initial state", func(t *testing.T) {
		m, players := setupCanastaWebMockWithPlayers()
	...
	t.Run("CPU cards shown in game end phase", func(t *testing.T) {
		...
		m.On("GetPhase").Return(domain.CanastaPhaseGameEnd)
	...
	t.Run("config values", func(t *testing.T) {
		...
		m.On("GetConfig").Return(domain.CanastaConfig{
			CpuDifficulty: domain.CanastaCpuDifficultyHard,
			PointLimit:    7500,
		})
		...
		assert.Equal(t, int(domain.CanastaCpuDifficultyHard), resObj.Config.CpuDifficulty)
	...
	t.Run("game end human wins", func(t *testing.T) {
		...
		assert.Equal(t, "canasta.result.humanWin", resObj.MessageCode)
	...
	t.Run("game end CPU wins", func(t *testing.T) {
		...
		assert.Equal(t, "canasta.result.cpuWin", resObj.MessageCode)
	...
	t.Run("game end nil player at winnerIdx", func(t *testing.T) {
		...
		m.On("GetPlayer", 99).Return((*domain.CanastaPlayer)(nil))
		m.On("GetPhase").Return(domain.CanastaPhaseGameEnd)
	...
	t.Run("draw phase messageCode", func(t *testing.T) {
		...
		assert.Equal(t, "canasta.drawPhase", resObj.MessageCode)
	})
	...
	t.Run("meld phase messageCode", func(t *testing.T) {
		...
		assert.Equal(t, "canasta.meldPhase", resObj.MessageCode)
	})
	...
	t.Run("discard phase messageCode", func(t *testing.T) {
		...
		assert.Equal(t, "canasta.discardPhase", resObj.MessageCode)
	})
	...
	t.Run("round end messageCode", func(t *testing.T) {
		...
		assert.Equal(t, "canasta.roundEnd", resObj.MessageCode)
	})
