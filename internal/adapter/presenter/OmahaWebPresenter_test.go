...
func TestOmahaWebPresenter_Output(t *testing.T) {
	...
	t.Run("side pots", func(t *testing.T) {
		h, _ := setup()
		h.SetPhase(domain.OmahaPhaseFlop)
		h.SetSidePots([]domain.OmahaSidePot{
	...
	t.Run("getHandName out of range returns Unknown", func(t *testing.T) {
		...
		gameMock.On("GetSidePots").Return([]domain.OmahaSidePot{})
		gameMock.On("GetCpuActions").Return([]domain.OmahaCpuAction{})
		gameMock.On("GetRoundResults").Return([]domain.OmahaResult{})
