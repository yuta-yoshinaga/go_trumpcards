...
func mustRussianSolitaireOutputJSON(msg string) string {
	out := &controller.RussianSolitaireWebOutput{
	...
	if err != nil {
		panic(fmt.Sprintf("mustRussianSolitaireOutputJSON: %v", err))
	}
	return string(b)
}

func TestRussianSolitaireWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	riMock := new(usecase.MockRussianSolitaireInteractor)
	...
	factory := func() uc.RussianSolitaireInteractorIF { return riMock }
	ctrl := controller.NewRussianSolitaireWebController(factory)
	defer ctrl.Stop()

	t.Run("quit", func(t *testing.T) {
		var input controller.RussianSolitaireWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustRussianSolitaireOutputJSON("bye."))
	})
