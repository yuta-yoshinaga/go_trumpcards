...
// BasraCuiController はバスラ (Basra) の CUI コントローラークラス。
type BasraCuiController struct {
	bi usecase.BasraInteractorIF
}

// NewBasraCuiController コンストラクタ。
func NewBasraCuiController(bi usecase.BasraInteractorIF) *BasraCuiController {
	return &BasraCuiController{bi: bi}
}
...
//	h / hint                 → ヒント表示
//	log / l                  → 棋譜表示
func (c *BasraCuiController) Exec(command string) string {
