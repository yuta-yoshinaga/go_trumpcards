...
func TestPishtiWebController_Method(t *testing.T) {
	mockOutput := `{"players":[]}`
	piMock := new(usecase.MockPishtiInteractor)
	...
	factory := func() uc.PishtiInteractorIF { return piMock }
	ctrl := controller.NewPishtiWebController(factory)
	defer ctrl.Stop()

	var jsonInput controller.PishtiWebInput
	...
	t.Run("reset with config", func(t *testing.T) {
		input := controller.PishtiWebInput{
			BaseWebInput: controller.BaseWebInput{Command: "reset", SessionID: "s1"},
			Config:       &controller.PishtiWebConfig{PlayerCnt: 3, CpuDifficulty: 2},
		}
...
func TestPishtiWebConfig_ToConfig(t *testing.T) {
	wc := controller.PishtiWebConfig{PlayerCnt: 2, CpuDifficulty: 2}
	c := wc.ToConfig()
