...
func TestCanastaCuiController_Exec(t *testing.T) {
	mockOutput := `{"phase":0}`

	newMock := func() *mockUsecases.MockCanastaInteractor {
		m := new(mockUsecases.MockCanastaInteractor)
	...
	t.Run("reset command r preserves config", func(t *testing.T) {
		...
		m.AssertCalled(t, "ResetWithConfig", domain.DefaultCanastaConfig())
	})
	...
	t.Run("setdifficulty sd valid", func(t *testing.T) {
		...
		expected.CpuDifficulty = domain.CanastaCpuDifficultyHard
	...
	t.Run("setdifficulty long form", func(t *testing.T) {
		...
		assert.Equal(t, mockOutput, result)
		expected := domain.DefaultCanastaConfig()
		expected.CpuDifficulty = domain.CanastaCpuDifficultyEasy
		m.AssertCalled(t, "ResetWithConfig", expected)
	})
