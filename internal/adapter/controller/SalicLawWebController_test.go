...
func mustSalicLawOutputJSON(msg string) string {
	out := &controller.SalicLawWebOutput{
		Tableau:       [][]*controller.WebOutputCard{},
		Foundation:    [][]*controller.WebOutputCard{},
		Queens:        []*controller.WebOutputCard{},
		WebOutputBase: controller.WebOutputBase{Message: msg},
	}
	b, err := json.Marshal(out)
	if err != nil {
		panic(fmt.Sprintf("mustSalicLawOutputJSON: %v", err))
	}
	return string(b)
}

func TestSalicLawWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"stockCount":0,"queens":[],"openPiles":0,"phase":0,"moveCount":0,"message":""}`

	ciMock := new(usecase.MockSalicLawInteractor)
	...
	ctrl := controller.NewSalicLawWebController(func() uc.SalicLawInteractorIF { return ciMock })
	defer ctrl.Stop()

	exec := func(t *testing.T, body string) *recorded {
		t.Helper()
		var input controller.SalicLawWebInput
	...
	t.Run("quit q", func(t *testing.T) {
		...
		r.BodyIs(mustSalicLawOutputJSON("bye."))
	})
	...
	for _, tc := range []struct{ name, body string }{
		...
		// 捨て札も山札も移動元ではない。リネームだけして残すと、ヘルプに無い
		// ゾーンが半分動く状態になる。
		{"waste is not a zone", `{"command":"m","sessionId":"s1","from":{"zone":"waste"},"to":{"zone":"foundation"}}`},
		{"waste to tableau is not a move", `{"command":"m","sessionId":"s1","from":{"zone":"waste"},"to":{"zone":"tableau","col":2}}`},
		{"stock is not a zone", `{"command":"m","sessionId":"s1","from":{"zone":"stock"},"to":{"zone":"tableau","col":3}}`},
