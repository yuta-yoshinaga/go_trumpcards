...
func TestNapCuiController_Exec(t *testing.T) {
	mockOutput := `{"phase":0}`

	newMock := func() *mockUsecases.MockNapInteractor {
		m := new(mockUsecases.MockNapInteractor)
	...
	t.Run("quit", func(t *testing.T) {
		assert.Equal(t, "bye.", controller.NewNapCuiController(newMock()).Exec("q"))
		assert.Equal(t, "bye.", controller.NewNapCuiController(newMock()).Exec("quit"))
	})

	t.Run("reset preserves config", func(t *testing.T) {
		m := newMock()
		c := controller.NewNapCuiController(m)
		assert.Equal(t, mockOutput, c.Exec("r"))
		m.AssertCalled(t, "ResetWithConfig", domain.DefaultNapConfig())
	})

	t.Run("bid three", func(t *testing.T) {
		m := newMock()
		c := controller.NewNapCuiController(m)
	...
	t.Run("setdifficulty", func(t *testing.T) {
		...
		expected.CpuDifficulty = domain.NapCpuDifficultyHard
	...
	t.Run("unknown command", func(t *testing.T) {
		result := controller.NewNapCuiController(newMock()).Exec("zzz")
