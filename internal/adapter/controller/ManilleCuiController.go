...
// ManilleCuiController マニーユのCUIコントローラークラス
type ManilleCuiController struct {
	di usecase.ManilleInteractorIF
}

// NewManilleCuiController コンストラクタ
func NewManilleCuiController(di usecase.ManilleInteractorIF) *ManilleCuiController {
	return &ManilleCuiController{di: di}
}
...
//	h / hint                 → ヒント表示
//	log / l                  → 棋譜表示
func (c *ManilleCuiController) Exec(command string) string {
