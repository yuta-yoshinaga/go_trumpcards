...
func mustStreetsAndAlleysOutputJSON(msg string) string {
	out := &controller.StreetsAndAlleysWebOutput{
		Tableau:       [][]*controller.StreetsAndAlleysWebOutputTableauCard{},
		Foundation:    [][]*controller.WebOutputCard{},
		WebOutputBase: controller.WebOutputBase{Message: msg},
	}
	b, err := json.Marshal(out)
	if err != nil {
		panic(fmt.Sprintf("mustStreetsAndAlleysOutputJSON: %v", err))
	}
	return string(b)
}

func TestStreetsAndAlleysWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	biMock := new(usecase.MockStreetsAndAlleysInteractor)
	...
	factory := func() uc.StreetsAndAlleysInteractorIF { return biMock }
	ctrl := controller.NewStreetsAndAlleysWebController(factory)
	defer ctrl.Stop()

	t.Run("quit q", func(t *testing.T) {
		var input controller.StreetsAndAlleysWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustStreetsAndAlleysOutputJSON("bye."))
	})
