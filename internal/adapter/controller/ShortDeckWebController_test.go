...
func newShortDeckTestHandler(mi *mockUsecase.MockShortDeckInteractor) (*rest.Api, *ShortDeckWebController) {
	api := rest.NewApi()
	api.Use(rest.DefaultDevStack...)
	owc := NewShortDeckWebController(func() usecase.ShortDeckInteractorIF {
		return mi
	})
	router, _ := rest.MakeRouter(
		rest.Post("/shortdeck/exec", owc.Exec),
	)
	api.SetApp(router)
	return api, owc
}
...
func TestShortDeckWebController_Reset(t *testing.T) {
	...
	api, owc := newShortDeckTestHandler(mi)
	defer owc.Stop()
	...
	recorded := test.RunRequest(t, api.MakeHandler(),
		test.MakeSimpleRequest("POST", "http://localhost/shortdeck/exec",
...
func TestShortDeckWebController_BadRequest_EmptyBody(t *testing.T) {
	...
	defer owc.Stop()
	recorded := test.RunRequest(t, api.MakeHandler(),
		test.MakeSimpleRequest("POST", "http://localhost/shortdeck/exec", nil))
	recorded.CodeIs(400)
}
...
func TestShortDeckWebController_Stop(t *testing.T) {
	mi := new(mockUsecase.MockShortDeckInteractor)
	c := NewShortDeckWebController(func() usecase.ShortDeckInteractorIF {
		return mi
	})
	c.Stop()
	c.Stop()
