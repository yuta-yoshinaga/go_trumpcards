...
func (c *ShengJiCuiController) Exec(command string) string {
	return execCuiCommand(
		...
		func(cmd string, args []string) (string, bool) {
			...
			case "b", "bury":
				return shengJiParseIndexes(args, domain.ShengJiKittySize, c.gi.BuryKitty)
			case "p", "play":
				return shengJiParseIndexes(args, 0, c.gi.Play)
			case "n", "next":
...
func shengJiParseIndexes(args []string, want int, apply func([]int) string) (string, bool) {
	if len(args) == 0 {
	...
	maxIdx := domain.ShengJiHandSize + domain.ShengJiKittySize - 1
