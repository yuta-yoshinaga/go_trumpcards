...
// TonkCuiController Tonk CUIコントローラークラス
type TonkCuiController struct {
	ci usecase.TonkInteractorIF
}

// NewTonkCuiController コンストラクタ
func NewTonkCuiController(ci usecase.TonkInteractorIF) *TonkCuiController {
	return &TonkCuiController{ci: ci}
}

// Exec コマンド実行
func (c *TonkCuiController) Exec(command string) string {
