...
func TestPinochleWebController_Commands(t *testing.T) {
	...
	piMock := new(usecase.MockPinochleInteractor)
	piMock.On("ResetWithConfig", domain.DefaultPinochleConfig()).Return(mockOutput)
	piMock.On("Bid", 25).Return(mockOutput)
	...
	factory := func() uc.PinochleInteractorIF { return piMock }
	ctrl := controller.NewPinochleWebController(factory)
	defer ctrl.Stop()
	...
	tests := []struct {
		...
		{"bid", `{"command":"b","bidAmount":25,"sessionId":"s1"}`, http.StatusOK},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var input controller.PinochleWebInput
...
func TestPinochleWebConfig_ToConfig(t *testing.T) {
	t.Run("default config", func(t *testing.T) {
		input := controller.PinochleWebInput{}
		cfg := input.ToConfig()
	...
	t.Run("custom config", func(t *testing.T) {
		...
		limit := 3000
		input := controller.PinochleWebInput{
			Config: &controller.PinochleWebConfig{
				CpuDifficulty: &diff,
				PointLimit:    &limit,
			},
		}
