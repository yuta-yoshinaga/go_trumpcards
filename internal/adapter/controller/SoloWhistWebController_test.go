...
func mustSoloWhistOutputJSON(msg string) string {
	...
	if err != nil {
		panic(fmt.Sprintf("mustSoloWhistOutputJSON: %v", err))
	}
	return string(b)
}

func TestSoloWhistWebController_Method(t *testing.T) {
	mockOutput := `{"phase":0}`

	diMock := new(usecase.MockSoloWhistInteractor)
	diMock.On("ResetWithConfig", domain.DefaultSoloWhistConfig()).Return(mockOutput)
	...
	factory := func() uc.SoloWhistInteractorIF { return diMock }
	ctrl := controller.NewSoloWhistWebController(factory)
	defer ctrl.Stop()

	run := func(t *testing.T, body, want string, code int) {
		t.Helper()
		var input controller.SoloWhistWebInput
	...
	t.Run("q", func(t *testing.T) {
		run(t, `{"command":"q","sessionId":"s1"}`, mustSoloWhistOutputJSON("bye."), http.StatusOK)
	})
	t.Run("reset", func(t *testing.T) {
		run(t, `{"command":"reset","sessionId":"s1"}`, mockOutput, http.StatusOK)
	})
	t.Run("bid", func(t *testing.T) {
		input := controller.SoloWhistWebInput{
	...
	t.Run("bid missing", func(t *testing.T) {
		run(t, `{"command":"b","sessionId":"s1"}`, mustSoloWhistOutputJSON("param error: bid is required."), http.StatusBadRequest)
	})
	...
	t.Run("play missing cardIndex", func(t *testing.T) {
		run(t, `{"command":"p","sessionId":"s1"}`, mustSoloWhistOutputJSON("param error: cardIndex is required."), http.StatusBadRequest)
	})
	...
	t.Run("unsupported command", func(t *testing.T) {
		run(t, `{"command":"other","sessionId":"s1"}`, mustSoloWhistOutputJSON("Unsupported command."), http.StatusBadRequest)
	})
	t.Run("empty command", func(t *testing.T) {
		run(t, `{"command":"","sessionId":"s1"}`, mustSoloWhistOutputJSON("param error."), http.StatusBadRequest)
	})
}

func TestSoloWhistWebController_ResetWithConfig(t *testing.T) {
	mockOutput := `{"players":[],"currentTrick":[]}`

	t.Run("custom config passed through", func(t *testing.T) {
		diff := 2
		pts := 15
		expected := domain.SoloWhistConfig{
			CpuDifficulty: domain.SoloWhistCpuDifficultyHard,
			TargetPoints:  15,
		}
		diMock := new(usecase.MockSoloWhistInteractor)
		diMock.On("ResetWithConfig", expected).Return(mockOutput)
		ctrl := controller.NewSoloWhistWebController(func() uc.SoloWhistInteractorIF { return diMock })
		defer ctrl.Stop()

		input := controller.SoloWhistWebInput{
			BaseWebInput: controller.BaseWebInput{Command: "reset", SessionID: "c1"},
			Config:       &controller.SoloWhistWebConfig{CpuDifficulty: &diff, TargetPoints: &pts},
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
	...
	t.Run("out of range difficulty falls back to default", func(t *testing.T) {
		...
		input := controller.SoloWhistWebInput{
			...
			Config:       &controller.SoloWhistWebConfig{CpuDifficulty: &diff},
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		diMock.AssertCalled(t, "ResetWithConfig", expected)
...
func TestSoloWhistWebController_Stop(t *testing.T) {
	diMock := new(usecase.MockSoloWhistInteractor)
	c := controller.NewSoloWhistWebController(func() uc.SoloWhistInteractorIF { return diMock })
	c.Stop()
	c.Stop()
}
