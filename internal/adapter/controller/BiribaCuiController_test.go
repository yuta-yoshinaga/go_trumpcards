...
func TestBiribaCuiController_Exec(t *testing.T) {
	mockOutput := `{"phase":0}`

	newMock := func() *mockUsecases.MockBiribaInteractor {
		m := new(mockUsecases.MockBiribaInteractor)
	...
	t.Run("quit command q", func(t *testing.T) {
		c := controller.NewBiribaCuiController(newMock())
	...
	t.Run("reset command r preserves config", func(t *testing.T) {
		m := newMock()
		c := controller.NewBiribaCuiController(m)
		result := c.Exec("r")
		assert.Equal(t, mockOutput, result)
		m.AssertCalled(t, "GetConfig")
		m.AssertCalled(t, "ResetWithConfig", domain.DefaultBiribaConfig())
	})
	...
	t.Run("setdifficulty sd valid", func(t *testing.T) {
		...
		expected.CpuDifficulty = domain.BiribaCpuDifficultyHard
	...
	t.Run("setdifficulty long form", func(t *testing.T) {
		...
		assert.Equal(t, mockOutput, result)
		expected := domain.DefaultBiribaConfig()
		expected.CpuDifficulty = domain.BiribaCpuDifficultyEasy
		m.AssertCalled(t, "ResetWithConfig", expected)
	})
