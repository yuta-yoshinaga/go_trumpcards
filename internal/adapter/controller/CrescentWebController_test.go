...
func mustCrescentOutputJSON(msg string) string {
	...
	if err != nil {
		panic(fmt.Sprintf("mustCrescentOutputJSON: %v", err))
	}
	return string(b)
}

func TestCrescentWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"redealsRemaining":3,"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	ciMock := new(usecase.MockCrescentInteractor)
	...
	factory := func() uc.CrescentInteractorIF { return ciMock }
	ctrl := controller.NewCrescentWebController(factory)
	defer ctrl.Stop()

	t.Run("quit q", func(t *testing.T) {
		var input controller.CrescentWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustCrescentOutputJSON("bye."))
	})
