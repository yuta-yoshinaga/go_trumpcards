...
func TestMarriageCuiController_Exec(t *testing.T) {
	const mockOutput = `{"phase":0}`

	newMock := func() *mockUsecases.MockMarriageInteractor {
		m := new(mockUsecases.MockMarriageInteractor)
	...
	t.Run("quit", func(t *testing.T) {
		c := controller.NewMarriageCuiController(newMock())
	...
	t.Run("reset preserves config", func(t *testing.T) {
		m := newMock()
		c := controller.NewMarriageCuiController(m)
		assert.Equal(t, mockOutput, c.Exec("r"))
		m.AssertCalled(t, "ResetWithConfig", domain.DefaultMarriageConfig())
	})
	...
	t.Run("setplayers", func(t *testing.T) {
		...
		m.AssertCalled(t, "ResetWithConfig", mock.MatchedBy(func(cfg domain.MarriageConfig) bool {
	...
	t.Run("setdifficulty", func(t *testing.T) {
		...
		m.AssertCalled(t, "ResetWithConfig", mock.MatchedBy(func(cfg domain.MarriageConfig) bool {
			return cfg.CpuDifficulty == domain.MarriageCpuDifficultyHard
		}))
	})
