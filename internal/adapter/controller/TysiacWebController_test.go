...
func mustTysiacOutputJSON(msg string) string {
	...
	if err != nil {
		panic(fmt.Sprintf("mustTysiacOutputJSON: %v", err))
	}
	return string(b)
}

func TestTysiacWebController_Method(t *testing.T) {
	mockOutput := `{"phase":0}`

	diMock := new(usecase.MockTysiacInteractor)
	diMock.On("ResetWithConfig", domain.DefaultTysiacConfig()).Return(mockOutput)
	diMock.On("Bid", true).Return(mockOutput)
	...
	factory := func() uc.TysiacInteractorIF { return diMock }
	ctrl := controller.NewTysiacWebController(factory)
	defer ctrl.Stop()

	run := func(t *testing.T, body, want string, code int) {
		t.Helper()
		var input controller.TysiacWebInput
	...
	t.Run("q", func(t *testing.T) {
		run(t, `{"command":"q","sessionId":"s1"}`, mustTysiacOutputJSON("bye."), http.StatusOK)
	})
	t.Run("reset", func(t *testing.T) {
		run(t, `{"command":"reset","sessionId":"s1"}`, mockOutput, http.StatusOK)
	})
	t.Run("bid", func(t *testing.T) {
		input := controller.TysiacWebInput{
			BaseWebInput: controller.BaseWebInput{Command: "b", SessionID: "s1"},
			Raise:        func() *bool { v := true; return &v }(),
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mockOutput)
	})
	t.Run("bid missing raise", func(t *testing.T) {
		run(t, `{"command":"b","sessionId":"s1"}`, mustTysiacOutputJSON("param error: raise is required."), http.StatusBadRequest)
	})
	t.Run("discard", func(t *testing.T) {
		input := controller.TysiacWebInput{
			BaseWebInput: controller.BaseWebInput{Command: "d", SessionID: "s1"},
			CardIndex:    func() *int { v := 2; return &v }(),
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mockOutput)
	...
	t.Run("discard missing cardIndex", func(t *testing.T) {
		run(t, `{"command":"d","sessionId":"s1"}`, mustTysiacOutputJSON("param error: cardIndex is required."), http.StatusBadRequest)
	})
	...
	t.Run("play missing cardIndex", func(t *testing.T) {
		run(t, `{"command":"p","sessionId":"s1"}`, mustTysiacOutputJSON("param error: cardIndex is required."), http.StatusBadRequest)
	})
	...
	t.Run("unsupported command", func(t *testing.T) {
		run(t, `{"command":"other","sessionId":"s1"}`, mustTysiacOutputJSON("Unsupported command."), http.StatusBadRequest)
	})
	t.Run("empty command", func(t *testing.T) {
		run(t, `{"command":"","sessionId":"s1"}`, mustTysiacOutputJSON("param error."), http.StatusBadRequest)
	})
}

func TestTysiacWebController_ResetWithConfig(t *testing.T) {
	mockOutput := `{"players":[],"currentTrick":[]}`

	t.Run("custom config passed through", func(t *testing.T) {
		diff := 2
		pts := 500
		expected := domain.TysiacConfig{
			CpuDifficulty: domain.TysiacCpuDifficultyHard,
			TargetPoints:  500,
		}
		diMock := new(usecase.MockTysiacInteractor)
		diMock.On("ResetWithConfig", expected).Return(mockOutput)
		ctrl := controller.NewTysiacWebController(func() uc.TysiacInteractorIF { return diMock })
		defer ctrl.Stop()

		input := controller.TysiacWebInput{
			BaseWebInput: controller.BaseWebInput{Command: "reset", SessionID: "c1"},
			Config:       &controller.TysiacWebConfig{CpuDifficulty: &diff, TargetPoints: &pts},
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
	...
	t.Run("out of range difficulty falls back to default", func(t *testing.T) {
		...
		input := controller.TysiacWebInput{
			...
			Config:       &controller.TysiacWebConfig{CpuDifficulty: &diff},
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		diMock.AssertCalled(t, "ResetWithConfig", expected)
...
func TestTysiacWebController_Stop(t *testing.T) {
	diMock := new(usecase.MockTysiacInteractor)
	c := controller.NewTysiacWebController(func() uc.TysiacInteractorIF { return diMock })
	c.Stop()
	c.Stop()
}
