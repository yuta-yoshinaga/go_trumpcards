...
func TestTeenPattiCuiController_Exec(t *testing.T) {
	mockOutput := `{"phase":0}`

	newMock := func() *mockUsecases.MockTeenPattiInteractor {
		m := new(mockUsecases.MockTeenPattiInteractor)
		...
		m.On("RequestSideShow").Return(mockOutput)
		m.On("RespondSideShow", mock.Anything).Return(mockOutput)
	...
	t.Run("quit", func(t *testing.T) {
		assert.Equal(t, "bye.", controller.NewTeenPattiCuiController(newMock()).Exec("q"))
		assert.Equal(t, "bye.", controller.NewTeenPattiCuiController(newMock()).Exec("quit"))
	})
	...
	t.Run("reset preserves config", func(t *testing.T) {
		...
		m.AssertCalled(t, "ResetWithConfig", domain.DefaultTeenPattiConfig())
	})
	...
	t.Run("raise missing amount", func(t *testing.T) {
		result := controller.NewTeenPattiCuiController(newMock()).Exec("rs")
	...
	t.Run("sideshow and alias", func(t *testing.T) {
		m := newMock()
		c := controller.NewTeenPattiCuiController(m)
		assert.Equal(t, mockOutput, c.Exec("ss"))
		assert.Equal(t, mockOutput, c.Exec("sideshow"))
		m.AssertCalled(t, "RequestSideShow")
	})

	t.Run("accept side show", func(t *testing.T) {
		m := newMock()
		c := controller.NewTeenPattiCuiController(m)
		assert.Equal(t, mockOutput, c.Exec("ac"))
		assert.Equal(t, mockOutput, c.Exec("accept"))
		m.AssertCalled(t, "RespondSideShow", true)
	})

	t.Run("decline side show", func(t *testing.T) {
		m := newMock()
		c := controller.NewTeenPattiCuiController(m)
		assert.Equal(t, mockOutput, c.Exec("dc"))
		assert.Equal(t, mockOutput, c.Exec("decline"))
		m.AssertCalled(t, "RespondSideShow", false)
	})
	...
	t.Run("unknown command", func(t *testing.T) {
		result := controller.NewTeenPattiCuiController(newMock()).Exec("zzz")
