...
func mustCalculationOutputJSON(msg string) string {
	out := &controller.CalculationWebOutput{
	...
	if err != nil {
		panic(fmt.Sprintf("mustCalculationOutputJSON: %v", err))
	}
	return string(b)
}

func TestCalculationWebController_Method(t *testing.T) {
	mockOut := `{"foundations":[],"wastes":[],"stockCount":0,"phase":0,"moveCount":0,"canUndo":false,"isStalemate":false,"undoToEscape":0,"message":""}`
	expected := mockOut

	m := new(usecase.MockCalculationInteractor)
	...
	factory := func() uc.CalculationInteractorIF { return m }
	ctrl := controller.NewCalculationWebController(factory)
	defer ctrl.Stop()

	decode := func(raw string) controller.CalculationWebInput {
		var input controller.CalculationWebInput
	...
	t.Run("quit", func(t *testing.T) {
		...
		recorded.BodyIs(mustCalculationOutputJSON("bye."))
	})
