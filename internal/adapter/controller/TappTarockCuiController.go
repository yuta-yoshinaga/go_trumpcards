...
// TappTarockCuiController タップ・タロックの CUI コントローラー。
type TappTarockCuiController struct {
	zi usecase.TappTarockInteractorIF
}

// NewTappTarockCuiController コンストラクタ。
func NewTappTarockCuiController(zi usecase.TappTarockInteractorIF) *TappTarockCuiController {
	return &TappTarockCuiController{zi: zi}
}
...
//	h / hint                 → ヒント表示
//	log / l                  → 棋譜表示
func (c *TappTarockCuiController) Exec(command string) string {
			...
			case "sd", "setdifficulty":
				return cuiutil.WithParsedIntKeys(args,
					...
					"invalidCpuDifficulty", 0, 2, func(v int) string {
						cfg := c.zi.GetConfig()
						cfg.CpuDifficulty = domain.TappTarockCpuDifficulty(v)
						return c.zi.ResetWithConfig(cfg)
					})
			case "st", "setdeals":
				return cuiutil.WithParsedIntKeys(args, "numberOfDealsRequired112", "invalidNumberOfDeals112",
					domain.TappTarockMinDeals, domain.TappTarockMaxDeals, func(v int) string {
						cfg := c.zi.GetConfig()
						cfg.TargetDeals = v
						return c.zi.ResetWithConfig(cfg)
					})
...
func (c *TappTarockCuiController) execBid(args []string) (string, bool) {
	if len(args) == 0 {
		return invalidArg("bidRequiredDreierOrSolo"), true
	}
	bid := tapptarockParseBid(args[0])
	if bid == domain.TappTarockBidPass {
		return invalidArg("invalidBidDreierOrSolo", "val", args[0]), true
	}
	return c.zi.Bid(bid), true
}

// execDiscard discard サブコマンドを解釈する (6 枚のインデックス)。
func (c *TappTarockCuiController) execDiscard(args []string) (string, bool) {
	if len(args) < domain.TappTarockTalonSize {
