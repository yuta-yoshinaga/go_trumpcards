...
func mustFortyFivesOutputJSON(msg string) string {
	...
	if err != nil {
		panic(fmt.Sprintf("mustFortyFivesOutputJSON: %v", err))
	}
	return string(b)
}

func TestFortyFivesWebController_Method(t *testing.T) {
	mockOutput := `{"phase":0}`

	diMock := new(usecase.MockFortyFivesInteractor)
	diMock.On("ResetWithConfig", domain.DefaultFortyFivesConfig()).Return(mockOutput)
	...
	factory := func() uc.FortyFivesInteractorIF { return diMock }
	ctrl := controller.NewFortyFivesWebController(factory)
	defer ctrl.Stop()

	run := func(t *testing.T, body, want string, code int) {
		t.Helper()
		var input controller.FortyFivesWebInput
	...
	t.Run("q", func(t *testing.T) {
		run(t, `{"command":"q","sessionId":"s1"}`, mustFortyFivesOutputJSON("bye."), http.StatusOK)
	})
	t.Run("reset", func(t *testing.T) {
		run(t, `{"command":"reset","sessionId":"s1"}`, mockOutput, http.StatusOK)
	})
	t.Run("bid", func(t *testing.T) {
		input := controller.FortyFivesWebInput{
	...
	t.Run("bid missing", func(t *testing.T) {
		run(t, `{"command":"b","sessionId":"s1"}`, mustFortyFivesOutputJSON("param error: bid is required."), http.StatusBadRequest)
	})
	...
	t.Run("play missing cardIndex", func(t *testing.T) {
		run(t, `{"command":"p","sessionId":"s1"}`, mustFortyFivesOutputJSON("param error: cardIndex is required."), http.StatusBadRequest)
	})
	...
	t.Run("unsupported command", func(t *testing.T) {
		run(t, `{"command":"other","sessionId":"s1"}`, mustFortyFivesOutputJSON("Unsupported command."), http.StatusBadRequest)
	})
	t.Run("empty command", func(t *testing.T) {
		run(t, `{"command":"","sessionId":"s1"}`, mustFortyFivesOutputJSON("param error."), http.StatusBadRequest)
	})
}

func TestFortyFivesWebController_ResetWithConfig(t *testing.T) {
	mockOutput := `{"players":[],"currentTrick":[]}`

	t.Run("custom config passed through", func(t *testing.T) {
		diff := 2
		pts := 60
		expected := domain.FortyFivesConfig{
			CpuDifficulty: domain.FortyFivesCpuDifficultyHard,
			TargetPoints:  60,
		}
		diMock := new(usecase.MockFortyFivesInteractor)
		diMock.On("ResetWithConfig", expected).Return(mockOutput)
		ctrl := controller.NewFortyFivesWebController(func() uc.FortyFivesInteractorIF { return diMock })
		defer ctrl.Stop()

		input := controller.FortyFivesWebInput{
			BaseWebInput: controller.BaseWebInput{Command: "reset", SessionID: "c1"},
			Config:       &controller.FortyFivesWebConfig{CpuDifficulty: &diff, TargetPoints: &pts},
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
	...
	t.Run("out of range difficulty falls back to default", func(t *testing.T) {
		...
		input := controller.FortyFivesWebInput{
			...
			Config:       &controller.FortyFivesWebConfig{CpuDifficulty: &diff},
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		diMock.AssertCalled(t, "ResetWithConfig", expected)
...
func TestFortyFivesWebController_Stop(t *testing.T) {
	diMock := new(usecase.MockFortyFivesInteractor)
	c := controller.NewFortyFivesWebController(func() uc.FortyFivesInteractorIF { return diMock })
	c.Stop()
	c.Stop()
}
