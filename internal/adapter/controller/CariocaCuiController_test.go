...
func TestCariocaCuiController_Exec(t *testing.T) {
	const mockOutput = `{"phase":0}`

	newMock := func() *mockUsecases.MockCariocaInteractor {
		m := new(mockUsecases.MockCariocaInteractor)
	...
	t.Run("quit", func(t *testing.T) {
		c := controller.NewCariocaCuiController(newMock())
	...
	t.Run("reset preserves config", func(t *testing.T) {
		...
		m.AssertCalled(t, "ResetWithConfig", domain.DefaultCariocaConfig())
	})
	...
	t.Run("setplayers", func(t *testing.T) {
		...
		m.AssertCalled(t, "ResetWithConfig", mock.MatchedBy(func(cfg domain.CariocaConfig) bool {
	...
	t.Run("setdifficulty", func(t *testing.T) {
		...
		m.AssertCalled(t, "ResetWithConfig", mock.MatchedBy(func(cfg domain.CariocaConfig) bool {
			return cfg.CpuDifficulty == domain.CariocaCpuDifficultyHard
		}))
	})
