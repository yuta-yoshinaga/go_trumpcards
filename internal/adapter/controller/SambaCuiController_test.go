...
func TestSambaCuiController_Exec(t *testing.T) {
	mockOutput := `{"phase":0}`

	newMock := func() *mockUsecases.MockSambaInteractor {
		m := new(mockUsecases.MockSambaInteractor)
	...
	t.Run("reset preserves config", func(t *testing.T) {
		m := newMock()
		c := controller.NewSambaCuiController(m)
		assert.Equal(t, mockOutput, c.Exec("r"))
		m.AssertCalled(t, "GetConfig")
		m.AssertCalled(t, "ResetWithConfig", domain.DefaultSambaConfig())
	})
	...
	t.Run("setdifficulty valid", func(t *testing.T) {
		...
		expected.CpuDifficulty = domain.SambaCpuDifficultyHard
