...
func TestKlaverjasCuiController_Exec(t *testing.T) {
	mockOutput := `{"phase":0}`

	newMock := func() *mockUsecases.MockKlaverjasInteractor {
		m := new(mockUsecases.MockKlaverjasInteractor)
	...
	t.Run("quit", func(t *testing.T) {
		assert.Equal(t, "bye.", controller.NewKlaverjasCuiController(newMock()).Exec("q"))
		assert.Equal(t, "bye.", controller.NewKlaverjasCuiController(newMock()).Exec("quit"))
	})

	t.Run("reset preserves config", func(t *testing.T) {
		m := newMock()
		c := controller.NewKlaverjasCuiController(m)
		assert.Equal(t, mockOutput, c.Exec("r"))
		m.AssertCalled(t, "ResetWithConfig", domain.DefaultKlaverjasConfig())
	})
	...
	t.Run("setdifficulty", func(t *testing.T) {
		...
		expected.CpuDifficulty = domain.KlaverjasCpuDifficultyHard
	...
	t.Run("unknown command", func(t *testing.T) {
		result := controller.NewKlaverjasCuiController(newMock()).Exec("zzz")
