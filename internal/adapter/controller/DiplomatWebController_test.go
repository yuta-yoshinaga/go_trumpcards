...
func mustDiplomatOutputJSON(msg string) string {
	...
	if err != nil {
		panic(fmt.Sprintf("mustDiplomatOutputJSON: %v", err))
	}
	return string(b)
}

func TestDiplomatWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"stockCount":0,"waste":[],"phase":0,"moveCount":0,"message":""}`

	ciMock := new(usecase.MockDiplomatInteractor)
	...
	ctrl := controller.NewDiplomatWebController(func() uc.DiplomatInteractorIF { return ciMock })
	defer ctrl.Stop()

	exec := func(t *testing.T, body string) *recorded {
		t.Helper()
		var input controller.DiplomatWebInput
	...
	t.Run("quit q", func(t *testing.T) {
		...
		r.BodyIs(mustDiplomatOutputJSON("bye."))
	})
