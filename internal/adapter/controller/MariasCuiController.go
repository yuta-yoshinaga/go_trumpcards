...
// MariasCuiController マリアーシュのCUIコントローラークラス
type MariasCuiController struct {
	di usecase.MariasInteractorIF
}

// NewMariasCuiController コンストラクタ
func NewMariasCuiController(di usecase.MariasInteractorIF) *MariasCuiController {
	return &MariasCuiController{di: di}
}
...
//	h / hint                 → ヒント表示
//	log / l                  → 棋譜表示
func (c *MariasCuiController) Exec(command string) string {
