...
func mustCongressOutputJSON(msg string) string {
	out := &controller.CongressWebOutput{
	...
	if err != nil {
		panic(fmt.Sprintf("mustCongressOutputJSON: %v", err))
	}
	return string(b)
}

func TestCongressWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"stockCount":0,"waste":[],"phase":0,"moveCount":0,"message":""}`

	ciMock := new(usecase.MockCongressInteractor)
	...
	ctrl := controller.NewCongressWebController(func() uc.CongressInteractorIF { return ciMock })
	defer ctrl.Stop()

	exec := func(t *testing.T, body string) *recorded {
		t.Helper()
		var input controller.CongressWebInput
	...
	t.Run("quit q", func(t *testing.T) {
		...
		r.BodyIs(mustCongressOutputJSON("bye."))
	})
