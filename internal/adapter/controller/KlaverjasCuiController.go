...
// KlaverjasCuiController クラヴァヤスのCUIコントローラークラス
type KlaverjasCuiController struct {
	di usecase.KlaverjasInteractorIF
}

// NewKlaverjasCuiController コンストラクタ
func NewKlaverjasCuiController(di usecase.KlaverjasInteractorIF) *KlaverjasCuiController {
	return &KlaverjasCuiController{di: di}
}
...
//	h / hint                 → ヒント表示
//	log / l                  → 棋譜表示
func (c *KlaverjasCuiController) Exec(command string) string {
