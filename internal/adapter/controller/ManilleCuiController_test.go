...
func TestManilleCuiController_Exec(t *testing.T) {
	mockOutput := `{"phase":0}`

	newMock := func() *mockUsecases.MockManilleInteractor {
		m := new(mockUsecases.MockManilleInteractor)
	...
	t.Run("quit", func(t *testing.T) {
		assert.Equal(t, "bye.", controller.NewManilleCuiController(newMock()).Exec("q"))
		assert.Equal(t, "bye.", controller.NewManilleCuiController(newMock()).Exec("quit"))
	})

	t.Run("reset preserves config", func(t *testing.T) {
		m := newMock()
		c := controller.NewManilleCuiController(m)
		assert.Equal(t, mockOutput, c.Exec("r"))
		m.AssertCalled(t, "ResetWithConfig", domain.DefaultManilleConfig())
	})
	...
	t.Run("setdifficulty", func(t *testing.T) {
		...
		expected.CpuDifficulty = domain.ManilleCpuDifficultyHard
	...
	t.Run("unknown command", func(t *testing.T) {
		result := controller.NewManilleCuiController(newMock()).Exec("zzz")
