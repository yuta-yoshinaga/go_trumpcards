...
func mustBigBenOutputJSON(msg string) string {
	out := &controller.BigBenWebOutput{
		Tableau:       [][]*controller.BigBenWebOutputTableauCard{},
		Foundation:    []*controller.BigBenWebOutputFoundation{},
		WebOutputBase: controller.WebOutputBase{Message: msg},
	}
	b, err := json.Marshal(out)
	if err != nil {
		panic(fmt.Sprintf("mustBigBenOutputJSON: %v", err))
	}
	return string(b)
}

func TestBigBenWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"phase":0,"moveCount":0,"message":""}`

	giMock := new(usecase.MockBigBenInteractor)
	...
	ctrl := controller.NewBigBenWebController(func() uc.BigBenInteractorIF { return giMock })
	defer ctrl.Stop()

	exec := func(t *testing.T, body string) *recorded {
		t.Helper()
		var input controller.BigBenWebInput
	...
	t.Run("quit q", func(t *testing.T) {
		...
		r.BodyIs(mustBigBenOutputJSON("bye."))
	})
