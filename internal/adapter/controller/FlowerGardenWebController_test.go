...
func mustFlowerGardenOutputJSON(msg string) string {
	out := &controller.FlowerGardenWebOutput{
		Tableau:       [][]*controller.FlowerGardenWebOutputTableauCard{},
	...
	if err != nil {
		panic(fmt.Sprintf("mustFlowerGardenOutputJSON: %v", err))
	}
	return string(b)
}

func TestFlowerGardenWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"reserve":[],"foundation":[],"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	biMock := new(usecase.MockFlowerGardenInteractor)
	...
	biMock.On("MoveReserveToTableau", 2, 5).Return(mockOutput)
	biMock.On("MoveReserveToFoundation", 3).Return(mockOutput)
	biMock.On("Undo").Return(mockOutput)
	biMock.On("UndoN", 2).Return(mockOutput)

	factory := func() uc.FlowerGardenInteractorIF { return biMock }
	ctrl := controller.NewFlowerGardenWebController(factory)
	defer ctrl.Stop()

	t.Run("quit q", func(t *testing.T) {
		var input controller.FlowerGardenWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustFlowerGardenOutputJSON("bye."))
	})
	...
	t.Run("move reserve to tableau", func(t *testing.T) {
		var input controller.FlowerGardenWebInput
		_ = json.Unmarshal([]byte(`{"command":"m","sessionId":"s1","from":{"zone":"reserve","col":2},"to":{"zone":"tableau","col":5}}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(expectedBody)
	})
	...
	t.Run("move reserve to foundation", func(t *testing.T) {
		var input controller.FlowerGardenWebInput
		_ = json.Unmarshal([]byte(`{"command":"m","sessionId":"s1","from":{"zone":"reserve","col":3},"to":{"zone":"foundation"}}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(expectedBody)
	})
	...
	t.Run("move reserve to tableau missing to.col", func(t *testing.T) {
		var input controller.FlowerGardenWebInput
		_ = json.Unmarshal([]byte(`{"command":"m","sessionId":"s1","from":{"zone":"reserve","col":2},"to":{"zone":"tableau"}}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusBadRequest)
	})

	t.Run("move reserve to foundation missing col", func(t *testing.T) {
		var input controller.FlowerGardenWebInput
		_ = json.Unmarshal([]byte(`{"command":"m","sessionId":"s1","from":{"zone":"reserve"},"to":{"zone":"foundation"}}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusBadRequest)
	})
}
