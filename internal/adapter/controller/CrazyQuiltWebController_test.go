...
func mustCrazyQuiltOutputJSON(msg string) string {
	out := &controller.CrazyQuiltWebOutput{
		Quilt:               []*controller.WebOutputCard{},
		Available:           []bool{},
	...
	if err != nil {
		panic(fmt.Sprintf("mustCrazyQuiltOutputJSON: %v", err))
	}
	return string(b)
}

func TestCrazyQuiltWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"stockCount":0,"waste":[],"phase":0,"moveCount":0,"message":""}`

	ciMock := new(usecase.MockCrazyQuiltInteractor)
	...
	ciMock.On("MoveQuiltToFoundation", 1).Return(mockOutput)
	ciMock.On("MoveQuiltToWaste", 5).Return(mockOutput)
	ciMock.On("MoveWasteToFoundation").Return(mockOutput)
	ciMock.On("Undo").Return(mockOutput)
	ciMock.On("UndoN", 2).Return(mockOutput)

	ctrl := controller.NewCrazyQuiltWebController(func() uc.CrazyQuiltInteractorIF { return ciMock })
	defer ctrl.Stop()

	exec := func(t *testing.T, body string) *recorded {
		t.Helper()
		var input controller.CrazyQuiltWebInput
	...
	t.Run("quit q", func(t *testing.T) {
		...
		r.BodyIs(mustCrazyQuiltOutputJSON("bye."))
	})
	...
	for _, tc := range []struct{ name, body string }{
		...
		{"quilt to foundation", `{"command":"m","sessionId":"s1","from":{"zone":"quilt","col":1},"to":{"zone":"foundation"}}`},
		{"quilt to waste", `{"command":"m","sessionId":"s1","from":{"zone":"quilt","col":5},"to":{"zone":"waste"}}`},
	...
	for _, tc := range []struct{ name, body string }{
		...
		// キルトは移動先にならないし、山札は移動元にならない。
		{"stock as a source", `{"command":"m","sessionId":"s1","from":{"zone":"stock"},"to":{"zone":"quilt","col":3}}`},
		{"quilt as a destination", `{"command":"m","sessionId":"s1","from":{"zone":"waste"},"to":{"zone":"quilt","col":3}}`},
