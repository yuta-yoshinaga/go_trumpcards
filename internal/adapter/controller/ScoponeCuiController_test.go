...
func TestScoponeCuiController_Exec(t *testing.T) {
	mockOutput := `{"players":[]}`

	newMock := func() *mockUsecases.MockScoponeInteractor {
		m := new(mockUsecases.MockScoponeInteractor)
		m.On("Reset").Return(mockOutput)
		m.On("NextRound").Return(mockOutput)
		m.On("Play", mock.Anything, mock.Anything).Return(mockOutput)
		m.On("GetConfig").Return(domain.DefaultScoponeConfig())
	...
	t.Run("quit command", func(t *testing.T) {
		c := controller.NewScoponeCuiController(newMock())
