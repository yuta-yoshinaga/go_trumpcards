...
// JulepeCuiController フレペCUIコントローラークラス
type JulepeCuiController struct {
	ri usecase.JulepeInteractorIF
}

// NewJulepeCuiController コンストラクタ
func NewJulepeCuiController(ri usecase.JulepeInteractorIF) *JulepeCuiController {
	return &JulepeCuiController{ri: ri}
}
...
//	h / hint      → ヒント表示
//	log / l       → 棋譜表示
func (c *JulepeCuiController) Exec(command string) string {
