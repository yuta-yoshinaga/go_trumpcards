...
func mustCaribbeanStudOutputJSON(msg string) string {
	out := &controller.CaribbeanStudWebOutput{
	...
	if err != nil {
		panic(fmt.Sprintf("mustCaribbeanStudOutputJSON: %v", err))
	}
	return string(b)
}

func TestCaribbeanStudWebController_Method(t *testing.T) {
	...
	ciMock := new(usecase.MockCaribbeanStudInteractor)
	...
	factory := func() uc.CaribbeanStudInteractorIF { return ciMock }
	ctrl := controller.NewCaribbeanStudWebController(factory)
	defer ctrl.Stop()

	t.Run("quit q", func(t *testing.T) {
		var input controller.CaribbeanStudWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustCaribbeanStudOutputJSON("bye."))
	})
