...
func TestEscobaWebController_Method(t *testing.T) {
	mockOutput := `{"players":[]}`
	siMock := new(usecase.MockEscobaInteractor)
	...
	factory := func() uc.EscobaInteractorIF { return siMock }
	ctrl := controller.NewEscobaWebController(factory)
	defer ctrl.Stop()

	var jsonInput controller.EscobaWebInput
