...
func mustKingAlbertOutputJSON(msg string) string {
	out := &controller.KingAlbertWebOutput{
		Tableau:       [][]*controller.KingAlbertWebOutputTableauCard{},
	...
	if err != nil {
		panic(fmt.Sprintf("mustKingAlbertOutputJSON: %v", err))
	}
	return string(b)
}

func TestKingAlbertWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"reserve":[],"foundation":[],"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	biMock := new(usecase.MockKingAlbertInteractor)
	...
	factory := func() uc.KingAlbertInteractorIF { return biMock }
	ctrl := controller.NewKingAlbertWebController(factory)
	defer ctrl.Stop()

	t.Run("quit q", func(t *testing.T) {
		var input controller.KingAlbertWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustKingAlbertOutputJSON("bye."))
	})
