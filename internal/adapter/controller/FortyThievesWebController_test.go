...
func mustFortyThievesOutputJSON(msg string) string {
	out := &controller.FortyThievesWebOutput{
		Tableau:       [][]*controller.FortyThievesWebOutputTableauCard{},
	...
	if err != nil {
		panic(fmt.Sprintf("mustFortyThievesOutputJSON: %v", err))
	}
	return string(b)
}

func TestFortyThievesWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"stockCount":0,"waste":[],"foundation":[],"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	fiMock := new(usecase.MockFortyThievesInteractor)
	...
	factory := func() uc.FortyThievesInteractorIF { return fiMock }
	ctrl := controller.NewFortyThievesWebController(factory)
	defer ctrl.Stop()

	t.Run("quit q", func(t *testing.T) {
		var input controller.FortyThievesWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustFortyThievesOutputJSON("bye."))
	})
