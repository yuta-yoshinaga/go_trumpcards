...
func TestBauernschnapsenCuiController_Exec(t *testing.T) {
	mockOutput := `{"phase":0}`

	newMock := func() *mockUsecases.MockBauernschnapsenInteractor {
		m := new(mockUsecases.MockBauernschnapsenInteractor)
	...
	t.Run("reset r preserves config", func(t *testing.T) {
		m := newMock()
		c := controller.NewBauernschnapsenCuiController(m)
	...
	t.Run("setdifficulty valid", func(t *testing.T) {
		...
		expected := domain.DefaultBauernschnapsenConfig()
		expected.CpuDifficulty = domain.BauernschnapsenCpuDifficultyHard
