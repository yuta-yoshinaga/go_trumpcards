...
func mustBeleagueredCastleOutputJSON(msg string) string {
	out := &controller.BeleagueredCastleWebOutput{
		Tableau:       [][]*controller.BeleagueredCastleWebOutputTableauCard{},
		Foundation:    [][]*controller.WebOutputCard{},
		WebOutputBase: controller.WebOutputBase{Message: msg},
	}
	b, err := json.Marshal(out)
	if err != nil {
		panic(fmt.Sprintf("mustBeleagueredCastleOutputJSON: %v", err))
	}
	return string(b)
}

func TestBeleagueredCastleWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	biMock := new(usecase.MockBeleagueredCastleInteractor)
	...
	factory := func() uc.BeleagueredCastleInteractorIF { return biMock }
	ctrl := controller.NewBeleagueredCastleWebController(factory)
	defer ctrl.Stop()

	t.Run("quit q", func(t *testing.T) {
		var input controller.BeleagueredCastleWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustBeleagueredCastleOutputJSON("bye."))
	})
