...
func mustCalabresellaOutputJSON(msg string) string {
	...
	if err != nil {
		panic(fmt.Sprintf("mustCalabresellaOutputJSON: %v", err))
	}
	return string(b)
}

func TestCalabresellaWebController_Method(t *testing.T) {
	mockOutput := `{"phase":0}`

	diMock := new(usecase.MockCalabresellaInteractor)
	...
	factory := func() uc.CalabresellaInteractorIF { return diMock }
	ctrl := controller.NewCalabresellaWebController(factory)
	defer ctrl.Stop()

	run := func(t *testing.T, body, want string, code int) {
		t.Helper()
		var input controller.CalabresellaWebInput
	...
	t.Run("q", func(t *testing.T) {
		run(t, `{"command":"q","sessionId":"s1"}`, mustCalabresellaOutputJSON("bye."), http.StatusOK)
	})
	...
	t.Run("bid", func(t *testing.T) {
		input := controller.CalabresellaWebInput{
			...
			Bid:          func() *int { v := int(domain.CalabresellaBidChiamo); return &v }(),
		}
		recorded := execRequest(t, ctrl.Exec, &input)
	...
	t.Run("bid missing bid", func(t *testing.T) {
		run(t, `{"command":"b","sessionId":"s1"}`, mustCalabresellaOutputJSON("param error: bid is required."), http.StatusBadRequest)
	})
	...
	t.Run("play missing cardIndex", func(t *testing.T) {
		run(t, `{"command":"p","sessionId":"s1"}`, mustCalabresellaOutputJSON("param error: cardIndex is required."), http.StatusBadRequest)
	})
	...
	t.Run("unsupported command", func(t *testing.T) {
		run(t, `{"command":"other","sessionId":"s1"}`, mustCalabresellaOutputJSON("Unsupported command."), http.StatusBadRequest)
	})
	t.Run("empty command", func(t *testing.T) {
		run(t, `{"command":"","sessionId":"s1"}`, mustCalabresellaOutputJSON("param error."), http.StatusBadRequest)
	})
}

func TestCalabresellaWebController_ResetWithConfig(t *testing.T) {
	mockOutput := `{"players":[],"currentTrick":[]}`

	t.Run("custom config passed through", func(t *testing.T) {
		diff := 2
		pts := 30
		expected := domain.CalabresellaConfig{
			CpuDifficulty: domain.CalabresellaCpuDifficultyHard,
			TargetPoints:  30,
		}
		diMock := new(usecase.MockCalabresellaInteractor)
		diMock.On("ResetWithConfig", expected).Return(mockOutput)
		ctrl := controller.NewCalabresellaWebController(func() uc.CalabresellaInteractorIF { return diMock })
		defer ctrl.Stop()

		input := controller.CalabresellaWebInput{
			BaseWebInput: controller.BaseWebInput{Command: "reset", SessionID: "c1"},
			Config:       &controller.CalabresellaWebConfig{CpuDifficulty: &diff, TargetPoints: &pts},
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
	...
	t.Run("out of range difficulty falls back to default", func(t *testing.T) {
		...
		input := controller.CalabresellaWebInput{
			...
			Config:       &controller.CalabresellaWebConfig{CpuDifficulty: &diff},
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		diMock.AssertCalled(t, "ResetWithConfig", expected)
...
func TestCalabresellaWebController_Stop(t *testing.T) {
	diMock := new(usecase.MockCalabresellaInteractor)
	c := controller.NewCalabresellaWebController(func() uc.CalabresellaInteractorIF { return diMock })
	c.Stop()
	c.Stop()
}
