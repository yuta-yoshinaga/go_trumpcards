...
func newOmahaTestHandler(mi *mockUsecase.MockOmahaInteractor) (*rest.Api, *OmahaWebController) {
	api := rest.NewApi()
	api.Use(rest.DefaultDevStack...)
	owc := NewOmahaWebController(func() usecase.OmahaInteractorIF {
		return mi
	})
	router, _ := rest.MakeRouter(
		rest.Post("/omaha/exec", owc.Exec),
	)
	api.SetApp(router)
	return api, owc
}
...
func TestOmahaWebController_BadRequest_EmptyBody(t *testing.T) {
	...
	defer owc.Stop()
	recorded := test.RunRequest(t, api.MakeHandler(),
		test.MakeSimpleRequest("POST", "http://localhost/omaha/exec", nil))
	recorded.CodeIs(400)
}
...
func TestOmahaWebController_Stop(t *testing.T) {
	mi := new(mockUsecase.MockOmahaInteractor)
	c := NewOmahaWebController(func() usecase.OmahaInteractorIF {
		return mi
	})
	c.Stop()
	c.Stop()
