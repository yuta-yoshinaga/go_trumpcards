...
func mustCitadelOutputJSON(msg string) string {
	out := &controller.CitadelWebOutput{
		Tableau:       [][]*controller.CitadelWebOutputTableauCard{},
		Foundation:    [][]*controller.WebOutputCard{},
		WebOutputBase: controller.WebOutputBase{Message: msg},
	}
	b, err := json.Marshal(out)
	if err != nil {
		panic(fmt.Sprintf("mustCitadelOutputJSON: %v", err))
	}
	return string(b)
}

func TestCitadelWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	biMock := new(usecase.MockCitadelInteractor)
	...
	factory := func() uc.CitadelInteractorIF { return biMock }
	ctrl := controller.NewCitadelWebController(factory)
	defer ctrl.Stop()

	t.Run("quit q", func(t *testing.T) {
		var input controller.CitadelWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustCitadelOutputJSON("bye."))
	})
