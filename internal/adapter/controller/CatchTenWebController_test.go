...
func mustCatchTenOutputJSON(msg string) string {
	...
	if err != nil {
		panic(fmt.Sprintf("mustCatchTenOutputJSON: %v", err))
	}
	return string(b)
}

func TestCatchTenWebController_Method(t *testing.T) {
	...
	ciMock := new(usecase.MockCatchTenInteractor)
	ciMock.On("ResetWithConfig", domain.DefaultCatchTenConfig()).Return(mockOutput)
	...
	factory := func() uc.CatchTenInteractorIF { return ciMock }
	ctrl := controller.NewCatchTenWebController(factory)
	defer ctrl.Stop()

	t.Run("success Exec q", func(t *testing.T) {
		var input controller.CatchTenWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"test-session-1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.ContentTypeIsJson()
		recorded.BodyIs(mustCatchTenOutputJSON("bye."))
	})
...
func TestCatchTenWebConfig_ToConfig(t *testing.T) {
	t.Run("nil config uses defaults", func(t *testing.T) {
		input := controller.CatchTenWebInput{}
		assert.Equal(t, domain.DefaultCatchTenConfig(), input.ToConfig())
	})

	t.Run("custom config", func(t *testing.T) {
		diff := 2
		limit := 50
		c := &controller.CatchTenWebConfig{CpuDifficulty: &diff, PointLimit: &limit}
		cfg := c.ToConfig()
		assert.Equal(t, domain.CatchTenCpuDifficultyHard, cfg.CpuDifficulty)
