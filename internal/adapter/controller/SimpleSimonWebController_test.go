...
func newSsWebController() *controller.SimpleSimonWebController {
	factory := func() uc.SimpleSimonInteractorIF {
		g := domain.NewDefaultSimpleSimon()
		g.Reset()
		return uc.NewSimpleSimonInteractor(g, new(presenter.SimpleSimonWebPresenter))
	}
	return controller.NewSimpleSimonWebController(factory)
}

func TestSimpleSimonWebController_Exec(t *testing.T) {
	ctrl := newSsWebController()
	defer ctrl.Stop()
