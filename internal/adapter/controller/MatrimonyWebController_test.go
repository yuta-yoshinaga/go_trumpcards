...
func mustMatrimonyOutputJSON(msg string) string {
	out := &controller.MatrimonyWebOutput{
	...
	if err != nil {
		panic(fmt.Sprintf("mustMatrimonyOutputJSON: %v", err))
	}
	return string(b)
}

func TestMatrimonyWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"stockCount":0,"waste":[],"phase":0,"moveCount":0,"message":""}`

	ciMock := new(usecase.MockMatrimonyInteractor)
	...
	ctrl := controller.NewMatrimonyWebController(func() uc.MatrimonyInteractorIF { return ciMock })
	defer ctrl.Stop()

	exec := func(t *testing.T, body string) *recorded {
		t.Helper()
		var input controller.MatrimonyWebInput
	...
	t.Run("quit q", func(t *testing.T) {
		...
		r.BodyIs(mustMatrimonyOutputJSON("bye."))
	})
