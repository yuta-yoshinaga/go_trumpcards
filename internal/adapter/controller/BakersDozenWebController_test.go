...
func mustBakersDozenOutputJSON(msg string) string {
	out := &controller.BakersDozenWebOutput{
		Tableau:       [][]*controller.BakersDozenWebOutputTableauCard{},
		Foundation:    [][]*controller.WebOutputCard{},
		WebOutputBase: controller.WebOutputBase{Message: msg},
	}
	b, err := json.Marshal(out)
	if err != nil {
		panic(fmt.Sprintf("mustBakersDozenOutputJSON: %v", err))
	}
	return string(b)
}

func TestBakersDozenWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	biMock := new(usecase.MockBakersDozenInteractor)
	...
	factory := func() uc.BakersDozenInteractorIF { return biMock }
	ctrl := controller.NewBakersDozenWebController(factory)
	defer ctrl.Stop()
	...
	t.Run("quit q", func(t *testing.T) {
		...
		recorded.BodyIs(mustBakersDozenOutputJSON("bye."))
	})
