...
func mustSomersetOutputJSON(msg string) string {
	...
	if err != nil {
		panic(fmt.Sprintf("mustSomersetOutputJSON: %v", err))
	}
	return string(b)
}
...
func TestSomersetWebController_Method(t *testing.T) {
	...
	factory := func() uc.SomersetInteractorIF { return biMock }
	ctrl := controller.NewSomersetWebController(factory)
	defer ctrl.Stop()

	t.Run("quit q", func(t *testing.T) {
		var input controller.SomersetWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustSomersetOutputJSON("bye."))
	})
