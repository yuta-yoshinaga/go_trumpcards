...
func mustScorpionOutputJSON(msg string) string {
	out := &controller.ScorpionWebOutput{
		Tableau:       [][]*controller.KlondikeWebOutputTableauCard{},
		WebOutputBase: controller.WebOutputBase{Message: msg},
	}
	b, err := json.Marshal(out)
	if err != nil {
		panic(fmt.Sprintf("mustScorpionOutputJSON: %v", err))
	}
	return string(b)
}

func TestScorpionWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"stockCount":0,"completedSuits":0,"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	siMock := new(usecase.MockScorpionInteractor)
	...
	factory := func() uc.ScorpionInteractorIF { return siMock }
	ctrl := controller.NewScorpionWebController(factory)
	defer ctrl.Stop()

	t.Run("quit", func(t *testing.T) {
		var input controller.ScorpionWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustScorpionOutputJSON("bye."))
	})
