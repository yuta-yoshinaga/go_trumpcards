...
// MadrassoCuiController マドラッソのCUIコントローラークラス
type MadrassoCuiController struct {
	ti usecase.MadrassoInteractorIF
}

// NewMadrassoCuiController コンストラクタ
func NewMadrassoCuiController(ti usecase.MadrassoInteractorIF) *MadrassoCuiController {
	return &MadrassoCuiController{ti: ti}
}
...
//	h / hint              → ヒント表示
//	log / l               → 棋譜表示
func (c *MadrassoCuiController) Exec(command string) string {
		...
		func(cmd string, args []string) (string, bool) {
			...
			case "sd", "setdifficulty":
				return cuiutil.WithParsedIntKeys(args, "cpuDifficultyRequired", "invalidCpuDifficulty", 0, 2, func(v int) string {
					cfg := c.ti.GetConfig()
					cfg.CpuDifficulty = domain.MadrassoCpuDifficulty(v)
					return c.ti.ResetWithConfig(cfg)
				})
			case "st", "settarget":
