...
func mustYukonOutputJSON(msg string) string {
	out := &controller.YukonWebOutput{
	...
	if err != nil {
		panic(fmt.Sprintf("mustYukonOutputJSON: %v", err))
	}
	return string(b)
}

func TestYukonWebController_Method(t *testing.T) {
	mockOutput := `{"tableau":[],"foundation":[],"phase":0,"moveCount":0,"message":""}`
	expectedBody := mockOutput

	yiMock := new(usecase.MockYukonInteractor)
	yiMock.On("Reset").Return(mockOutput)
	yiMock.On("GiveUp").Return(mockOutput)
	yiMock.On("Hint").Return(mockOutput)
	yiMock.On("AutoComplete").Return(mockOutput)
	yiMock.On("ActionLog").Return(mockOutput)
	yiMock.On("MoveTableauToTableau", 0, 2, 4).Return(mockOutput)
	yiMock.On("MoveTableauToFoundation", 1).Return(mockOutput)
	yiMock.On("Undo").Return(mockOutput)
	yiMock.On("UndoN", 3).Return(mockOutput)

	factory := func() uc.YukonInteractorIF { return yiMock }
	ctrl := controller.NewYukonWebController(factory)
	defer ctrl.Stop()

	t.Run("quit", func(t *testing.T) {
		var input controller.YukonWebInput
		_ = json.Unmarshal([]byte(`{"command":"q","sessionId":"s1"}`), &input)
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		recorded.BodyIs(mustYukonOutputJSON("bye."))
	})
