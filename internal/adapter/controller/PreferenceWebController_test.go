...
func mustPreferenceOutputJSON(msg string) string {
	...
	if err != nil {
		panic(fmt.Sprintf("mustPreferenceOutputJSON: %v", err))
	}
	return string(b)
}

func TestPreferenceWebController_Method(t *testing.T) {
	mockOutput := `{"phase":0}`

	diMock := new(usecase.MockPreferenceInteractor)
	diMock.On("ResetWithConfig", domain.DefaultPreferenceConfig()).Return(mockOutput)
	...
	factory := func() uc.PreferenceInteractorIF { return diMock }
	ctrl := controller.NewPreferenceWebController(factory)
	defer ctrl.Stop()

	run := func(t *testing.T, body, want string, code int) {
		t.Helper()
		var input controller.PreferenceWebInput
	...
	t.Run("q", func(t *testing.T) {
		run(t, `{"command":"q","sessionId":"s1"}`, mustPreferenceOutputJSON("bye."), http.StatusOK)
	})
	t.Run("reset", func(t *testing.T) {
		run(t, `{"command":"reset","sessionId":"s1"}`, mockOutput, http.StatusOK)
	})
	t.Run("bid", func(t *testing.T) {
		input := controller.PreferenceWebInput{
	...
	t.Run("bid missing", func(t *testing.T) {
		run(t, `{"command":"b","sessionId":"s1"}`, mustPreferenceOutputJSON("param error: bid is required."), http.StatusBadRequest)
	})
	...
	t.Run("play missing cardIndex", func(t *testing.T) {
		run(t, `{"command":"p","sessionId":"s1"}`, mustPreferenceOutputJSON("param error: cardIndex is required."), http.StatusBadRequest)
	})
	...
	t.Run("unsupported command", func(t *testing.T) {
		run(t, `{"command":"other","sessionId":"s1"}`, mustPreferenceOutputJSON("Unsupported command."), http.StatusBadRequest)
	})
	t.Run("empty command", func(t *testing.T) {
		run(t, `{"command":"","sessionId":"s1"}`, mustPreferenceOutputJSON("param error."), http.StatusBadRequest)
	})
}

func TestPreferenceWebController_ResetWithConfig(t *testing.T) {
	mockOutput := `{"players":[],"currentTrick":[]}`

	t.Run("custom config passed through", func(t *testing.T) {
		diff := 2
		pts := 15
		expected := domain.PreferenceConfig{
			CpuDifficulty: domain.PreferenceCpuDifficultyHard,
			TargetPoints:  15,
		}
		diMock := new(usecase.MockPreferenceInteractor)
		diMock.On("ResetWithConfig", expected).Return(mockOutput)
		ctrl := controller.NewPreferenceWebController(func() uc.PreferenceInteractorIF { return diMock })
		defer ctrl.Stop()

		input := controller.PreferenceWebInput{
			BaseWebInput: controller.BaseWebInput{Command: "reset", SessionID: "c1"},
			Config:       &controller.PreferenceWebConfig{CpuDifficulty: &diff, TargetPoints: &pts},
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
	...
	t.Run("out of range difficulty falls back to default", func(t *testing.T) {
		...
		input := controller.PreferenceWebInput{
			...
			Config:       &controller.PreferenceWebConfig{CpuDifficulty: &diff},
		}
		recorded := execRequest(t, ctrl.Exec, &input)
		recorded.CodeIs(http.StatusOK)
		diMock.AssertCalled(t, "ResetWithConfig", expected)
...
func TestPreferenceWebController_Stop(t *testing.T) {
	diMock := new(usecase.MockPreferenceInteractor)
	c := controller.NewPreferenceWebController(func() uc.PreferenceInteractorIF { return diMock })
	c.Stop()
	c.Stop()
}
