...
- [agent-browser](https://github.com/vercel-labs/agent-browser) がインストール済み（`bun install -g agent-browser && agent-browser install`）
- Go サーバーが起動可能な状態
...
```sh
[ -f /tmp/desktop-ux-review-server.pid ] && kill "$(cat /tmp/desktop-ux-review-server.pid)" 2>/dev/null || true
PORT=8080 go run ./cmd/server &
echo $! > /tmp/desktop-ux-review-server.pid
# サーバー起動待ち（最大10秒）
...
- **ビューポート収まり**: プレイ中の全要素（カード、ボタン、スコア等）が1280x800のビューポート内に収まるか
...
- **ボタン/クリック対象**: クリック可能であることの視覚的手がかり。
...
- **収まることが大きさより優先**: 要素を大きくする・情報を足す等の変更がスクロールを生むなら改悪。小さくてもビューポートに収まりゲームが成立するならOK
- **参照情報はオンデマンド**: 配当表・ルール説明等の参照情報は折りたたみ表示が原則。展開して空白を埋めるのは本末転倒
- **アクション誘導**: ユーザーが次のアクションを迷わずできるか
...
| 重大度 | 基準 |
|--------|------|
| HIGH | ゲームプレイ不能、または操作・情報がビューポートに収まらない |
| MEDIUM | 操作性や視認性に問題がある |
...
agent-browser close
[ -f /tmp/desktop-ux-review-server.pid ] && kill "$(cat /tmp/desktop-ux-review-server.pid)" 2>/dev/null || true
rm -f /tmp/desktop-ux-review-server.pid
