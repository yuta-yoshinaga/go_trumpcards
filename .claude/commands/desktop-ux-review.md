...
症状（「空白が多い」「情報が多すぎる」等）ではなく、以下の設計原則で課題かどうかを判断する:

- **アクション誘導優先**: ベットフェーズ等の操作画面では「ユーザーが次のアクションを迷わずできるか」が最優先。空白もコンテンツ過多も、CTAの明確さで判断する
- **参照情報はオンデマンド**: 配当表・ルール説明等の参照情報は折りたたみ表示が原則。展開して空白を埋めるのは本末転倒
- **空白は必ずしも問題ではない**: CTAが明確で操作に迷わないなら、余白は意図的なデザインとして許容する
- **一貫性 > 個別最適**: 同種のゲーム（例: Video Poker系3ゲーム）は同じレイアウトパターンを共有すべき
- **デスクトップ特有**: PC画面の広さを活用すべきだが、「空白を埋めること」自体を目的にしない。情報の視認性と操作効率が向上する場合のみ横幅を活用する
