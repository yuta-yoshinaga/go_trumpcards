...
{casino, classic, solo, extra, extra2, extra3, extra4, extra5, extra6, extra7, extra8, extra9, extra10, extra11}) is fully and consistently wired, then report
...
- All four frontend `N` values AND the global total must be equal. The Go total is
  `expectedTotal`, which is defined as the sum of all fourteen per-bucket consts — check that its
