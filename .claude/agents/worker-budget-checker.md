...
Games ship to ten Cloudflare Workers as TinyGo WASM binaries. The split is a **binary-size
...
```bash
bash .claude/skills/rebucket-game/scripts/measure.sh              # all ten
...
same Go toolchain CI uses, so the sizes match). A full ten-worker run takes several minutes — build only the
...
Recommendation: put <game> in <worker> (<headroom> free, the largest of the ten).
Runner-up: <worker> (<headroom>).

Caveats:
  - <e.g. only 2 of 10 workers were measured; the unmeasured eight may have more room>
...
- If you measured only some workers, say so in Caveats and do not describe the winner as "the
  largest of the ten" — it is the largest of what you measured.
