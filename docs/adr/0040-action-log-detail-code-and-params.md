...
3. **セッション状態の一部。** 427 ゲームが `ActionLog []*ActionLogEntry \`json:"al"\`` を
   永続化する。KV セッションは 1 手ごとに伸び、本番の Klondike で 117 手目に
   503 / Cloudflare 1102 を再現した実績がある。

この掛け持ちが実際に壊した例:

- **ロケール漏れ (#7179 / #7180 / #7181 / #7182)。** `Detail` の文面は書いた人の言語で
  固定される。実測すると domain の `appendLog` 呼び出し **2,695 件**のうち、
  日本語リテラルが **513 件 (105 ファイル)**、英語リテラルが **1,937 件 (260 ファイル)**、
...
1. **`Detail` を i18n キーそのものにする。** `appendLog(idx, "trick_win", "piquet.trickWin", cards)`
   ...
   ただし **パラメータを持てない。** 実測 2,695 件のうち数値や名前を埋め込むもの
...
2. **`Detail` を残したまま `DetailCode` / `DetailParams` を足す。** 移行が段階的にでき、
   ...
   再現する。さらに 427 ゲームが永続化する `al` にフィールドが 2 つ増え、
...
2,695 呼び出しを一度に変えることはしない。ゲーム単位で移す。
...
- **2,695 呼び出しの書き換え**が必要で、ゲームあたり数件〜数十件になる。
...
- ロケールファイルに `log.*` が 2,695 件分 (重複を除いても数百件) 増える。
