...
classDiagram
    ...
    note for GamePage "全289ゲームページが同一パターンで構成\nuseGamePageSetup → ゲーム固有Hook → 描画"
```
...
classDiagram
    ...
    note for gameCategories "6カテゴリの各メンバー構成 (全289ゲーム) は\nfrontend/src/constants/gameRoutes.ts が SSoT。\n個別の所属はそこで定義される"
