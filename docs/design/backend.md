...
- [1. クラス図](#1-クラス図)
  ...
  - [1.2 ゲームドメイン (全335ゲーム)](#12-ゲームドメイン-全335ゲーム)
...
335 ゲームの多くが同じ関心事を持つので、以下は**埋め込みで共有**している。
...
classDiagram
    ...
    note for GameCuiController "335ゲーム × CUI/Web = 670 バインディング\n実装型は 644 種類 (CuiController 322 + WebController 322)\n差分は複数ゲームで共有される Controller\n(総称基底 GameWebController[I,P,O] は別)"
    note for GameCuiPresenter "335ゲーム × CUI/Web = 670 バインディング\n実装型は 646 種類 (CuiPresenter 323 + WebPresenter 323)"
```
...
classDiagram
    ...
    gameEntry --> GameWebController : holds 335 controllers
    GameManager --> "*" CuiExecer : holds 335 games
