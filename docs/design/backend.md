...
- [1. クラス図](#1-クラス図)
  ...
  - [1.2 ゲームドメイン (全334ゲーム)](#12-ゲームドメイン-全334ゲーム)
...
334 ゲームの多くが同じ関心事を持つので、以下は**埋め込みで共有**している。
...
classDiagram
    ...
    note for GameCuiController "334ゲーム × CUI/Web = 668 バインディング\n実装型は 642 種類 (CuiController 321 + WebController 321)\n差分は複数ゲームで共有される Controller\n(総称基底 GameWebController[I,P,O] は別)"
    note for GameCuiPresenter "334ゲーム × CUI/Web = 668 バインディング\n実装型は 644 種類 (CuiPresenter 322 + WebPresenter 322)"
```
...
classDiagram
    ...
    gameEntry --> GameWebController : holds 334 controllers
    GameManager --> "*" CuiExecer : holds 334 games
