...
- [1. クラス図](#1-クラス図)
  ...
  - [1.2 ゲームドメイン (全331ゲーム)](#12-ゲームドメイン-全331ゲーム)
...
331 ゲームの多くが同じ関心事を持つので、以下は**埋め込みで共有**している。
...
classDiagram
    ...
    note for GameCuiController "331ゲーム × CUI/Web = 662 バインディング\n実装型は 636 種類 (CuiController 318 + WebController 318)\n差分は複数ゲームで共有される Controller\n(総称基底 GameWebController[I,P,O] は別)"
    note for GameCuiPresenter "331ゲーム × CUI/Web = 662 バインディング\n実装型は 638 種類 (CuiPresenter 319 + WebPresenter 319)"
```
...
classDiagram
    ...
    gameEntry --> GameWebController : holds 331 controllers
    GameManager --> "*" CuiExecer : holds 331 games
