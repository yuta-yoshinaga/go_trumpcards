...
- [1. クラス図](#1-クラス図)
  ...
  - [1.2 ゲームドメイン (全321ゲーム)](#12-ゲームドメイン-全321ゲーム)
...
321 ゲームの多くが同じ関心事を持つので、以下は**埋め込みで共有**している。
...
classDiagram
    ...
    note for GameCuiController "321ゲーム × CUI/Web = 642 バインディング\n実装型は 616 種類 (CuiController 308 + WebController 308)\n差分は複数ゲームで共有される Controller\n(総称基底 GameWebController[I,P,O] は別)"
    note for GameCuiPresenter "321ゲーム × CUI/Web = 642 バインディング\n実装型は 618 種類 (CuiPresenter 309 + WebPresenter 309)"
```
...
classDiagram
    ...
    gameEntry --> GameWebController : holds 321 controllers
    GameManager --> "*" CuiExecer : holds 321 games
