...
- [1. クラス図](#1-クラス図)
  ...
  - [1.2 ゲームドメイン (全326ゲーム)](#12-ゲームドメイン-全326ゲーム)
...
326 ゲームの多くが同じ関心事を持つので、以下は**埋め込みで共有**している。
...
classDiagram
    ...
    note for GameCuiController "326ゲーム × CUI/Web = 652 バインディング\n実装型は 626 種類 (CuiController 313 + WebController 313)\n差分は複数ゲームで共有される Controller\n(総称基底 GameWebController[I,P,O] は別)"
    note for GameCuiPresenter "326ゲーム × CUI/Web = 652 バインディング\n実装型は 628 種類 (CuiPresenter 314 + WebPresenter 314)"
```
...
classDiagram
    ...
    gameEntry --> GameWebController : holds 326 controllers
    GameManager --> "*" CuiExecer : holds 326 games
