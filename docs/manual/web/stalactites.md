...
ブラウザで `http://localhost:8080` にアクセスし、ナビゲーションバーから「スタラクタイト」を選択します。
...
1. タブローのカードを選択し、空いているセルをクリックして退避します
2. セルのカードを選択し、タブローや組札をクリックして戻します
...
- **セルエリア**: 4つの一時退避スペース（カード1枚ずつ）
- **組札エリア**: 4つの組札。**スートは無視**し、開始ランクから昇順（K の次は A）
- **タブロー**: 8列のカード列（すべて表向き）
- **ボタン**:
  - 「元に戻す」: 直前の操作を取り消す
  - 「ヒント」: 次の一手を提案
