...
ビリバはギリシャで広く親しまれている、4人（2対2）のパートナーシップ制ラミーです。106枚のカードで同じスートのシーケンスを作り、純ビリバとポゼットの消化による得点を競います。
...
- 4人（2対2）で遊び、各プレイヤーに11枚、伏せた買い手札（ポゼット）を2組配ります。
- メルドは同じスートの連続したランクによるシーケンスです。ジョーカーと2はワイルドです。
- 最初の手札を使い切った側はポゼットを拾って続行します。
- 7枚以上でワイルドを1枚も使わないシーケンスは純ビリバとして高得点です。
- 両方のポゼットを消化して上がると追加点を得ます。
...
- パートナーのメルドとスートを見ながら、ポゼットを安全に拾いましょう。
