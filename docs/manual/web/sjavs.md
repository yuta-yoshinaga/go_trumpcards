...
3. ゲーム一覧から **シャウス** を選ぶか、`/sjavs` へ直接アクセスします
...
| 操作 | 説明 |
|------|------|
| **申告** ボタン | 枚数を選んで申告します。5 枚未満と、自分の最長を超える枚数は選べません |
| **パス** ボタン | 申告せずに次へ回します |
| 手札のカードをクリック | その札を出します（出せる札だけが選択可能になります） |
| **次のハンド** ボタン | 精算を読んだあと、次のハンドへ進みます |
| **ヒント** トグル | 推奨する手をハイライトします。ビッド中は推奨する申告枚数（パス推奨ならパス）のボタン、プレイ中は推奨する手札に枠が付きます |
| **設定** パネル | CPU 難易度を変更してリセットします |
| **CLI** トグル | ターミナル風の操作に切り替えます |

## 画面構成

- **ヘッダー**: 切札（ビッドが終わるまで「未定」）と、両チームの **24 からの残り**
- **常時切札の並び**: 常時表示します。切札スートの札しか切札でないと思い込むのが、このゲームで最も多い誤解です
- **切札の総数**: 切札が決まると 13（赤）か 12（黒）を表示します。手で数え直す必要はありません
- **場**: 現在のトリック
- **プレイヤー**: チーム番号、手札の枚数、申告した枚数。CPU の手札は裏向きで枚数のみ
- **精算**: ハンドが終わると、どちらが何点消化したか（または 60-60 で流れたこと）を表示します
- **アクションログ**: 進行を後から追えます
