...
- **ヘッダー**: ラウンド数（最大 **12**、`KoiKoiMaxRounds`）・山札の残り枚数・こいこい回数
- **場**: 場札は **8枚**（`KoiKoiFieldSize`）。手札と同じ月の札をクリックして取ります
- **手札**: 各自 **8枚**（`KoiKoiHandSize`）。**2人対戦**（`KoiKoiPlayerCnt`）です
- **取り札**: `CapturedCategories` が光・タネ・タン・カスのカテゴリ別に並べるので、役の成立状況が一目で分かります
- **こいこい / あがり**: 役が成立したときの決断。`こいこい` は続行して得点を伸ばし、`あがり` はそこで確定します
