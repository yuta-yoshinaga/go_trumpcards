...
- **ヘッダー**: ラウンド数・山札の残り枚数・目標点
- **場**: 場札は **8枚**（`GoStopFieldSize`）から始まります。手札と同じ月の札をクリックして取ります
- **手札**: 各自 **10枚**（`GoStopHandSize`）。**2人対戦**（`GoStopPlayerCnt`）です
- **捕獲 / 得点**: 取り札の枚数と現在の役の点数
- **ゴー / ストップ**: 目標点に届いたときに出る決断。`ゴー` は続行して倍率を上げ、`ストップ` はそこで確定します
