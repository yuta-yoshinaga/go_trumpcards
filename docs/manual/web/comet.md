...
ブラウザで `http://localhost:8080` にアクセスし、ナビゲーションバーから「コメット」を選択します。
...
- **手札を選ぶ**: 出せる札だけが選べます。選べる札が 1 枚も無いときはパスするしかありません。
- **出す**: 選んだ札を出します。**K とコメットを出すと連なりが切れ、次の先頭をあなたが選べます。**
- **パス**: 出せる札が無いときだけ押せます。
- **次の局**: 集計を読んだら押して次へ進みます。
- **設定**: CPU難易度、席数（2〜5）、目標点（20〜200）、ヒントの ON/OFF を切り替えられます。

## 画面構成

- **ヘッダー**: ゲーム名・フェーズ表示・**局と伏せた死に手の枚数**・CLI 切り替え
- **中央**: 今の連なりと、**次に要るランク**
- **情報サイドバー**: 各席の手札枚数と通算得点、局の集計（上がり・残り札・出なかった K・コメットの罰点）
