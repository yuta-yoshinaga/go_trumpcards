...
ブラウザで `http://localhost:8080` を開き、ナビバーの「ラムシュ」を選択します。
...
```mermaid
flowchart TD
    A[リセット] --> B["10枚ずつ配る + 伏せ札2枚"]
    B --> C["カードをクリックして選び「カードを出す」"]
    C --> D["「次のトリック」で進む"]
    D --> E{10トリック終わった?}
    E -- No --> C
    E -- Yes --> F["最終トリックの獲得者が伏せ札を受け取る"]
    F --> G{1人が総取り?}
    G -- Yes --> H["Durchmarsch - 他の2人が120点失点"]
    G -- No --> I["最多得点者がその点数を失点 (同点なら全員)"]
    H --> J["「次のラウンド」"]
    I --> J
    J --> K{目標スコア到達?}
    K -- No --> B
...
1. 手札のカードをクリックして選びます（もう一度クリックで選択解除）
2. **「カードを出す」**を押します

リードスートに追随する義務があります。追随できない札を出そうとするとエラーが返ります。

### トリックとラウンドを進める

- 3人が出し終えたら**「次のトリック」**
- 10トリック終わったら**「次のラウンド」**（そのラウンドの失点が確定します）

### ヒント

設定パネルでヒントを有効にすると、手番のときに助言が出ます。**助言は「取らない」方向**で、
0点の札で降りられるとき、リードで安い札を出すべきとき、点札しか出せないときで理由が変わります。
...
- **情報バー（上部）**: ラウンド数、ディーラー、**切り札の説明**、**得点が罰点であること**
  - どちらも常に表示されます。スカート系のつもりで読むと逆に取るところなので、隠しません
- **現在のトリック**: 場に出ている札
- **あなたの手札**: クリックで選択。選んでから「カードを出す」
- **スコア表**: 取ったトリック数、**このラウンドで集めてしまった点**、累計スコア
  - ラウンド終了時、最多を取った席に **(最多・失点)** の印が付きます
  - 総取りした席には **(Durchmarsch)** の印が付きます
- **伏せ札**: ラウンドが終わるまで表示されません。最終トリックの獲得者が受け取る2枚なので、
  途中で中身が分かると終盤の判断が完全情報になってしまいます
