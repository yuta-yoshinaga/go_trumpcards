...
52 枚 2 組（104 枚）で遊ぶソリティアです。各スートの **K と A を 1 枚ずつ抜いて 8 つの組札の種札**にし、残り 96 枚を **12 列 × 8 枚**のタブローに配ります。タブローは組札を囲む輪として並び、**列の位置がそのまま規則になります**。
...
輪に並べたとき、それぞれの列が隣り合っている段しか使えない、という規則です。**1 回目の再配りで解けます。** 画面では列が「上・横・下」の 3 帯に分かれて並び、送れない組札は押せません。
...
- アンドゥとオートコンプリートを併用すると、昇順と降順の組札を同時に伸ばす戦略も狙えます。
