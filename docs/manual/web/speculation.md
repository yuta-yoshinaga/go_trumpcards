# スペキュレーション（Web版）遊び方

## ゲーム概要

18〜19世紀のイギリスで遊ばれた**純粋な競り（オークション）ゲーム**です。トリックテイキングも役の強さもありません。あるのは「その札は他人にとっていくらの値打ちがあるか」だけです。

全員が参加料をポットに出し、伏せ札を3枚ずつ受け取ります。もう1枚めくって**切り札スート**を決めたら、順番に自分の伏せ札を1枚ずつ表にしていきます。**切り札以外は何を出しても関係ありません。** 出た札がそれまでの最高切り札を上回ると、その持ち主に対して他家が「その札を買いたい」と値を付けます。持ち主は売っても断ってもかまいません。

全員がめくり終えた時点で最高の切り札を持っている人がポットを総取りします。

## 起動方法

```sh
go run ./cmd/trumpcards web   # CLI経由でサーバを起動
go run ./cmd/server           # サーバを直接起動
```

ブラウザで `http://localhost:8080/speculation` を開きます。

## ルール

### 基本ルール

- 52枚のトランプ（ジョーカーなし）を使用
- 参加は人間1人 + CPU 1〜7人（既定は4人卓）
- 各自に伏せ札3枚。さらに1枚めくって切り札スートを決める（この札は誰の手にもならない）
- 全員が参加料（既定10）をポットに出す
- 手番の席が伏せ札を1枚めくる
- **切り札で、かつそれまでの最高を上回った**とき、その札の持ち主が「最高札」を持つ
- 最高札が動くと競りが開く。買い手が値を付け、持ち主が売るか断るかを決める
- 全員がめくり終えたら、最高切り札の持ち主がポットを総取り

### 札の強さ

**Aは14**として扱い、Kより強くなります。1のままだと最強の札が最弱になってしまいます。

| 札 | 強さ |
|----|------|
| A | 14（最強） |
| K | 13 |
| Q | 12 |
| J | 11 |
| 10〜2 | 数字どおり |

**切り札スート以外の札には強さがありません。** 何枚出ても競りの対象になりません。

### 覚えておくと違う3つの扱い

| 状況 | この実装の扱い | なぜ |
|------|---------------|------|
| 参加料を払えない席がある | **席は残り、出せるだけ出す** | 席を外すと座席番号がずれ、ラウンドを跨いだ集計が崩れる |
| 切り札が1枚も出なかった | **参加料を全員に返す** | 繰り越すと、降りた席の負担だけが積み上がる |
| リセットを続けて呼んだ | **未精算のポットを先に返してから集め直す** | 集め直すだけだとチップが消える（実測: 200→190→180でポットは40のまま） |

## ゲームの流れ

```mermaid
flowchart TD
    A[開始] --> B[全員が参加料を出す<br/>伏せ札3枚ずつ配る]
    B --> C[1枚めくって切り札スートを決める]
    C --> D[めくりフェーズ]
    D --> E["f で手番の席が1枚めくる"]
    E --> F{切り札で最高を上回った?}
    F -->|いいえ| G{全員めくり終えた?}
    F -->|はい| H[その席が最高札を持つ<br/>競りフェーズへ]
    H --> I{あなたは売り手? 買い手?}
    I -->|売り手| J["a で売る / d で断る"]
    I -->|買い手| K["a で買う / bid 額 で上乗せ / d で見送る"]
    J --> G
    K --> G
    G -->|いいえ| D
    G -->|はい| L{切り札が出た?}
    L -->|はい| M[最高札の持ち主がポットを総取り]
    L -->|いいえ| N[参加料を全員に返す]
    M --> O{規定ラウンド消化?}
    N --> O
    O -->|いいえ| P["next で次のラウンド"]
    P --> B
    O -->|はい| Q[終了・最終チップで勝敗]
```

## 画面の操作方法

| 操作 | 説明 |
|------|------|
| めくるボタン | 手番の席の伏せ札を1枚めくる |
| 受けるボタン | 競りの申し出を受ける（あなたが売り手なら売る、買い手なら買う） |
| 断るボタン | 競りの申し出を断る。札もチップも動かない |
| 上乗せ入力 + 買うボタン | 提示額を超える額を入れて買う。**下回る額は受け付けません** |
| 次のラウンド | ラウンド決着後に次を始める |
| リセットボタン | ゲームを最初からやり直す |
| ヒントのトグル | めくり時と競り時の両方で助言を表示 |
| CLIモード | 画面上でコマンド入力に切り替える（`f`, `a`, `d`, `bid <額>`, `next`, `h`, `log`, `r`） |

## 画面構成

- **上部**: ラウンド数、ポット、切り札スート、現在のフェーズ
- **座席一覧**: 各席の名前・チップ・**伏せ札の枚数**。手番の席と最高札を持つ席が強調されます。**伏せ札の中身は表示されません** —— サーバも送っていません。見えていたら競りでいくら出すべきかが分かってしまい、賭けが賭けでなくなります
- **最高札**: 現在いちばん強い切り札と、それを持っている席
- **競りパネル**: 申し出が出ているときだけ表示。誰がいくらで買おうとしているか（またはいくらで譲るか）と、受ける・断る・上乗せの操作
- **下部**: フェーズに応じたボタン（めくる / 競りの応答 / 次のラウンド / リセット）
- **結果表示**: ポットを取った席、または切り札が出ずに参加料が返ったこと
- **アクションログ**: そのラウンドの操作履歴

