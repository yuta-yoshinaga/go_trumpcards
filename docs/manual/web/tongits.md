...
2人（プレイヤー1人 + CPU 1人）で遊ぶ短時間ラミー系カードゲームです。手札5枚でメルド（セットやラン）を作り、デッドウッド（メルドに含まれないカード）の合計点を減らします。配牌時に49点・50点なら「Tongits」即勝利。先にポイント上限（デフォルト50点）に達したプレイヤーが勝利します。
...
ブラウザで `http://localhost:8080` にアクセスし、ナビゲーションバーから「トンギッツ」を選択します。
...
- 配布直後、いずれかの手札合計が49点または50点なら「Tongits」即勝利（手札合計 + 50点ボーナス）
...
- 配布5枚の合計点が49点または50点なら、Tongitsを宣言して即ラウンド勝利
- スコア = 50点ボーナス + 手札合計
...
flowchart TD
    ...
    B --> C[カード配布 - 各5枚]
    C --> D{配牌Tongits = 49 or 50?}
    D -->|はい| K[ラウンド終了 - Tongitsボーナス]
    D -->|いいえ| E{プレイヤーの手番?}
...
2. 配牌Tongitsが成立した場合は即ラウンド終了し、メッセージが表示される
...
- 5枚という少ない手札なので、配布直後の判定（Tongits可能性、メルド形成）が重要
