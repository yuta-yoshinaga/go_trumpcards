# Tablanet / Tablić タブラネット（タブリッチ）（Web版）遊び方

## ゲーム概要

Tablanet（タブラネット / Tablić）は、バルカン半島（セルビア・マケドニアなど）で親しまれている**フィッシング（釣り）系のカードゲーム**です。本実装は 4 人（あなた＝席0＋CPU3人）で個人戦としてプレイします。各プレイヤーは手札を 1 枚ずつ出し、**場札（テーブルの表向きの札）を釣り上げて（capture）**取り札を増やします。数札は**同じランク**や**合計値が一致する組み合わせ**をまとめて取れ、ジャック（J）は場を一掃します。1 枚の非ジャック札で場札を全て取り切ると **"Tabla"（タブラ）** となり大きなボーナスになります。山札を配り切った後、取り札の内容で得点を計算し、最も高いプレイヤーが勝ちます。
...
ブラウザで `/tablanet` にアクセスします。ナビバーの JA/EN ボタンで日本語・英語を切り替えられます。
...
- 取った札が最も多いプレイヤー（単独）= **+3**
- ダイヤの10（10♦）= **+2**、クラブの2（2♣）= **+1**
- エース（A）1枚につき **+1**、ジャック（J）1枚につき **+1**
- Tabla 1回につき **+1**
...
```
┌────────────────────────────────────────────┐
│ Tablanet (タブラネット)  Phase = Play  Deck 32│ ← ヘッダー
├──────────────┬─────────────────────────────┤
│ P1 (CPU)     │                             │
│ Captured 4   │      場札（Table）           │
│ P2 (CPU)     │   ♠7  ♥3  ♦A  ♣10           │
...
- **情報サイドバー**: 各プレイヤーの取り札枚数（Captured）・Tabla 回数。
...
- 場札が少ないときは、非ジャックの 1 枚で全て取り切って **Tabla（+1）** を狙いましょう。
- ジャックはいつでも場を一掃できます（Tabla にはなりません）。相手に取られそうな場札の回収に有効です。
- 10♦（+2）、2♣（+1）、各エース（A、+1）、各ジャック（J、+1）、取り札の総数（最多で +3）が得点源です。これらを含む場札は優先的に釣りましょう。
