...
ディーラーの左隣から順に、全員が一度ずつ宣言かパスをします。**先に言った人が取るのではなく、最も順位の高い契約が取ります**（Solo > Wenz > Rufspiel）。同位なら先に言った席が残ります。

既に出ている宣言を上回れない契約は**ボタン自体が出ません** — 押せるのに必ず拒否される操作面を作らないためです。全員がパスした場合は、ディーラーの左隣が Rufspiel を引き受けます。

開幕の配りではあなたが最初に話すので、3契約すべてのボタンが並びます。
...
flowchart TD
    ...
    B --> C[PICKフェーズ - 全員が一度ずつ宣言かパス]
    C --> P{全員が発言した?}
    P -- いいえ --> C
    P -- はい --> Q[最上位の契約が落札]
    Q --> D{落札は Rufspiel?}
    D -- はい --> E[CALLフェーズ - スートを選んでコールボタン]
    D -- いいえ Wenz/Solo --> F[PLAYフェーズ - 単独で戦う]
    E --> F
    F --> I{トリック完了?}
    I -- いいえ --> F
    I -- はい --> J[次のトリックボタン]
...
- **「パスする」ボタン**: 宣言せずに見送ります。全員がパスすると、ディーラーの左隣が Rufspiel を引き受けます。

宣言ボタンは、既に出ている宣言を**上回れるものだけ**が並びます。先に Solo が出ていれば残るのは「パスする」だけです。
...
- **競りの読み**: Solo は最上位なので、宣言すれば必ず落札できます。逆に先に Solo が出ていると、あなたに残る選択はパスだけです。
