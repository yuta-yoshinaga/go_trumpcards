...
ブラウザで `http://localhost:8080` にアクセスし、ナビゲーションバーから「ポッホ」を選択します。
...
| 操作 | 説明 |
|------|------|
| **賭ける** ボタン | pochen で 1 単位出します |
| **降りる** ボタン | pochen から抜けます |
| 手札のカードをクリック | ストップスで出す 1 枚を選びます |
...
- **ヘッダー**: ディール数、めくり札
- **ルール表示**: 常時表示します。「pay suit の札でしか区画は取れない」と「pochen は宣言ではなく組の比べ合い」が最も誤解されやすい 2 点です
- **盤**: 9 区画とそれぞれのチップ。**取られなかった区画は持ち越される**ので、数字が膨らんでいる区画が狙いどころです
- **第 1 段階の結果**: 誰がどの区画を取ったか。自動で解決するので、ここが唯一の手がかりです
- **場に出た札**: ストップスの並び。止まっているときは好きな札から始められます
- **プレイヤー**: チップ、手札枚数、賭け額、降りたかどうか
