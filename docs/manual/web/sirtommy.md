...
現存する記録の中で最古級とされるイギリス発祥の一人用ペイシェンス。「ソリティアの祖父」とも呼ばれる。山札を 1 枚ずつめくって 4 つのウェイスト（捨て札列）へ振り分けながら、4 つの組札を A から K まで積み上げる。
...
- 山札の最上段を、組札（A のときのみ新規に開ける／組札の次のランクのとき）かウェイストのいずれかに置く
...
| 組札をクリック | 山札またはウェイスト最上段の選択中カードを組札に置く |
...
- **中央上段**: 4 つの組札。積み上がった枚数を `n/13` で表示
