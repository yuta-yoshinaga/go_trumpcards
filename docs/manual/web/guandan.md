...
3. ゲーム一覧から **掼蛋** を選ぶか、`/guandan` へ直接アクセスします
...
```mermaid
flowchart TD
    A[108枚を4人に27枚ずつ] --> B{2局目以降か}
    B -->|初局| E[プレイ開始]
    B -->|2局目以降| C{払う側が赤ジョーカー2枚を持つか}
    C -->|持つ| D[抗貢 貢は流れる]
    C -->|持たない| F[最下位がワイルドを除く最強を1着へ]
    F --> G[受け取った側が1枚返す]
    D --> E
    G --> E
    E --> H[リードした役に 同形でより強いか ボムを被せる]
    H --> I{残り全員がパスしたか}
    I -->|はい| J[場が流れ 最後に出した人がリード]
    I -->|いいえ| H
    J --> K{3人が上がったか}
    K -->|いいえ| H
    K -->|はい| L{1着と同じチームの2人目は何着か}
    L -->|2着| M[4段階昇級]
    L -->|3着| N[2段階昇級]
    L -->|4着| O[1段階昇級]
    M --> P{Aを超えたか}
...
| 操作 | 説明 |
|------|------|
| 手札のカードをクリック | 役に使う札を選びます。**複数枚選べます**。もう一度押すと外れます |
| **出す** ボタン | 選んだ札を 1 つの役として出します。何も選んでいないと押せません |
| **パス** ボタン | パスします。**場が流れているときは押せません**（リードは必ず何か出します） |
| **還貢する** ボタン | 貢の返礼に渡す札を選んで返します。**1 枚だけ**選んだときに押せます |
| **次の局へ** ボタン | 局の精算を確認してから次の局を配ります |
| **CLI** トグル | ターミナル風の操作に切り替えます |

## 画面構成

- **情報**: 局番号、この局のレベル、両チームのレベル。**レベル札が A より強いこと**と**昇級量 1 / 2 / 4** もここに出ます
- **プレイヤー**: 各席のチーム、残り枚数、**上がった着順**。中身は誰のものも見えません
- **場**: いま出ている役と枚数、出した席。流れているときはその旨が出ます
- **進貢・還貢**: 誰が誰に何を渡したか、返礼が済んでいるか。**抗貢で流れた場合は理由**が出ます
- **局の精算**: どちらのチームが何段階上がったか。**1 着 2 着の独占は別表示**です
- **アクションログ**: 進行を後から追えます
