...
ブラウザで `http://localhost:8080` にアクセスし、ナビゲーションバーから「シートグッベ」を選択します。
...
- **手札**: 自分の手札。出せる札だけが選択可能です。**出せない札にはその理由**（`場の札を上回れません` / `いまは自分の番ではありません`）が付き、読み上げでも分かります。CPU の手札は裏向きで枚数のみ表示されます
- **アクションログ**: 進行を後から追えます
