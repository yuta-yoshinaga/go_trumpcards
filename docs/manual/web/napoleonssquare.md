...
2 デッキ 104 枚を使う大型ソリティア。中央に 8 つの組札（スートごとに 2 つ）、その周囲を 12 列×4 枚のタブローが正方形に囲む。残りは山札で、1 枚ずつめくって使う。
...
- 52 枚 2 組（104 枚）。8 枚のエースが組札に置かれ、48 枚が 12 列×4 枚に配られる。残り 48 枚が山札
- 組札は**同じスートで昇順** A→K。スートごとに 2 列あり、2 組目のエースが 2 列目を開く
