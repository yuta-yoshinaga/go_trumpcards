...
52 枚 2 組（104 枚）で遊ぶイギリス発祥のソリティア。中央に 8 つの組札を置き、その左右に **4 つずつ計 8 つのタブロー山**を並べる。タブローには**最初 1 枚ずつ**しか配らず、残り 96 枚が山札になる。

組札は**最初は空**で、A が出てきたときにそこへ置き、同スートで K まで 13 枚積む。8×13 = 104 枚すべてを積み切ればクリア。
...
- **組札**: 8 つ（スートごとに 2 つ）。**最初は空**で、A が出たら置く。以降は同スートで昇順、K で完成（13 枚）
...
> (1) **8 枚の A を最初から組札に並べるのではない**。組札は空で始まり、A が出た順に置く（並べてしまうと山札から A を探し出す必要があり、別のゲームになる）。
> (2) **残り 96 枚はタブローではなく山札**。issue の配り方だと山札が 0 枚になり、同じ issue の規則 5「山札をめくる」と矛盾する。
> (3) 組札は「各スート 8 枚まで」ではなく **8 つの山に 13 枚ずつ**（前者は 32 枚にしかならず 104 枚を吸収できない）。
