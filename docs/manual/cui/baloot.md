...
**相手の Baloot は伏せられます。**自分のぶんは配られた時点で出ますが、他の席は `Baloot: 不明` のままで、その席が**切り札の K か Q を実際に出した時点**（またはラウンド終了）で初めて `Baloot(K+Q)=20点` / `役なし` に変わります。
...
CPU1[T1]: Baloot: 不明 獲得0 手札8枚
CPU2[T0]: Baloot: 不明 獲得0 手札8枚
CPU3[T1]: Baloot: 不明 獲得0 手札8枚
----------
手番: あなた
play <idx>・・・カードを出す
```
