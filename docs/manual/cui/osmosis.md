...
オズモシス（Osmosis、別名 Treasure Trove）は1人用ソリティア。4列×4枚のリザーブ、縦4段の組札、ストックとウェイストを使い、52枚すべてを組札に積み上げることを目指す。連番ではなく「上の段に置かれたランクが下の段に浸透していく」という独特のルールを持つ。
...
- 各組札段は1スート専用。最初に配られた1枚のランクが「ベースランク」となる。
...
```mermaid
flowchart TD
    A[ゲーム開始 - reset] --> B[初期配布 リザーブ4列x4 組札1枚 残りストック]
    B --> C{操作選択}
    C -->|d| D[ストックから1枚引く]
...
|----------|--------|------|
| `d` / `draw` | `d` | ストックから1枚引く |
| `m w f <seg>` | — | ウェイストから組札（seg段目）へ |
| `m r <col> f <seg>` | — | リザーブ（col列）から組札（seg段目）へ |
| `g` / `giveup` | `g` | ギブアップ |
| `hint` | `h` | ヒント |
...
- **組札（組札）**: 縦4段。各段1スート。1段目は順不同、下段は上段からの浸透のみ
