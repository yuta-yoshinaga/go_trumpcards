...
```
==========
カッコー (Cuckoo / Chase the Ace)
==========
ラウンド: 1  山札: 48
あなた [親]: ライフ 3  カード: DIAMOND 5
CPU 1: ライフ 3  カード: 非公開
CPU 2: ライフ 3  カード: 非公開
CPU 3: ライフ 3  カード: 非公開
----------
手番: あなた
k / keep ・・・保持／s / swap ・・・山札から新しいカードを引く（親の特権）
==========
```

- **ラウンド行**: `ラウンド: N  山札: M` — ラウンド数と山札の枚数。
  **親はこの行ではなく、席の名前の後ろに `[親]`** として出ます。
- **プレイヤー行**: `あなた [親]: ライフ 3  カード: DIAMOND 5` —
  ライフは**数値**（♥ の並びではありません）。他家の札は `非公開`。
- **手番表示**: `手番: あなた` の行で示します（`<- turn` の印ではありません）。
- **操作行**: `k / keep`（保持）と `s / swap`（山札から引く、**親の特権**）。
