...
ディーラーの左隣から順に、全員が一度ずつ宣言かパスをします。**先に言った人が取るのではなく、最も順位の高い契約が取ります**（Solo > Wenz > Rufspiel）。同位なら先に言った席が残るので、既に出ている宣言を上回れない契約は宣言できません — 画面には宣言できる契約だけが案内されます。

全員がパスした場合は、ディーラーの左隣が Rufspiel を引き受けます（配り直しはしません）。

開幕の配りではあなたが最初に話すので、3契約すべてから選べます。
...
```mermaid
flowchart TD
    A[ゲーム開始 - reset] --> B[PICKフェーズ - 全員が一度ずつ発言]
    B --> C{全員が発言した?}
    C -- いいえ --> B
    C -- はい --> E{誰か宣言した?}
    E -- はい --> F[最上位の契約が落札]
    E -- いいえ --> G[ディーラーの左隣が Rufspiel を引き受ける]
    G --> D{Rufspiel?}
    F --> D
...
既に出ている宣言を上回れない契約は拒否されます。画面の「宣言できる契約」行に、その時点で打てるものだけが出ます。
...
----------
Youの手番 ― 契約を宣言しますか？
宣言できる契約: Rufspiel（A呼び） / Wenz（Unterのみ切り札） / Solo（スート指定）
...
- **パス人数行**: 競りが終わるまで表示されます
- **宣言できる契約行**: 既に出ている宣言を上回れるものだけ。打てば必ず拒否されるコマンドは案内しません
- 相棒は呼ばれたAが出た瞬間に正体が公開されます

## 遊び方のコツ

- **契約の選び方**: Q・Jが3枚以上あれば Rufspiel が有利。Jを3枚以上握っているなら Wenz、1スートに5枚以上偏っているなら Solo が候補です。
- **競りの読み**: Solo は最上位なので、宣言すれば必ず落札できます。逆に先に Solo が出ていると、あなたはパスするしかありません。
