...
```sh
go run ./cmd/trumpcards popejoan
go run ./cmd/trumpcards --lang en popejoan   # 英語表示
```
...
```
ディール1/5 · めくり札 SPADE 5
区画はトランプの札でしか取れません（Pope=♦9 だけは例外）／♦8 が抜いてあるので ♦7 の次は必ず止まります
盤: A:1 K:1 Q:1 J:1 ゲーム:1 ポープ:6 マトリモニー:2 イントリーグ:2
あなた: チップ0 手札13枚
  [0]SPADE 1 [1]HEART 11 [2]CLOVER 5 ...
CPU 1: チップ-15 手札13枚
```

- **盤の 8 区画は毎回すべて出ます。**膨らんでいる区画がそのディールの狙いどころです
- **めくり札で区画が動いた場合は、その旨が別行で出ます**
- **手札の枚数は全員ぶん公開**されます。出し切られると 1 枚につき 1 チップ払うので、そのまま負債額です
- CPU の**手札の中身**は伏せられます
- **`[Pope保持]`**: ♦9 を抱えている席に付きます。**伏せ手の CPU にも付きます** — その人は出し切られたときの支払いを免除されるので、卓全体に関わる公開情報です
