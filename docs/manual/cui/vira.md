...
ポット: 3
あなた (ディフェンダー): 13枚  得点: -1  トリック: 0
[0]SPADE 5  [1]SPADE 6  [2]SPADE 10  [3]CLOVER 9  [4]CLOVER 10  [5]HEART 6  ...
CPU 1 (ディフェンダー): 13枚  得点: -1  トリック: 0
CPU 2 (ディフェンダー): 13枚  得点: -1  トリック: 0
----------
入札してください (0=パス, 1=ガスク, 2=ソロ, 3=ミゼール, 4=ヴィーラ)
bid <0-4>・・・0=パス, 1=ガスク(7トリック), 2=ソロ(8トリック), 3=ミゼール(0トリック・切り札なし), 4=ヴィーラ(10トリック)／パス以外は現在の入札を上回る必要ありてください。
play <idx>
```

- **1 行目**: `ラウンド: N  トリック: M  切り札: なし` — 切り札は**同じ行**
  (宣言前は `なし`)。`フェーズ: BID` / `フェーズ: PLAY` の行は出ません。
- **`ポット:` 行**: いまのポット。**達成した宣言者の総取り**なので入札判断に効きます。
- **プレイヤー行**: `あなた (ディフェンダー): 13枚  得点: -1  トリック: 0` —
  役割・手札枚数・累積得点・獲得トリック数。`ゲーム点:` という行はありません。
