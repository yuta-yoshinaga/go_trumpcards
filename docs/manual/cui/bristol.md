...
ブリストル（Bristol）は1人用ソリティア。8列のタブロー（場札）、3つのファン（予備列）、ストック、4つの組札を使い、52枚すべてを組札に積み上げることを目指す。タブローはスートに縛られず降順に積めるのが特徴。
...
- 組札は4つ。AからKへ昇順に積む（スート不問）。タブローやファンのトップから積める。
- 4つの組札すべてが13枚（合計52枚）になればクリア。
...
- **手詰まりの警告**: どこにも置けなくなると赤字で表示され、続けて「何回 `u`（元に戻す）すれば打てる盤面に戻れるか」が出ます。ブリストルはストックを作り直せないので、この状態には普通に到達します
...
- 行き詰まったら `u`（元に戻す）で読み直す。手詰まりになった場合は、戻すべき回数が画面に出ます。
