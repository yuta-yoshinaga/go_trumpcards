...
| `ds` |  | 山札から 1 枚引く |
| `dd` |  | 捨て札の一番上を取る |
| `meld <i,j,k>` | `m` | 手札の `i,j,k` をメルドとして出す（カンマ区切り） |
| `layoff <i> <m>` | `o` | 手札 `i` をメルド `m` に付ける（**他家のメルドに付けた札はその人のものになり、自分の 10 枚には数えません**） |
| `desmoche <from> <i> <to>` | `x` | メルド `from` の `i` 枚目をメルド `to` へ移す（desmoche） |
| `discard <i>` | `d` | 手札 `i` を捨てる |
| `next` | `n` | 次のラウンドへ進む |
| `hint` | `h` | ヒントを表示する |
| `log` | `l` | 棋譜を表示する |
| `reset` | `r` | ゲームごとリセットする |
