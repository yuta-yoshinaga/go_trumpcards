...
- ダウト判定ウィンドウの長さは、Web版では設定パネルから変更できますが、CUI版ではコード上の `DoubtConfig.DoubtWindowSec` のデフォルト値（10秒）が使用されます
...
- 記憶の精度（保持確率）はコード上の `DoubtConfig.CpuMemoryLevel` で変更可能:
...
- CUI版ではコード上の `DoubtConfig.PenaltyDrawLimit` で変更可能
- Web版では設定パネルから変更できる
