...
- カードが場に出された後、**10秒間**のダウト判定ウィンドウがある
- CPU がカードを出した場合: 10秒以内に `d`（ダウト）または Enter（スルー）を入力する
...
- `[ダウト]`: ダウト判定の結果（嘘/正直、引き取り枚数、公開カード）
...
```
ダウト！と言いますか？ (d / doubt → ダウト、Enter でスキップ) [10秒]
```

- `d` または `doubt` を入力してEnter → ダウトを宣言
- Enter のみ（または何も入力せず10秒経過） → スルー（ダウトしない）
