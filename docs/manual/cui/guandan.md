...
| `next` | `n` | 次の局へ進む |
| `check <添字...>` | `ch` | **出さずに**役だけ調べる（役名・枚数・場に勝てるか） |
| `log` | `l` | 棋譜を表示する |
| `reset` | `r` | ゲームごとリセットする |
| `quit` | `q` | 終了する |
| `help` | `?` | コマンド一覧を表示 |
...
- **役に自信が無いときは `ch` で下読みできます。**`ch 0 1 2` は手札を動かさずに「役: ボム (4枚)【通常役はすべて上回ります】【場の役に勝ちます】」のように答えます。ワイルド（レベル札の♥）を混ぜた組み合わせも判定できます
