...
| `deal` |  | 自分が親の局を配る |
| `stick` | `s` | スティック（合計 15 以上でのみ） |
| `twist` | `t` | ツイスト（1 枚引く・賭け金は増えない） |
| `buy <額>` |  | バイ（賭け金を足して 1 枚引く・ツイスト後は不可） |
| `split` | `sp` | スプリット（同ランク 2 枚を分ける） |
| `bt` |  | 親として 1 枚引く |
| `bs` |  | 親として引くのをやめて精算する |
...
- その下に**相手の停止ライン**が出る。親は 17 以上になるまで引き、CPU 席は 15 で止まる（4 枚のときだけ 17 未満でももう 1 枚引く）

## 遊び方のコツ

- **14 以下では止まれない。** 12〜14 は最も苦しい形で、引けばバーストしやすく、引かなければ宣言できない
- **Buy は最初の 1 枚に使う。** 一度 Twist すると二度と買えないので、伸ばす気があるうちに賭けておく
- **相手がどこで止まるかを見る。** 親は 17 まで引くので 17〜18 では足りないことが多く、CPU 席は 15 で止まるので競り勝つのは楽
