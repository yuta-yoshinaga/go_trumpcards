...
1人用のソリティアカードゲームです。1デッキ（52枚）を使用し、開始時にすべてのカードを表向きで配ります。9列のオープンなタブロー（列0は1枚、列1は2枚……列8は9枚＝計45枚）に加え、**「ローレンス」と呼ばれる7枚のリザーブ**が1枚ずつ表向きで並びます。組札は最初は空です。リザーブのカードはタブローや組札へ出せますが、**一度出すと二度とリザーブには戻せません（一方通行）**。4つの組札へA→Kの順にすべて積み上げきればクリアです。
