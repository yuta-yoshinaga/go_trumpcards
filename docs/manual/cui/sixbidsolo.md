...
低い順に並びます。**上回る宣言だけが通ります。**入札中の画面にも `ビッドの序列:` として同じ 6 段階が必要点数付きで出るので、この表を開き直さずに選べます（切札が決まるのは入札後なので、ギャランティーだけは `♥74点/他80点` と両方出ます）。
...
| コマンド | 短縮形 | 説明 |
|----------|--------|------|
| `bid <1-6>` | `b` | 宣言する（**1=ソロ 2=ハートソロ 3=ミゼール 4=ギャランティー 5=スプレッド 6=コール**） |
| `pass` | `ps` | 宣言を見送る |
| `declare <1-4>` | `d` | 切札を指定する（**1=♠ 2=♣ 3=♥ 4=♦**） |
| `declare <1-4> <1-4> <1-13>` | `d` | コール・ソロ: 切札に続けて**交換を求める札**を指定する |
| `play <i>` | `p` | 手札 `i` を出す |
| `next` | `n` | 次の局へ進む |
| `log` | `l` | 棋譜を表示する |
| `reset` | `r` | ゲームごとリセットする |
