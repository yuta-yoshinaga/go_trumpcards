...
|----------|--------|------|
| `reset` | `r` | 新しいゲームを開始 |
| `bet <amount> <type> [<playerPair>] [<bankerPair>]` | `b <amount> <type> [<playerPair>] [<bankerPair>]` | ベットする（金額とタイプを指定。ペアベット額は省略可、省略時は 0） |
...
- `r` → ゲームリセット
- `b 100 0` → 100チップをプレイヤーにベット（ペアベットなし）
- `b 200 1` → 200チップをバンカーにベット（ペアベットなし）
- `b 50 2` → 50チップをタイにベット
- `b 100 0 50 30` → 100チップをプレイヤーにベット＋プレイヤーペアに50・バンカーペアに30ベット
- `b 100 1 0 20` → 100チップをバンカーにベット＋バンカーペアに20ベット（プレイヤーペアは省略 → 0）
- `l` → 棋譜を表示
