...
| コマンド | 短縮形 | 説明 |
|----------|--------|------|
| `move <col> a` | `m` | 列 `<col>` の最上段を昇順組札 (A→K) に置く |
| `move <col> k` | `m` | 列 `<col>` の最上段を降順組札 (K→A) に置く |
| `move <fromCol> <toCol>` | `m` | 列 `<fromCol>` の最上段を列 `<toCol>` に重ねる（同スート±1） |
| `hint` | `h` | ヒントを表示する |
...
```
昇順組札 (A→K): ♠A | ♣A | ♥3 | ♦A     ← スート固定の 4 列
降順組札 (K→A): [空] | [空] | ♥K | [空]  ← キングを上げると開く
----------
