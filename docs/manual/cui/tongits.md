...
2人（プレイヤー1人 + CPU 1人）で遊ぶ短時間ラミー系カードゲームです。手札5枚でメルド（セットやラン）を作り、デッドウッドの少なさを競います。配牌時に49点・50点なら「Tongits」即勝利。先にポイント上限（デフォルト50点）に達したプレイヤーが勝利します。
...
flowchart TD
    ...
    B --> C{配牌Tongits = 49 or 50?}
    C -->|はい| L[ラウンド終了 - Tongitsボーナス]
    C -->|いいえ| D[捨て札の山に1枚表向き]
    D --> E{プレイヤーの手番?}
