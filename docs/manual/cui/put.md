...
Put（プット）は南米・スペインで人気の 2 人対戦（あなた vs CPU）トリックテイキング＋ブラフゲームです。40 枚デッキ（8・9・10 を除く）を使い、3 枚配って 3 バサ（トリック）を行い、2 バサ先取でそのマノ（ハンド）に勝利します。プレイ中に「Put」を宣言して賭け点を釣り上げる心理戦が最大の特徴で、設定点（既定 15 点）に先に到達したプレイヤーがマッチに勝利します。
...
自分の手番で `put` を宣言すると賭け点を引き上げられます。相手は受諾（Quiero）・拒否（No Quiero）・さらに引き上げ（Reput → Vale Cuatro）を選べます。
