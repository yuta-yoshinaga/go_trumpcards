...
```
==========
トゥエンティナイン (Twenty-Nine / 29)
==========
ラウンド: 1  トリック: 1  切り札: 非公開
ゲーム点 — チームA: 0  チームB: 0
今ラウンド — チームA: 0  チームB: 0
あなた（チームA・ディフェンダー）: 8枚  ゲーム点: 0  トリック: 0
[0]SPADE 11  [1]SPADE 13  [2]CLOVER 9  [3]CLOVER 1  [4]HEART 9  [5]HEART 1  ...
CPU 1（チームB・ディフェンダー）: 8枚  ゲーム点: 0  トリック: 0
CPU 2（チームA・ディフェンダー）: 8枚  ゲーム点: 0  トリック: 0
CPU 3（チームB・ディフェンダー）: 8枚  ゲーム点: 0  トリック: 0
----------
入札手番: あなた（現在の最高入札: 20）
bid <0/16/20/24/28>・・・0=パス／パス以外は現在の入札を上回る必要あり。落札チームは切り札を非公開で選ぶ。
==========

契約: チームA 10/20 点 — あと10点
フェーズ: PLAY
あなたの手番です。カードを出してください。
play <idx>
```

- **タイトル行**: `トゥエンティナイン (Twenty-Nine / 29)` — 日本語が先です。
- **1 行目**: `ラウンド: N  トリック: M  切り札: 非公開` — 切り札は**同じ行**。
  落札チームが非公開で選ぶので、公開されるまで `非公開` のままです。
- **`ゲーム点 —` / `今ラウンド —` の 2 行**: 累積と、このラウンド分。
  **どちらで稼いだかが分かれて出ます**。
- **プレイヤー行**: `あなた（チームA・ディフェンダー）: 8枚  ゲーム点: 0  トリック: 0`。
- **`入札手番: あなた（現在の最高入札: 20）`**: **いま上回るべき額**が括弧に出ます。
- **`[n]`付き行**: あなたの手札（インデックス付き）。
- **契約行**: PLAY／トリック終了フェーズで表示。入札チームの現在点／宣言点と、`成立` / `あと{{N}}点` / `不成立が確定` のいずれか。場に残る点（合計29点から両チームの取得点を引いた分）を全部取っても届かなくなった時点で「不成立が確定」になります
