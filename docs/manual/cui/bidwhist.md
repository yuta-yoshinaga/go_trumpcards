...
| コマンド | 短縮形 | 説明 |
|----------|--------|------|
| `bid <tricks> <dir>` | `b` | ビッド（tricks 1-7, dir 0=Uptown 1=Downtown 2=NoTrump） |
| `pass` | `pa` | パス |
| `trump <suit>` | `t` | 切り札スート宣言（1=♠ 2=♣ 3=♥ 4=♦） |
| `exchange <i1> <i2> <i3> <i4> <i5> <i6>` | `e` | キティ交換（捨てるカード6枚のインデックス） |
| `play <i>` | `p` | カードをプレイ |
| `next` | `n` | 次のトリックへ |
...
- **キティ交換フェーズで末尾に `+` が付くカード**: キティから入ってきた6枚。捨てる6枚を選ぶときの目印です（合法手を示す `*` とは別の記号）。落札者が自分のときだけ、そのフェーズでのみ表示されます
