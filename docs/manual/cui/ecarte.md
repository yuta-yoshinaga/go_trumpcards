...
Écarté (エカルテ)
==========
ディール: 1  トリック: 1  山札: 21枚  (プレイフェーズ)
切り札表示カード: HEART 9
累積得点: あなた=0  CPU=0
あなた: 獲得0トリック ディール0点 累積0点 5枚
[0]SPADE 10  [1]SPADE 13  [2]DIAMOND 10  [3]HEART 10  [4]HEART 1
CPU 1: 獲得0トリック ディール0点 累積0点 4枚
----------
トリック: CPU 1=CLOVER 13
手番: あなた
play <idx>・・・カードを出す
```

- **1 行目**: ディール番号・トリック番号・山札の残り枚数と、**いまが
  どのフェーズか**が括弧書きで付きます (`(プレイフェーズ)` など)。
  `フェーズ: EXCHANGE` のような独立した行は出ません。
- **`切り札表示カード:`**: 切り札を決めた表向きのカード。
- **`累積得点:`**: 両プレイヤーの累積スコア（目標点に先着で勝利）。
