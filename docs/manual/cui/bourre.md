...
| コマンド | 短縮形 | 説明 |
|----------|--------|------|
| `decide [0-1]` | `d` | 参加可否を決定 (1=参加, 0=フォールド) |
| `draw [idx...]` | `dr` | 不要なカードを捨ててドロー (空 = 交換なし) |
| `play [idx]` | `p` | 指定インデックスのカードをプレイ |
| `next` | `n` | 次のハンドへ進む |
...
- 参加可否を決めるあなたの手番では、参加して1トリックも取れなかった場合の罰金額 (ポット全額。手持ちチップが少ないときはその全額) が表示されます。降りれば罰金はありません。
