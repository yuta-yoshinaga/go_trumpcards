...
| `drawdiscard` | `dd` | 捨て札からカードを引く |
| `discard <idx>` | `d` | 手札のカードを捨てる |
| `knock` | `k` | ノックして勝負する |
| `nextround` | `nr` | 次のラウンドへ進む |
| `hint` | `h` | 推奨する手を表示（ドロー先／捨てる札／ノック） |
| `setdifficulty <0-2>` | `sd` | CPU難易度を変更 (0=易, 1=普通, 2=難) |
| `setlives <n>` | `sv` | 初期ライフを変更 (1〜10) |
...
- あなたの手札はインデックス付き (`[0] ♠1` など) で表示され、`d <idx>` の `<idx>` に対応します。
