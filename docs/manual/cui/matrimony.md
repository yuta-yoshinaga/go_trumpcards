...
2 組 104 枚を使う 1 人用ペイシェンスです。16 枚のタブローから札を組札へ送り、4 本の組札を13枚ずつ（計52枚）にすればクリアです。
...
- 組札は 4 本です。♠Q の 2 枚はそれぞれ降順、♦J の 2 枚はそれぞれ昇順に、**スートに従って**積みます。
  - ♠Q 始まり: Q→J→10→9→…→2→A→K
  - ♦J 始まり: J→Q→K→A→2→…→10
- 同名のもう 1 枚が出てきたら、それも同じスートの組札として置きます。組札はランクを一周するため、K の次は A（降順では A の次が K）、A の次は 2（昇順）です。
- タブロー上の、次に必要なランクとスートが一致する札を順に組札へ送ります。空いたタブローは山札または捨て札で補充できます。
- 山札を使い切ったら、捨て札を回収して配り直します。配り直しは最大 3 回です。
- 4 本の組札がすべて 13 枚ずつ（計 52 枚）になればクリアです。
...
```mermaid
flowchart TD
    A[リセット: 16枚をタブローへ配る] --> B{組札へ送れる札がある?}
    B -- はい --> C[タブローまたは捨て札を組札へ送る]
    C --> D{組札4本が13枚ずつ?}
    D -- はい --> E[ゲームクリア]
    D -- いいえ --> B
    B -- いいえ --> F{空きタブローがある?}
...
| `m t<slot> f` |  | タブロー枠 `<slot>` (0..15) の札を組札へ送る |
| `m w f` |  | 捨て札を組札へ送る |
| `m w t<slot>` |  | 捨て札で空きタブロー枠を補充する |
| `m s t<slot>` |  | 山札で空きタブロー枠を補充する |
| `autocomplete` | `ac` | 送れる札を自動で組札へ送る |
| `undo` | `u` | 直前の手を取り消す |
| `hint` | `h` | ヒントを表示する |
| `giveup` | `g` | ギブアップする |
| `log` | `l` | アクションログを表示する |
...
- 組札は開始札と方向を表示します。次に必要な札はスートも一致していなければ置けません。
- `[枠n]` は 1 枚だけ置けるタブロー枠です。空いた枠は山札または捨て札で補充します。
- 「配り直し」は、捨て札を回収して山札に戻した回数です。

## 遊び方のコツ

- 組札の進行をスートごとに追い、同じスートの 2 枚をどちらの方向へ送るかを見失わないようにします。
