...
----------
あなた (チームA): 手札5枚 / 捕獲0枚
[0]DIAMOND 11  [1]HEART 7(取れる: 1枚)  [2]DIAMOND 1  [3]DIAMOND 2  [4]CLOVER 6
CPU 1 (チームB): 手札5枚 / 捕獲0枚
CPU 2 (チームA): 手札5枚 / 捕獲0枚
CPU 3 (チームB): 手札5枚 / 捕獲0枚
----------
場: SPADE 12, CLOVER 7, CLOVER 3, CLOVER 13
手番: あなた
p <hand> で捕獲または場に置く / n で次のラウンド
```

- **チーム得点行**: `チームA: N点` / `チームB: N点` の 2 行。
- **チーム捕獲行**: `チームA 捕獲: N枚（21枚以上で +6）` の 2 行。
  **何枚で加点になるかがその場に書かれています**。
- **場行**: `場: SPADE 12, CLOVER 7, ...` — 場に出ているカード。
- **プレイヤー行**: 各プレイヤーの所属チーム・手札枚数・捕獲枚数。
- **手番表示**: 現在の手番のプレイヤー。
- **`(取れる: N枚)`**: あなたの手番のとき、その手札を出すと捕獲できる場札の枚数。同ランクの場札はまとめて取れるので、3枚以上ならロンダ（追加点）が狙えます。取れない札には何も付きません。
