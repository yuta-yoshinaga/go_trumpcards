...
- **組札**: 8 つ。0〜3 は A から昇順、4〜7 は K から降順。どちらも**同じスート**で積みます
...
| `m t <fromCol> f <foundationId>` | — | タブローから組札へ移動 |
| `redeal` | `rd` | 再配り（各タブロー列の順序を反転、最大 3 回） |
| `hint` | `h` | ヒントを表示 |
| `autocomplete` | `ac` | 残りすべてを組札に移動 |
| `undo` | `u` | 直前の操作を取り消し |
| `giveup` | `g` | ゲームを放棄 |
| `log` | `l` | これまでの操作ログを表示 |
| `quit` | `q` | ゲーム終了 |
...
St. Helena (セント・ヘレナ)
==========
組札 (昇順): [0]♠A | [1]♣A | [2]♥A | [3]♦A
組札 (降順): [4]♠K | [5]♣K | [6]♥K | [7]♦K
残り再配り回数: 2
