...
----------
初期カード2枚をめくってください (fi <pos>)
==========
```

- `ラウンド: N/9`: 現在のラウンド番号（全9ラウンド）
- `山札: N枚` / `捨て札: X`: 山札の残り枚数と捨て札の一番上
- `あなた (累計=0, R=0) <<`: 席名・累計スコア・このラウンドの得点。
  **末尾の `<<` が手番の印**です
- `[0]〜[5]`: グリッドの位置（0=上段左, 1=上段中, 2=上段右,
  3=下段左, 4=下段中, 5=下段右）。番号がそのまま `fi` / `sw` の引数です
- `??`: 裏向きカード
- `0+? pt 0+? pt 0+? pt`: **列ごとの得点**。3 列それぞれについて
  「確定している分 + まだ伏せている分」で表示します（同ランクの縦ペアは
  0 点になるので、伏せたままだと確定しません）
