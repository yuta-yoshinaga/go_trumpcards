...
2 デッキ（104 枚）を使うソリティアです。16 のタブロー山を 8 つの組札（A から昇順×4、K から降順×4）に向けて整理し、すべてのカードを組札に移せば勝利です。最大 3 回までタブローの並び順を反転させる「再配り（redeal）」が使えます。
...
- 開始時に各スートの A と K を 1 枚ずつ取り出し、8 つの組札の土台にします。
...
```mermaid
flowchart TD
    A[ゲーム開始 - reset] --> B[配り直し: タブロー16x6 + 組札8]
    B --> C{プレイ中?}
    C -- Yes --> D[コマンド入力]
    D --> E{コマンド種別}
...
- 同スートの ±1 を見つけたら、まず**組札へ送る** ことを優先しましょう。
