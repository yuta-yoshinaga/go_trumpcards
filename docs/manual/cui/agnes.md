...
アグネス・ソレル（Agnes Sorel）は1人用ソリティア。クロンダイク式の7列のタブローと、キャンフィールド式のベースランクから始まる4つの組札を組み合わせたゲーム。すべてのカードを組札に積み上げることを目指す。
...
- ベースランク: 28枚を配った後の次の1枚のランクがベースランク。そのカードは組札に置かれる。
- 組札: 同スート、ベースランクから昇順にラップアラウンド（K→A→...）
...
- **Base rank**: 組札の開始ランク
- **Foundation**: 4つの組札。ベースランクから昇順に積む
- **Stock**: ストックの残り枚数
