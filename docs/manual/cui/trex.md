...
| コマンド | 短縮形 | 説明 |
|----------|--------|------|
| `choose <n>` | `c` | 王として契約 `n` を選ぶ（0=♥K, 1=ダイヤ, 2=クイーン, 3=トリック, 4=ドミノ） |
| `play <i>` | `p` | 手札の `i` 番目を出す |
| `pass` | `s` | ドミノでパスする（出せる札が無いときのみ） |
| `next` | `n` | 次のディールへ進む |
| `hint` | `h` | ヒントを表示する |
| `log` | `l` | 棋譜を表示する |
| `reset` | `r` | ゲームごとリセットする |
...
- **現在のトリックの失点札には `(失-75)` のように減点が付きます。**どの札が危険かは契約ごとに変わる（♥K 契約なら ♥K だけ、ダイヤ契約なら ♦ 全部、クイーン契約なら Q 全部）ので、暗算せずに読めます。トリック契約とドミノには個別札の減点が無いので何も付きません
