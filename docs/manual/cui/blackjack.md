...
メインベットと同時にサイドベットを賭けることができます。`bet N PP T3 HC` の形式で、PP（Perfect Pairs）、T3（21+3）のベット額、HC（ハンド数）を指定します（例: `b 100 10 20 2`）。省略した場合はそれぞれ0（賭けない）、1ハンドとして扱われます。サイドベットはカード配布直後に評価され、メインハンドとは独立に精算されます。サイドベットはハンド0のみに適用されます。
