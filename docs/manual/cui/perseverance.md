...
- 4つの組札はスート別にA→Kの順に積み上げる（Aは最初から乗っている）
- タブローは**同スートでランクが1つ下**のカードにだけ重ねる（例: ♠5の上に置けるのは♠4だけ）
- **同スート降順に並んでいる部分は、まとめて動かせる**（並びが崩れていれば1枚も動かない）。CUI では `m <列> <位置> <列>` と開始位置を指定します
- **空の列にはカードを置けない**
...
|----------|--------|------|
| `reset` | `r` | 新しいゲームを開始 |
| `move` | `m` | カードを移動する（`m <列> <列>` / `m <列> <位置> <列>` / `m <列> f`） |
...
- `m 0 1 5` → 列0の**位置1から上の並び**をまとめて列5へ移動（同スート降順に並んでいなければ1枚も動きません）
