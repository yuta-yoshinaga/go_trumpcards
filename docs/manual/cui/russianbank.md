...
| コマンド | 短縮形 | 説明 |
|----------|--------|------|
| `pf <src>` |  | 移動元を組札へ（src: r/w/or/ow/t0..t3） |
...
- `組札` … 8本のトップカードのみ。A から順に積むので、下に何があるかはトップから決まります
- `タブロー` … 共有4列。**下から順に全ての札**が並び、`[ ]` で囲まれた札がその列のトップです（`[-]` は空列）。埋もれた札まで見えるので、Web のカスケード表示と同じ情報量になります
