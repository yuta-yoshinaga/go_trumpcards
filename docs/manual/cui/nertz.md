...
Nertz（別名: Pounce, Nerts, Racing Demon）は、複数プレイヤー（既定4人 / 1人間 + 3CPU、2〜6人で設定可能）が同時並行でクロンダイク風のソリティアを進める対戦型カードゲームです。各プレイヤーは自分専用の52枚デッキで盤面を構築しつつ、中央に置かれた共有組札へ最も多くカードを送り込み、自分の「ナッツパイル（13枚）」をいち早く空にした人がそのラウンドを取ります。
...
- 同スートで値+1の順に積み上げ、Kで完成。完成後はその組札は閉鎖されます。
