# クレイジーパイナップル（CUI版）遊び方

## ゲーム概要

クレイジーパイナップル（Crazy Pineapple）は、Pineapple Poker のバリアントです。各プレイヤーに3枚のホールカードが配られ、フロップ**ベッティング終了後**に1枚をディスカードします（通常の Pineapple はフロップ公開直後にディスカード）。残り2枚とコミュニティカード5枚から最強の5枚で役を作ります。テーブルサイズは4-max（デフォルト）、6-max、9-max から選択可能です。

## 起動方法

```sh
go run ./cmd/trumpcards crazypineapple
go run ./cmd/trumpcards --lang en crazypineapple  # 英語モード
```
