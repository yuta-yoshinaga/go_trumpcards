...
----------
あなた [-] [親]
  [0]SPADE 13  [1]SPADE 3  [2]DIAMOND 2  [3]HEART 10
CPU 1: 手札4枚 [-]
CPU 2: 手札4枚 [-]
CPU 3: 手札4枚 [-]
----------
あなたの番です。p <n> で渡す札を選んでください
==========
```

- **ラウンド行**: `ラウンド1  残りスプーン: 3本` — ラウンド数と残りスプーン。
  **山札の残り枚数は出ません**。
- **文字表示**: 各行の `[-]` が獲得した S-P-O-O-N-S の文字（6文字で脱落）。
  まだ 1 文字も取っていなければ `[-]`。
- **操作行**: 案内は短縮形で出ます (`p <n>`)。`pass <n>` も同じ意味です。
  スプーン争奪中は `g` / `grab` に変わります。
- **`フェーズ: PASS` / `フェーズ: GRAB` のような行は出ません** — いま何を
  すべきかは一番下の案内行で判断します。
- **`[親]` タグ**: そのラウンドの配り手。山札から札を取っては隣へ渡し続ける役なので、この人の隣は札が回ってくるペースが速くなります。
