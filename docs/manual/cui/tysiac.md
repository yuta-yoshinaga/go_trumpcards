...
- デクレアラーは伏せられた**タロン3枚を引き取り**（手札10枚）、**各相手に1枚ずつ渡します**（デクレアラー8枚、各相手8枚、合計24枚）。CUIには、現在選んでいるカードの宛先（1枚目は最初の相手、2枚目は次の相手）が表示されます。
...
| `discard <idx>` | `d` | 表示された宛先にタロン交換のカードを1枚渡す（2回） |
...
[0]♠10  [1]♣13  [2]♣11  [3]♥10  [4]♥12  [5]♥11  [6]♥9
...
- タロン交換中は、実行例のように次の宛先が表示されます。

```
タロン交換 — デクレアラー: あなた
d <idx>・・・CPU 1に1枚渡す（次は別の相手）
```
- **`いま宣言できるマリッジ:`**: プレイ中のあなたの手番で、K と Q を両方持つスートを結婚点つきで挙げます（`♠K-Q (+40)` 形式）。対象が無ければ行ごと出ません。CPU の手番では出しません
