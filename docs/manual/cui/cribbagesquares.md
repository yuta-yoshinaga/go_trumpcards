...
- `(行,列)` は 0 始まり。`.....` は空きマス
- **行スコアと列スコアはスターターがめくられるまで 0 のまま。** これは確定した得点で、スターター次第で増えるため途中では出しません
- **`途中集計` は別枠で出ます。**スターター抜きで**すでに確定している**点だけを、点の付いた行・列について `15が4 ペア2` のように要素ごとに表示します（スターターは足すことしかしないので、この値は最終点の下限です）
- 終了時には 8 手すべての内訳（0 点の手は「なし」）と、クリア基準 61 点との比較が表示される
