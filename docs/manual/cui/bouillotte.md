...
ブイヨット（3枚ポット・ヴァイイング）
==========
ラウンド: 1  ポット: 90  アンティ: 10
ルトゥルヌ（共有カード）: DIAMOND 13
あなた  チップ 190  賭け 10  [手番]
[0]CLOVER 12  [1]DIAMOND 8  [2]CLOVER 1
CPU 1  チップ 180  賭け 20  [待機]
CPU 2  チップ 170  賭け 30  [待機]
CPU 3  チップ 170  賭け 30  [待機]
----------
アクションしてください: c（コール・必要 20）/ ra（レイズ・現在 30→40）/ f（フォールド）
c / ra / f でアクション、n で次のラウンド
==========
```

- 1行目 — ラウンド番号・現在のポット・アンティ額（**現在のベットは出ません**）
- 2行目 — `ルトゥルヌ（共有カード）: DIAMOND 13` — 共有カード。**括弧の注記付き**で出ます
- 各プレイヤー行 — 名前・チップ・このラウンドの賭け累計・状態（参加中／降りた／脱落／勝者）
- カード行 — 手札（結果フェーズでは降りていないプレイヤーの手札と役名を公開）
- アクション案内 — `c（コール・必要 N）/ ra（レイズ・現在 X→Y）/ f（フォールド）`。**必要 N** はコールに要る追加額（既に払った分を引いた額）、**Y** はレイズ後の到達額（現在のベット + アンティ）です
