...
```sh
go run ./cmd/trumpcards carioca
go run ./cmd/trumpcards --lang en carioca  # 英語モード
```
...
- 山札と捨て札（トップ1枚を残し）が両方枯渇すると流局（誰の勝ちも付かず、得点加算なし）。
...
- 他プレイヤー: 手札に残ったカードのペナルティが累計に加算される。
  - A=15点、2-9=額面、10/J/Q/K=10点、ジョーカー=25点。
...
```mermaid
flowchart TD
    A[ゲーム開始 - reset] --> B[ラウンド開始<br/>12枚配布]
    B --> C{自分の手番?}
...
捨て札: ♥7
You: 累積0点 ラウンド0点 12枚 [未達成]
  [0]♠5 [1]♥5 [2]♦5 [3]♣K [4]♥K ...
CPU 1: 累積0点 ラウンド0点 12枚 [未達成]
CPU 2: 累積0点 ラウンド0点 12枚 [未達成]
----------
手番: You (ドローフェーズ)
ds・・・山札から引く
dd・・・捨て札トップを取る
...
- ジョーカーは25点と最も高ペナルティ。ワイルドとして役に組み込むか、早めに使い切るか判断する。
