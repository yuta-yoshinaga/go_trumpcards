...
| コマンド | 短縮形 | 説明 |
|----------|--------|------|
| `discard <idx...>` | `d` | 手札のカードを捨てる (空白区切りで複数指定可) |
| `yaniv` | `y` | ヤニブを宣言する (手札合計5以下) |
| `drawstock` | `ds` | 山札からカードを引く |
| `drawpickup <0\|1>` | `dp` | 直前の捨て札の端から引く (0=先頭, 1=末尾) |
| `nextround` | `nr` | 次のラウンドへ進む |
| `setdifficulty <0-2>` | `sd` | CPU難易度を変更 (0=易, 1=普通, 2=難) |
| `setlimit <n>` | `sl` | 脱落スコアを変更 (50〜500) |
...
- あなたの手札はインデックス付き (`[0] ♠9` など) で表示され、`d <idx...>` の `<idx>` に対応します。
