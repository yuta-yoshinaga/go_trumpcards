...
==========
ラウンド: 1  トリック: 1  切り札: ♦
あなた: 10枚  チームスコア: 0  トリック: 0
[0]CLOVER 3  [1]CLOVER 12  [2]CLOVER 11  [3]CLOVER 7  [4]HEART 3  ...
CPU 1: 9枚  チームスコア: 0  トリック: 0
CPU 2: 9枚  チームスコア: 0  トリック: 0
CPU 3: 9枚  チームスコア: 0  トリック: 0
----------
トリック: CPU 1=CLOVER 4, CPU 2=CLOVER 2, CPU 3=CLOVER 5
今ラウンドの獲得点  チームA: 0点  チームB: 0点
手番: あなた
```

- **1 行目**: `ラウンド: N  トリック: M  切り札: ♦` — 切り札は**同じ行**に出ます
- **プレイヤー行**: `あなた: 10枚  チームスコア: 0  トリック: 0` —
  手札枚数・チームの累積点・獲得トリック数
- **今ラウンドの獲得点**: 現在のラウンドでチームごとに取った点数。結婚（同スートのK+Q）は宣言した瞬間に加点されるので、ここで即座に確認できます（ラウンド終了時は「ラウンド終了（…）」行が同じ値を示します）
- **`[n]`付き行**: あなたの手札（インデックス付き）
- **`宣言可能: ...`**: リード権があって**結婚宣言**（同スートの K+Q）が
  できるときに出ます。`m <suit番号>` で宣言。**「コンテ」ではありません**
- **`トリック:` 行**: `トリック: CPU 1=CLOVER 4, ...` — 現在場に出ている
  カードと出した人。1 枚も出ていなければ行ごと出ません
