...
あなた (ディフェンダー): 5枚  チップ: 0  トリック: 0
[0]SPADE 1  [1]SPADE 12  [2]CLOVER 6  [3]CLOVER 2  [4]DIAMOND 1
CPU 1 (ディフェンダー): 5枚  チップ: 0  トリック: 0
CPU 2 (ディフェンダー): 5枚  チップ: 0  トリック: 0
CPU 3 (ディフェンダー): 5枚  チップ: 0  トリック: 0
----------
入札手番: あなた（現在の最高入札: パス）
bid <0/2/3/4/5>・・・0=パス, 2/3/4=5トリック中その枚数を宣言, 5=Nap(全5トリック・高配当)／パス以外は現在の最高入札を上回る必要あり
```

- **タイトル行**: `ナップ (Nap / Napoleon)` — 日本語が先に出ます。
- **1 行目**: `ラウンド: N  トリック: M  切り札: なし` — 切り札は
  **同じ行**に出ます（宣言前は `なし`）。
- **プレイヤー行**: `あなた (ディフェンダー): 5枚  チップ: 0  トリック: 0` —
  役割・手札枚数・累積チップ・獲得トリック数。`フェーズ: BID` /
  `フェーズ: PLAY` や `入札状況:` という行は出ません。
- **`入札手番: あなた（現在の最高入札: パス）`**: 入札フェーズの操作行。
  **いま上回るべき入札**が括弧に出ます。
