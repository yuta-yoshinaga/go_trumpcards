...
```
==========
オークション・フォーティファイブズ (Auction Forty-Fives)
==========
ラウンド: 1  トリック: 1  切り札: なし
得点 — チームA: 0  チームB: 0
あなた（チームA・ディフェンダー）: 5枚  チーム得点: 0  トリック: 0
[0]DIAMOND 1  [1]CLOVER 8  [2]HEART 8  [3]HEART 7  [4]CLOVER 7
CPU 1（チームB・ディフェンダー）: 5枚  チーム得点: 0  トリック: 0
CPU 2（チームA・ディフェンダー）: 5枚  チーム得点: 0  トリック: 0
CPU 3（チームB・ディフェンダー）: 5枚  チーム得点: 0  トリック: 0
----------
入札手番: あなた（現在の最高入札: 20）
ビッド状況: あなた=-, CPU 1=15, CPU 2=パス, CPU 3=20
bid <0/15/20/25>・・・0=パス／パス以外は現在の入札を上回る必要あり。25 = Jink（全トリック取得で即勝利）
```

- **タイトル行**: `オークション・フォーティファイブズ (Auction Forty-Fives)` —
  日本語が先に出ます。
- **1 行目**: `ラウンド: N  トリック: M  切り札: なし` — 切り札は**同じ行**
  (宣言前は `なし`)。`フェーズ: BID` / `フェーズ: PLAY` の行は出ません。
- **`得点 — チームA: 0  チームB: 0`**: 両チームの累積得点。
- **プレイヤー行**: `あなた（チームA・ディフェンダー）: 5枚  チーム得点: 0  トリック: 0`。
- **`入札手番: あなた（現在の最高入札: 20）`**: **いま上回るべき額**が括弧に出ます。
- **`ビッド状況:`**: 全員の入札の現状 (`-` = まだ宣言していない)。
- **`[n]`付き行**: あなたの手札（インデックス付き）。**`!!` は最上位の切り札**（切り札の5・切り札のJ・♥A）で、♥A は切り札スートに関係なく常に切り札扱いです
- **`!!` の凡例行**: 手札に最上位の切り札があるときだけ表示。これらを持っているとリネギング（マストフォロー免除）が使えます
