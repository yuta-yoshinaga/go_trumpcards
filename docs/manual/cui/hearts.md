...
ハートブレイク: なし
上限: 100点 / 最多失点: あなた 0点
あなた: 累積0点 ラウンド0点 13枚 0トリック
[0]SPADE 5  [1]SPADE 11  [2]CLOVER 9  [3]CLOVER 11  [4]CLOVER 12  [5]HEART 4  ...
CPU 1: 累積0点 ラウンド0点 13枚 0トリック
CPU 2: 累積0点 ラウンド0点 13枚 0トリック
CPU 3: 累積0点 ラウンド0点 13枚 0トリック
----------
パスフェーズ: 左へ渡す
pass <idx> <idx> <idx>・・・3枚のカードを選択
==========
```

- `ラウンド: N  トリック: M`: 現在のラウンドとトリック番号
- `ハートブレイク: なし/あり`: ハートがブレイクされたかどうか
- `上限: N点 / 最多失点: 名前 M点`: 終了となる失点の上限と、現在いちばん
  失点している人。**この行で「あと何点で終わるか」が読めます**
- `あなた: 累積N点 ラウンドM点 X枚 Yトリック`: 累積失点・このラウンドの失点・
  手札枚数・獲得トリック数。CPU も同じ形式です
- `[0]SPADE 5`...: インデックス付きの手札カード
- `パスフェーズ: 左へ渡す`: パスの方向。パスの無いラウンドはこの行が出ません
- `トリック: あなた=SPADE 2, CPU 1=SPADE 12`: プレイフェーズで、現在の
  トリックに出ている札。**まだ 1 枚も出ていないトリックでは行ごと出ません**
