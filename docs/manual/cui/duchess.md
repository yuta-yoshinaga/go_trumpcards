...
最大の特徴は**組札の開始ランクを自分で選ぶ**こと。ゲーム開始時に 4 つの扇の一番上から 1 枚を選び、そのランクが 4 つの組札すべての開始ランクになる。
...
- 組札はスートごとに 4 つ。開始ランクから同スートで昇順、**K の次は A に折り返す**。開始ランクの 1 つ手前まで積めばその組札は完成
...
| `move r <fan> f` | `m` | リザーブからそのランクの組札へ |
