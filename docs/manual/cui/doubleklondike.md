...
```
==========
ダブル・クロンダイク / ガルガンチュア (Double Klondike)
==========
ストック: 59  ウェイスト: [空]
組札: 0/104枚
組札0: [空]
...
- `ストック: N  ウェイスト: X` … 残りのめくり可能枚数と、めくった最上位カード
  (**`山札:` ではありません**)
- `組札: N/104枚` … 8 つの組札に乗った合計枚数 / 104。**104 枚すべてを組札に上げるとクリア**なので、残り枚数の目安になります
- `組札0:`〜`組札7:` … 各スート 2 組の積み上げ先。**1 行ずつ 8 行**に出ます
- `手数` の右の括弧 … `u` で戻せるかどうか。戻せない局面では打つ前に分かる
- `列0〜8` … 場札（右端が最上位、`##` は裏向き）
