...
Préférence（プレフェランス）は中欧・東欧で広く親しまれているトリックテイキングゲームです。32枚デッキを3人でプレイします。各ラウンドの最初に**入札フェーズ**があり、最高入札者が**デクレアラー**（宣言者）として1人で2人の**ディフェンダー**と対戦します。デクレアラーがコントラクトを達成すればそのコントラクトの価値分のゲーム点を獲得し、失敗すれば**各**ディフェンダーが同じ点数を獲得します。先にターゲット（デフォルト**30**点）に達したプレイヤーがマッチ勝利です。

## 起動方法

```sh
go run ./cmd/trumpcards preference
go run ./cmd/trumpcards --lang en preference  # 英語モード
```
...
あなた (ディフェンダー): 10枚  得点: 0  トリック: 0
[0]SPADE 9  [1]CLOVER 13  [2]CLOVER 8  [3]HEART 1  [4]HEART 9  ...
CPU 1 (ディフェンダー): 10枚  得点: 0  トリック: 0
CPU 2 (ディフェンダー): 10枚  得点: 0  トリック: 0
----------
入札手番: あなた（現在の契約: パス）
bid <0-4>・・・0=パス, 1=シックス(6トリック), 2=ミゼール(0トリック・切り札なし), 3=セブン(7トリック), 4=エイト(8トリック)／パス以外は現在の入札を上回る必要あり
```

- **タイトル行**: `プレフェランス (Préférence)` — 日本語が先に出ます。
- **1 行目**: `ラウンド: N  トリック: M  切り札: なし` — 切り札は**同じ行**
  (宣言前は `なし`)。`フェーズ: BID` / `フェーズ: PLAY`、`ゲーム点:` の
  各行は出ません。
- **プレイヤー行**: `あなた (ディフェンダー): 10枚  得点: 0  トリック: 0` —
  役割・手札枚数・累積得点・獲得トリック数。
- **`入札手番: あなた（現在の契約: パス）`**: **いま上回るべき契約**が
  括弧に出ます。
