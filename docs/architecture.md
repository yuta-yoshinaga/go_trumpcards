...
- **Web API**: Sixteen endpoints -- `POST /blackjack/exec` (BlackJack), `POST /poker/exec` (Poker), `POST /oldmaid/exec` (Old Maid), `POST /daifugo/exec` (Daifugo), `POST /sevens/exec` (Sevens), `POST /doubt/exec` (Doubt), `POST /holdem/exec` (Texas Hold'em), `POST /omaha/exec` (Omaha Hold'em), `POST /hearts/exec` (Hearts), `POST /memory/exec` (Memory), `POST /klondike/exec` (Klondike), `POST /freecell/exec` (FreeCell), `POST /baccarat/exec` (Baccarat), `POST /spades/exec` (Spades), `POST /crazyeights/exec` (Crazy Eights), and `POST /ginrummy/exec` (Gin Rummy) -- accept JSON with a `Cmd` field and game state.
