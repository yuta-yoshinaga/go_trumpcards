...
- **Web API**: 261 endpoints, one per game. Each accepts JSON with a `command` field and the game state; the full list is in [Web API endpoints](#web-api-endpoints) below.
...
One `POST /<game>/exec` per registered game -- **261** in total. Every endpoint takes JSON with a
