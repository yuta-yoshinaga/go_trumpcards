...
func TestDaifugoWebController_Method(t *testing.T) {
	...
	expectedBody := `{"players":[],"currentTurn":0,"tableCards":[],"lastPlayPlayerIdx":-1,"gameEndFlag":false,"cpuActions":[],"humanAction":null,"message":""}`
	...
	t.Run("failed Exec other", func(t *testing.T) {
		...
		recorded.CodeIs(http.StatusOK)
		recorded.ContentTypeIsJson()
		recorded.BodyIs(`{"players":[],"currentTurn":0,"tableCards":[],"lastPlayPlayerIdx":0,"gameEndFlag":false,"cpuActions":[],"humanAction":null,"message":"Unsupported command."}`)
	})
